...
import { parseDateValue, currentDate } from "./dates";
...
export function generatePeriodOptions(data: any[], dateField: string): Option[] {
  ...
  if (dates.length === 0) return [{ key: 0, label: 'All Time' }];
  
  const earliestDate = dates[0];
  const latestDate = dates[dates.length - 1];
  
  // Calculate total months in dataset
  const totalMonths = (latestDate.year - earliestDate.year) * 12 
  ...
  if (totalMonths >= 12) {
    ...
    if (mediumTerm > 6) { // Only add if different from short-term
      options.push({ 
        key: mediumTerm, 
  ...
  if (totalMonths >= 18) {
    ...
    if (longTerm > 12) { // Only add if different from medium-term
      options.push({ 
        key: longTerm, 
