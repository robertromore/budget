<script lang="ts">
  ...
  let {
    ...
    handleSubmit?: (selected: number) => void;
  }>();

  const { setValue, attrStore, value } = getFormField();

  let entities: EditableEntityItem[] = $state(getContext(entityLabel));

  const findCurrentEntity = () => entities.find((entity) => entity.id == $value);
          ...
          <Command.List>
            ...
            <Command.Group>
              {#each entities as entity}
                <Command.Item
                  value={entity.id + ''}
                  class={cn($value == entity.id && "bg-muted")}
                  onSelect={() => {
                    setValue(entity.id);
                  }}
                >
                  <span
                    class={cn(
                      ...
                      $value != entity.id && "text-transparent"
                    )}
                  />
                  <div class="flex-grow">
                    {entity.name}
