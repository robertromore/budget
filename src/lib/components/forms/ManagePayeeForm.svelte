<script lang="ts">
  ...
  let { payeeId, name, notes, onDelete, onSave } = $props<{
    ...
    onSave?: (new_entity: Payee, is_new: boolean) => void
  }>();
  ...
  const form = superForm(
    superValidateSync(formInsertPayeeSchema),
    {
      ...
      onSubmit: async(input) => {
        ...
        await trpc($page).payeeRoutes.save.mutate(data).then((new_entity) => {
          if (onSave) onSave(new_entity[0], !('id' in data));
        });
      },
    }
  );
  ...
  const loadEntity = async(id: number) => {
    return await trpc($page).payeeRoutes.load.query({ id }).then((entity) => {
