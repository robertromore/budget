...
<Popover.Root>
  <Popover.Trigger asChild let:builder>
    <Button
      ...
      builders={[builder]}
    >
      <span class="icon-[lucide--calendar-days] size-4 mr-2"></span>
      {dateFormatter.format(value ? value.toDate(getLocalTimeZone()) : today(getLocalTimeZone()).toDate(getLocalTimeZone()))}
    </Button>
  </Popover.Trigger>
