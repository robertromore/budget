<script lang="ts">
  ...
  const getCacheKey = (url: string, params?: any) => params ? `${url}?${JSON.stringify(params)}` : url;
  ...
  let filters = $state({ searchQuery: '', sortBy: 'date' as const, sortOrder: 'desc' as const, dateFrom: undefined, dateTo: undefined });
  ...
  async function loadData() {
    ...
    try {
      ...
      summary = { balance: summaryData?.balance || 0, transactionCount: summaryData?.transactionCount || 0 };
  ...
  const updateTransactionData = async (id: number, columnId: string, newValue?: unknown) => {
    try {
      ...
      const fieldMap: Record<string, string> = {
        'payee': 'payeeId', 'category': 'categoryId', 'date': 'date', 'amount': 'amount', 'notes': 'notes', 'status': 'status'
      };
...
<div class="space-y-6">
  ...
  <!-- Tabs Structure -->
  <Tabs.Root bind:value={activeTab} class="w-full mb-1">
      ...
      {:else if isLoading}
        ...
        <div class="space-y-4">
          <div class="flex items-center justify-between">
            ...
            <div class="h-10 w-64 bg-muted animate-pulse rounded"></div>
          </div>
          <div class="h-[400px] bg-muted animate-pulse rounded-lg"></div>
        </div>
      {/if}
    </Tabs.Content>
  </Tabs.Root>
