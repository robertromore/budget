...
<div class="flex">
  <Badge variant="outline" class="rounded-r-none h-8">{title}</Badge>
  <Popover.Root>
    <Popover.Trigger>
      {#snippet child({ props })}
  ...
  <Button variant="outline" class="p-2 h-8 rounded-l-none border-l-0" onclick={() => activeView.removeFilter(column.id)}><X/></Button>
</div>
