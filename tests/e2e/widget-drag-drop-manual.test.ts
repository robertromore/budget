...
test.describe('Widget Drag and Drop - Manual Testing', () => {
  test('should demonstrate widget drag and drop functionality', async ({ page }) => {
    ...
    if (widgetCount >= 2) {
      ...
      const firstWidgetId = await firstWidget.getAttribute('data-widget-id') || await firstWidget.locator('[data-widget-id]').first().getAttribute('data-widget-id');
      const secondWidgetId = await secondWidget.getAttribute('data-widget-id') || await secondWidget.locator('[data-widget-id]').first().getAttribute('data-widget-id');
      ...
      await page.evaluate(async () => {
        const firstWidget = document.querySelector('.widget-wrapper[draggable="true"]') as HTMLElement;
        const secondWidget = document.querySelector('.widget-wrapper[draggable="true"]:nth-child(2)') as HTMLElement;
        ...
        if (firstWidget && secondWidget) {
          ...
          // Set drag data
          dragStartEvent.dataTransfer?.setData('text/plain', firstWidget.querySelector('[data-widget-id]')?.getAttribute('data-widget-id') || '');
      ...
      const newFirstWidgetId = await newFirstWidget.getAttribute('data-widget-id') || await newFirstWidget.locator('[data-widget-id]').first().getAttribute('data-widget-id');
      const newSecondWidgetId = await newSecondWidget.getAttribute('data-widget-id') || await newSecondWidget.locator('[data-widget-id]').first().getAttribute('data-widget-id');
      
      const newFirstWidgetText = await newFirstWidget.textContent();
      const newSecondWidgetText = await newSecondWidget.textContent();
      
      console.log(`After drag - Widget 1 ID: ${newFirstWidgetId}, Text: "${newFirstWidgetText?.slice(0, 30)}..."`);
      console.log(`After drag - Widget 2 ID: ${newSecondWidgetId}, Text: "${newSecondWidgetText?.slice(0, 30)}..."`);
      
      // Check if positions swapped
      const positionsSwapped = (firstWidgetId === newSecondWidgetId && secondWidgetId === newFirstWidgetId) ||
