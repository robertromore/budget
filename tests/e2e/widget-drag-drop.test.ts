...
test.describe('Widget Drag and Drop Functionality', () => {
  test('should handle widget drag and drop operations correctly', async ({ page }) => {
    ...
    if (widgetCount >= 2) {
      ...
      console.log(`Initial order: First="${firstInitialText?.slice(0, 20)}...", Second="${secondInitialText?.slice(0, 20)}..."`);
      ...
      console.log(`New order: First="${firstNewText?.slice(0, 20)}...", Second="${secondNewText?.slice(0, 20)}..."`);
    ...
    if (widgetCount >= 2) {
      ...
      const positionChanged = Math.abs(secondNewBox!.x - secondInitialBox!.x) > 10 || 
    ...
    if (widgetCount >= 4) {
      ...
      const multiPositionChanged = Math.abs(firstNewBox!.x - firstInitialBox!.x) > 50 || 
    ...
    if (widgetCount >= 2) {
      ...
      const monitorPositions = async () => {
        for (let i = 0; i < 10; i++) { // Monitor for 1 second
          ...
          if (box && lastPosition) {
            ...
            if (deltaX > 5 || deltaY > 5) { // Significant position change
              positionChanges++;
            }
          }
      ...
      const [dragResult] = await Promise.all([
        testWidget.dragTo(targetWidget),
        monitorPositions()
      ]);
