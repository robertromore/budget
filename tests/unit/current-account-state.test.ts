...
describe("CurrentAccountState Balance Safety", () => {
  describe("Balance Derived Value Safety", () => {
    it("should handle undefined account balance gracefully", () => {
      ...
      expect(simulateBalanceCalculation(100.50)).toBe("$100.50");
    });
    ...
    it("should handle account balance update operations safely", () => {
      ...
      // Test normal case
      const result1 = simulateAddTransaction(100.00, 25.50);
      expect(result1.balance).toBe(125.50);
      expect(result1.isValidBalance).toBe(true);

      // Test with undefined current balance
      const result2 = simulateAddTransaction(undefined, 25.50);
      expect(result2.balance).toBe(25.50);
      expect(result2.isValidBalance).toBe(true);

      // Test with NaN transaction amount
      const result3 = simulateAddTransaction(100.00, NaN);
      expect(result3.balance).toBe(100.00);
    ...
    it("should handle transaction balance updates safely", () => {
      ...
      const simulateTransactionUpdate = (
        ...
        return {
          ...
          allValidBalances: [newAccountBalance, ...updatedBalances].every(b => !isNaN(b) && isFinite(b))
        };
      };

      // Test normal update
      const result1 = simulateTransactionUpdate(150.00, 25.00, 50.00, [100.00, 125.00, 150.00]);
      expect(result1.accountBalance).toBe(175.00);
      expect(result1.transactionBalances).toEqual([125.00, 150.00, 175.00]);
      expect(result1.allValidBalances).toBe(true);

      // Test with problematic values
      const result2 = simulateTransactionUpdate(NaN, null, undefined, [NaN, 125.00]);
      expect(result2.accountBalance).toBe(0);
      expect(result2.transactionBalances).toEqual([0, 125.00]);
    ...
    it("should handle transaction deletion balance recalculation safely", () => {
      ...
      expect(result1.balance).toBe(124.50);
  ...
  describe("Transaction Formatting Safety", () => {
    it("should safely format transaction data", () => {
      ...
      const testTransactions = [
        { id: 1, amount: 100.00, balance: 100.00 },
      ...
      expect(formatted[0].amount).toBe(100.00);
      expect(formatted[0].balance).toBe(100.00);
  ...
  describe("Balance State Consistency", () => {
    it("should maintain balance consistency during state updates", () => {
      ...
      // Add transactions
      state.addTransaction(100.00);
      state.addTransaction(-25.50);
      state.addTransaction(50.00);
      
      let balances = state.getAllBalances();
      expect(balances.accountBalance).toBe(124.50);
      expect(balances.transactionBalances).toEqual([100.00, 74.50, 124.50]);
      expect(balances.formattedAccountBalance).toBe("$124.50");
      
      // Update middle transaction
      state.updateTransaction(2, -50.00);
      
      balances = state.getAllBalances();
      expect(balances.accountBalance).toBe(100.00);
      expect(balances.transactionBalances).toEqual([100.00, 50.00, 100.00]);
      ...
      balances = state.getAllBalances();
      expect(balances.accountBalance).toBe(100.00); // Should remain unchanged
      expect(balances.formattedAccountBalance).toBe("$100.00");
