  ...
  describe("Input Validation Tests", () => {
    describe("Account Name Validation", () => {
      it("should reject empty account names", async () => {
        await expect(
          caller.accountRoutes.save({ name: "" })
        ).rejects.toThrow();
      });
      ...
      it("should reject account names that are too long", async () => {
        ...
        ).rejects.toThrow("Account name must be less than 50 characters");
      });
    ...
    describe("Notes Validation", () => {
      ...
      it("should preserve special characters in notes", async () => {
        const specialNotes = 'Notes with "quotes", \'single quotes\', and allowed symbols: ()_+-=:";\'?,./ àéîõü';
        const result = await caller.accountRoutes.save({
          name: "Test Account",
          notes: specialNotes,
        });
...
Final line`;
    ...
    describe("ID Validation", () => {
      it("should reject negative account IDs", async () => {
        ...
        await expect(
          caller.accountRoutes.save({ id: -1, name: "Test" })
        ).rejects.toThrow();
      ...
      it("should reject zero as account ID", async () => {
        await expect(
          caller.accountRoutes.load({ id: 0 })
        ).rejects.toThrow();

        await expect(
          caller.accountRoutes.remove({ id: 0 })
        ).rejects.toThrow();
      });
      ...
      it("should reject non-numeric string IDs", async () => {
        ...
        await expect(
          caller.accountRoutes.load({ id: "abc123" as any })
        ).rejects.toThrow();
      });

      it("should reject decimal IDs", async () => {
        await expect(
          caller.accountRoutes.load({ id: 1.5 as any })
        ).rejects.toThrow();
      });
    });
  });
  ...
  describe("Business Logic Validation", () => {
    ...
    describe("Account Updates", () => {
      ...
      it("should update timestamp when account is modified", async () => {
        ...
        expect(new Date(updated.updatedAt).getTime())
          .toBeGreaterThan(new Date(originalAccount.updatedAt).getTime());
      });
    ...
    describe("Account Deletion", () => {
      it("should soft delete accounts", async () => {
        ...
        expect(new Date(deleted.deletedAt!).getTime())
      ...
      it("should prevent access to deleted accounts", async () => {
        ...
        await expect(
          caller.accountRoutes.load({ id: accountToDelete.id })
        ).rejects.toThrow(TRPCError);
        
        // Should not appear in all() results
        const allAccounts = await caller.accountRoutes.all();
        expect(allAccounts.find(acc => acc.id === accountToDelete.id))
          .toBeUndefined();
      });
    });
  });
  ...
  describe("Error Handling Tests", () => {
    describe("Database Errors", () => {
      it("should handle database connection failures", async () => {
        ...
        await expect(
          invalidCaller.accountRoutes.all()
        ).rejects.toThrow();
      });
    ...
    describe("SQL Injection Prevention", () => {
      it("should handle SQL injection attempts in IDs", async () => {
        ...
        for (const attempt of sqlInjectionAttempts) {
          await expect(
            caller.accountRoutes.load({ id: attempt as any })
          ).rejects.toThrow();
        }
      });
