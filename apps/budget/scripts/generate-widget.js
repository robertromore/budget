...
async function generateWidget() {
  ...
  const isSimple =
    (await question("Is this a simple metric widget? (y/n): ")).toLowerCase() === "y";
  ...
  if (isSimple) {
    ...
    const isPositiveGood =
      (await question("Is positive change good for this metric? (y/n): ")).toLowerCase() === "y";
  ...
  registry =
    registry.slice(0, componentIndex) + componentEntry + "\n" + registry.slice(componentIndex);
  ...
  types =
    types.slice(0, definitionsIndex) + widgetDefinition + "\n" + types.slice(definitionsIndex);
