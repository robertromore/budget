...
async function generateChart() {
  ...
  if (template === "bar") {
    const isGrouped =
      (await question("Support grouped data (income vs expenses)? (y/n): ")).toLowerCase() === "y";
    const primaryLabel = await question('Primary series label (e.g., "Expenses"): ');
    const secondaryLabel = isGrouped
      ? await question('Secondary series label (e.g., "Income"): ')
      : "";
  ...
  console.log("\nNext steps:");
  console.log(
    `1. Import the chart: import { ${toPascalCase(chartName)} } from '$lib/components/charts';`
  );
