...
test.describe("Schedule Workflows - End-to-End Tests", () => {
  ...
  test("should complete full schedule lifecycle - create, view, edit, delete", async ({page}) => {
    ...
    const scheduleItem = page.locator(
      '[data-testid="schedule-item"]:has-text("Monthly Rent Payment")'
    );
    if (await scheduleItem.isVisible()) {
      ...
      await page
        .locator('a:has-text("Monthly Rent Payment"), button:has-text("Monthly Rent Payment")')
        .click();
    }
  ...
  test("should handle recurring schedule configuration", async ({page}) => {
    ...
    const frequencySelect = page.locator(
      'select[name="frequency"], select:has(option[value="weekly"])'
    );
    ...
    const recurringIndicator = page.locator(
      '[data-testid="recurring-indicator"], .recurring-icon, text="Recurring"'
    );
  ...
  test("should handle schedule status changes", async ({page}) => {
    ...
    const inactiveIndicator = page.locator(
      '[data-testid="inactive-indicator"], .inactive-status, text="Inactive"'
    );
  ...
  test("should handle form validation errors", async ({page}) => {
    ...
    const nameError = page.locator(
      'text="Name is required", .error:has-text("name"), [data-error]:has-text("name")'
    );
    const amountError = page.locator(
      'text="Amount is required", .error:has-text("amount"), [data-error]:has-text("amount")'
    );
  ...
  test("should handle schedule search and filtering", async ({page}) => {
    ...
    const statusFilter = page.locator(
      'select[name="statusFilter"], select:has(option[value="active"])'
    );
  ...
  test("should handle bulk schedule operations", async ({page}) => {
    ...
    const selectAllCheckbox = page.locator(
      'input[type="checkbox"][name="selectAll"], thead input[type="checkbox"]'
    );
    if (await selectAllCheckbox.isVisible()) {
      ...
      const bulkDeleteButton = page.locator(
        'button:has-text("Delete Selected"), button:has-text("Bulk Delete")'
      );
      const bulkStatusButton = page.locator(
        'button:has-text("Change Status"), button:has-text("Bulk Status")'
      );
      ...
      if (await bulkDeleteButton.isVisible()) {
        ...
        const confirmButton = page.locator(
          'button:has-text("Confirm"), button:has-text("Delete All")'
        );
        if (await confirmButton.isVisible()) {
          await confirmButton.click();
          await page.waitForLoadState("networkidle");
        ...
        const statusOption = page.locator(
          'button:has-text("Inactive"), select option[value="inactive"]'
        );
        if (await statusOption.isVisible()) {
          ...
          const inactiveIndicators = page.locator(
            '[data-testid="inactive-indicator"], .inactive-status'
          );
