...
test.describe("Individual Account Page Integration Tests", () => {
  ...
  test.describe("Individual Account Page Deep Integration", () => {
    test("should display complete account header with all metadata", async ({page}) => {
      ...
      await expect(page.locator("h1, [data-testid='account-title']")).toContainText(
        /Test (Checking|Savings)/
      );
      ...
      if (await balanceElement.isVisible()) {
        ...
        await expect(
          page.locator("[data-testid='balance-amount']") || page.locator(".balance, .amount")
        ).toBeVisible();
      }

      // Verify account metadata (creation date, account type, notes)
      await expect(
        page.locator("[data-testid='account-created']") || page.getByText(/Created:|Since:|Opened:/)
      ).toBeVisible();

      // Check for account slug/ID display
      await expect(
        page.locator("[data-testid='account-slug']") || page.getByText(/test-checking|test-savings/)
      ).toBeVisible();
    });
    ...
    test("should show comprehensive transaction list with sorting options", async ({page}) => {
      ...
      const transactionRows =
        page.locator("[data-testid='transaction-row']") || page.locator("tbody tr");
    ...
    test("should provide comprehensive transaction filtering capabilities", async ({page}) => {
      ...
      if (await startDateFilter.isVisible()) {
        ...
        const endDateFilter =
          page.locator("[data-testid='end-date-filter']") ||
      ...
      const minAmountFilter =
        page.locator("[data-testid='min-amount-filter']") ||
    ...
    test("should support advanced search with multiple criteria", async ({page}) => {
      ...
      if (await advancedSearchButton.isVisible()) {
        ...
        await expect(
          page.locator("[data-testid='advanced-search-panel']") || page.locator("[role='dialog']")
        ).toBeVisible();
      }
    });
    ...
    test("should handle transaction CRUD operations with proper validation", async ({page}) => {
      ...
      if (await addTransactionButton.isVisible()) {
        ...
        const transactionForm =
          page.locator("[data-testid='transaction-form']") ||
          page.locator("form") ||
        ...
        const dateInput = page.locator("input[name='date']") || page.locator("input[type='date']");
        ...
        await expect(
          page.getByText(/Transaction added|Success|Created/i) ||
    ...
    test("should provide detailed transaction view with edit capabilities", async ({page}) => {
      ...
      if (await firstTransaction.isVisible()) {
        ...
        const editButton =
          page.locator("[data-testid='edit-transaction']") ||
        ...
        if (await editButton.isVisible()) {
          ...
          const editForm =
            page.locator("[data-testid='transaction-edit-form']") ||
    ...
    test("should provide robust account data export functionality", async ({page}) => {
      ...
      if (await exportButton.isVisible()) {
        ...
        const exportDialog =
          page.locator("[data-testid='export-dialog']") || page.locator("[role='dialog']");
        ...
        const exportDateRange =
          page.locator("[data-testid='export-date-range']") ||
        ...
        const closeExportButton =
          page.locator("button:has-text('Close')") || page.locator("[data-testid='close-export']");
      ...
      if (await reconcileButton.isVisible()) {
        ...
        if (await endingBalanceInput.isVisible()) {
          ...
          const differenceDisplay =
            page.locator("[data-testid='balance-difference']") ||
          ...
          const markClearedCheckbox =
            page.locator("input[type='checkbox'][data-testid='mark-cleared']").first() ||
          ...
          if (await markClearedCheckbox.isVisible()) {
            ...
            const runningBalance =
              page.locator("[data-testid='running-balance']") || page.getByText(/Running Balance:/);
          ...
          const finishReconcileButton =
            page.locator("button:has-text('Finish Reconciliation')") ||
          ...
          if (await finishReconcileButton.isVisible()) {
            ...
            const balancedIndicator =
              page.locator("[data-testid='balanced']") || page.getByText(/Balanced|Match/);
            if (await balancedIndicator.isVisible()) {
              await finishReconcileButton.click();
              await expect(
                page.getByText(/Reconciliation completed|Account reconciled/i)
              ).toBeVisible();
            }
          }
        }
    ...
    test("should support comprehensive bulk transaction operations", async ({page}) => {
      ...
      if (await selectAllCheckbox.isVisible()) {
        ...
        const bulkCategorizeButton =
          page.locator("[data-testid='bulk-categorize']") ||
        ...
        if (await bulkCategorizeButton.isVisible()) {
          ...
          const categorySelectDialog =
            page.locator("[data-testid='bulk-category-dialog']") || page.locator("[role='dialog']");
    ...
    test("should provide comprehensive account management and settings", async ({page}) => {
      ...
      if (await accountSettingsButton.isVisible()) {
        ...
        const settingsPanel =
          page.locator("[data-testid='account-settings-panel']") || page.locator("[role='dialog']");
        await expect(settingsPanel).toBeVisible();

        // Check for account editing options
        const editAccountNameInput =
        ...
        const accountNotesTextarea =
          page.locator("textarea[name='notes']") || page.locator("[data-testid='account-notes']");
        ...
        const accountPreferences =
          page.locator("[data-testid='account-preferences']") ||
        ...
        const colorPicker =
          page.locator("[data-testid='account-color']") || page.locator("input[type='color']");
        ...
        const saveSettingsButton =
          page.locator("button[type='submit']") || page.locator("button:has-text('Save Settings')");
      ...
      if (await deleteAccountButton.isVisible()) {
        ...
        const deleteWarningDialog =
          page.locator("[data-testid='delete-account-warning']") ||
        ...
        const confirmationInput =
          page.locator("input[name='confirmationText']") ||
    ...
    test("should handle complex error states and recovery gracefully", async ({page}) => {
      ...
      const refreshButton =
        page.locator("button:has-text('Refresh')") || page.locator("[data-testid='refresh-data']");
