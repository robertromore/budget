...
export class TestEnvironment {
   ...
   */
  static withTiming<T extends (...args: any[]) => any>(fn: T, maxExecutionMs = 1000): T {
    return ((...args: Parameters<T>) => {
      const start = Date.now();
      const result = fn(...args);
      const duration = Date.now() - start;

      expect(duration, `Execution took ${duration}ms, expected < ${maxExecutionMs}ms`).toBeLessThan(
        maxExecutionMs
      );
...
export class DatabaseTestHelpers {
   ...
   */
  static async expectRecordNotExists<T>(findFn: () => Promise<T | null>): Promise<void> {
    const record = await findFn();
