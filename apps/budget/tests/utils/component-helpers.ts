...
export class ComponentTestHelpers {
   ...
   */
  static async measureRenderTime<T>(renderFn: () => Promise<T>, maxRenderTimeMs = 100): Promise<T> {
    ...
    expect(
      duration,
      `Render took ${duration.toFixed(2)}ms, expected < ${maxRenderTimeMs}ms`
...
export class ReactiveTestHelpers {
   ...
   */
  static testStateRune<T>(initialValue: T, updateFn: (current: T) => T, expectedValue: T): void {
   ...
   * Test effect cleanup
   */
  static testEffectCleanup(setupEffect: () => () => void, triggerCleanup: () => void): void {
    const cleanup = vi.fn();
...
export class InteractionTestHelpers {
   ...
   */
  static simulateDragDrop(source: Element, target: Element): void {
