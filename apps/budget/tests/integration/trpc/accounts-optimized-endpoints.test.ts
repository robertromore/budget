...
describe("Optimized Account Endpoints Integration Tests", () => {
  ...
  describe("All Account Summaries Endpoint", () => {
    test("should load all account summaries efficiently", async () => {
      ...
      expect(firstAccountSummary?.balance).toBe(-50.0);
      expect(firstAccountSummary?.transactionCount).toBe(1);

      expect(secondAccountSummary?.balance).toBe(100.0);
  ...
  describe("Transaction Search and Filtering", () => {
    ...
    test("should search transactions by amount", async () => {
      ...
      expect(result.transactions[0].amount).toBe(-85.5);
    });
  ...
  describe("Performance Considerations", () => {
    test("should handle large datasets efficiently", async () => {
      ...
      console.log(
        `Paginated load time for ${largeDatasetSize} transactions: ${(endTime - startTime).toFixed(2)}ms`
      );
    });
    ...
    test("should efficiently load account summaries vs full data", async () => {
      ...
      console.log(
        `Summary load: ${summaryTime.toFixed(2)}ms vs Full load: ${fullTime.toFixed(2)}ms`
      );
      console.log(
        `Performance improvement: ${(((fullTime - summaryTime) / fullTime) * 100).toFixed(1)}%`
      );
    });
  });
});
