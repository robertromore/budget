...
describe("Views tRPC Integration Tests", () => {
      ...
      await db.insert(views).values([
        ...
        },
        {
          ...
          filters: JSON.stringify([
            { column: "status", filter: "equals", value: ["cleared"] }
          ])
        },
        {
          name: "Custom View",
  ...
  describe("views.save", () => {
    ...
    describe("Updating existing views", () => {
      test("should update existing view", async () => {
        const [existing] = await db.insert(views).values({
          name: "Original View",
    ...
    describe("Validation errors", () => {
      test("should reject empty name", async () => {
        await expect(caller.viewsRoutes.save({ name: "" })).rejects.toThrow("Name must contain at least 2 characters");
        await expect(caller.viewsRoutes.save({ name: "a" })).rejects.toThrow("Name must contain at least 2 characters");
      });

      test("should reject description longer than 500 characters", async () => {
        const longDescription = "a".repeat(501);
        await expect(caller.viewsRoutes.save({
          name: "Valid Name",
  ...
  describe("views.remove", () => {
    ...
    test("should throw NOT_FOUND for non-existent view", async () => {
      await expect(caller.viewsRoutes.remove({ id: 999 })).rejects.toThrow("View not found or could not be deleted");
    });
  ...
  describe("views.delete (bulk)", () => {
    test("should hard delete multiple views", async () => {
      const views1 = await db.insert(views).values([
    ...
    test("should handle mix of valid and invalid IDs", async () => {
      const [validView] = await db.insert(views).values({
        name: "Valid View"
      }).returning();
    ...
    test("should maintain JSON data integrity", async () => {
      const complexData = {
        ...
        display: {
          ...
          sort: [{ id: "date", desc: true }, { id: "amount", desc: false }],
  ...
  describe("Edge cases", () => {
    ...
    test("should handle array values in filters", async () => {
      const filtersWithArrays: ViewFilter[] = [
        { column: "category", filter: "in", value: ["food", "transport", "utilities", "entertainment"] },
  ...
  describe("Business logic", () => {
    ...
    test("should handle view icons consistently", async () => {
      const iconTypes = [
        "table", "grid", "list", "chart", "filter",
        "eye", "settings", "star", "bookmark", "folder"
      ];
