  ...
  describe("transactions.save", () => {
    describe("Creating new transactions", () => {
      test("should create basic transaction with required fields", async () => {
        ...
        // Verify in database
        const dbTransaction = await db.select().from(transactions).where(eq(transactions.id, result.id));
    ...
    describe("Updating existing transactions", () => {
      test("should update existing transaction", async () => {
        const [existing] = await db.insert(transactions).values({
          accountId: testAccount.id,
          amount: 100.00,
          status: "pending",
          date: "2023-01-15"
    ...
    describe("Account validation", () => {
      ...
      test("should throw NOT_FOUND for deleted account", async () => {
        const [deletedAccount] = await db.insert(accounts).values({
    ...
    describe("Amount validation", () => {
      ...
      test("should accept valid currency precision", async () => {
        const validAmounts = [100.00, 50.25, 0.01, -25.99];
  ...
  describe("transactions.delete", () => {
    ...
    test("should soft delete multiple transactions", async () => {
      const transactions1 = await db.insert(transactions).values([
        { accountId: testAccount.id, amount: 100.00, date: "2023-01-15" },
        { accountId: testAccount.id, amount: 50.00, date: "2023-01-16" },
        { accountId: testAccount.id, amount: 25.00, date: "2023-01-17" }
      ]).returning();
    ...
    test("should enforce maximum deletion limit", async () => {
      ...
      await expect(caller.transactionRoutes.delete({
        entities: tooManyIds,
        accountId: testAccount.id
