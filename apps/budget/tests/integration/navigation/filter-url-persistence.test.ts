...
test.describe("Filter URL Persistence", () => {
  ...
  test("should persist view selection in URL parameters", async ({page}) => {
    ...
    const viewSelector = page
      .locator('[data-testid="view-selector"], .view-selector, [role="combobox"]')
      .first();
  ...
  test("should persist filter selections in URL parameters", async ({page}) => {
    ...
    const filterButtons = page.locator(
      'button:has-text("Category"), button:has-text("Payee"), button:has-text("Date")'
    );
    ...
    if (filterCount > 0) {
      ...
      await page
        .waitForSelector('[role="option"], .filter-option', {
          state: "visible",
          timeout: 5000,
        })
