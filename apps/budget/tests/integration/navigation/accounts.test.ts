...
test.describe("Accounts Navigation Tests", () => {
  ...
  test.describe("Individual Account Page Navigation", () => {
    ...
    test("should display account balance and summary", async ({ page }) => {
      ...
      await expect(page.locator("[data-testid='account-balance']") || 
                  page.getByText(/Balance:|Current Balance:/)).toBeVisible();
      
      // Should show transaction count or summary
      await expect(page.locator("[data-testid='transaction-count']") ||
                  page.getByText(/transactions/i)).toBeVisible();
      
      // Should show account metadata
      await expect(page.locator("[data-testid='account-created']") ||
                  page.getByText(/Created:/)).toBeVisible();
    });
    ...
    test("should sort transactions by different columns", async ({ page }) => {
      ...
      const amountHeader = page.locator("[data-testid='sort-amount']") ||
                          page.locator("th:has-text('Amount')");
      ...
      if (await amountHeader.isVisible()) {
        ...
        await expect(page.locator("[data-testid='sort-indicator']") ||
                    amountHeader.locator("svg")).toBeVisible();
      ...
      // Try sorting by date
      const dateHeader = page.locator("[data-testid='sort-date']") ||
    ...
    test("should show transaction details in modal or expanded view", async ({ page }) => {
      ...
      if (await transactionRow.isVisible()) {
        ...
        await expect(page.locator("[data-testid='transaction-details']") ||
                    page.locator("[role='dialog']") ||
                    page.locator(".transaction-detail")).toBeVisible();
    ...
    test("should delete transaction with confirmation", async ({ page }) => {
      ...
      if (await deleteButton.isVisible()) {
        ...
        await expect(page.locator("[data-testid='delete-confirmation']") ||
                    page.getByText(/Are you sure|Delete transaction/i)).toBeVisible();
    ...
    test("should show account statistics and charts", async ({ page }) => {
      ...
      // Look for charts
      const chart = page.locator("[data-testid='account-chart']") ||
                   page.locator("canvas") ||
                   page.locator(".chart");
    ...
    test("should show account settings and preferences", async ({ page }) => {
      ...
      const settingsButton = page.locator("[data-testid='account-settings']") ||
    ...
    test("should handle bulk transaction operations", async ({ page }) => {
      ...
      if (await selectAllCheckbox.isVisible()) {
        ...
        await expect(page.locator("[data-testid='bulk-actions']") ||
                    page.getByText(/selected|Bulk actions/i)).toBeVisible();
    ...
    test("should show transaction categories and tags", async ({ page }) => {
      ...
      const categoryColumn = page.locator("[data-testid='transaction-category']").first() ||
      ...
      if (await categoryColumn.isVisible()) {
        ...
        const categoryFilter = page.locator("[data-testid='category-filter']") ||
                             page.getByText(/Filter by category/i);
      ...
      const transactionTag = page.locator("[data-testid='transaction-tag']").first() ||
    ...
    test("should handle transaction recurring patterns", async ({ page }) => {
      ...
      const recurringIndicator = page.locator("[data-testid='recurring-transaction']") ||
      ...
      if (await recurringIndicator.isVisible()) {
        ...
        await expect(page.locator("[data-testid='recurring-details']") ||
                    page.getByText(/Every|Monthly|Weekly/i)).toBeVisible();
        
        // Should have options to manage recurrence
        const manageRecurring = page.locator("[data-testid='manage-recurring']") ||
    ...
    test("should display transaction attachments and receipts", async ({ page }) => {
      ...
      const attachmentIcon = page.locator("[data-testid='transaction-attachment']") ||
      ...
      if (await attachmentIcon.isVisible()) {
        ...
        await expect(page.locator("[data-testid='attachment-preview']") ||
                    page.locator("[role='dialog']") ||
                    page.getByText(/Download|Preview/i)).toBeVisible();
    ...
    test("should handle transaction splits and multiple categories", async ({ page }) => {
      ...
      const splitIndicator = page.locator("[data-testid='split-transaction']") ||
      ...
      if (await splitIndicator.isVisible()) {
        ...
        await expect(page.locator("[data-testid='split-details']") ||
                    page.locator(".split-breakdown")).toBeVisible();
    ...
    test("should show account balance history and trends", async ({ page }) => {
      ...
      const balanceHistory = page.locator("[data-testid='balance-history']") ||
      ...
      if (await balanceHistory.isVisible()) {
        ...
        const timePeriodButtons = page.locator("[data-testid='time-period']") ||
    ...
    test("should handle transaction imports and bank sync", async ({ page }) => {
      ...
      const importButton = page.locator("[data-testid='import-transactions']") ||
      ...
      if (await importButton.isVisible()) {
        ...
        await expect(page.locator("[data-testid='import-dialog']") ||
                    page.locator("[role='dialog']")).toBeVisible();
        ...
        const bankSyncOption = page.locator("[data-testid='bank-sync']") ||
                             page.getByText(/Connect Bank|Sync Bank/i);
    ...
    test("should validate transaction amounts and dates", async ({ page }) => {
      ...
      if (await addButton.isVisible()) {
        ...
        if (await futureDateField.isVisible()) {
          ...
          const warningOrSuccess = page.locator("[data-testid='warning'], [data-testid='success']") ||
    ...
    test("should show transaction pending and cleared status", async ({ page }) => {
      ...
      const pendingTransaction = page.locator("[data-testid='transaction-pending']") ||
      ...
      if (await pendingTransaction.isVisible()) {
        ...
        const markClearedButton = page.locator("[data-testid='mark-cleared']") ||
        ...
        if (await markClearedButton.isVisible()) {
          ...
          await expect(page.locator("[data-testid='transaction-cleared']") ||
                      page.getByText(/Cleared/i)).toBeVisible();
        }
      }
    ...
    test("should handle account reconciliation", async ({ page }) => {
      ...
      if (await reconcileButton.isVisible()) {
        ...
        await expect(page.locator("[data-testid='reconciliation-dialog']") ||
                    page.locator("[role='dialog']")).toBeVisible();
        
        // Should have ending balance field
        const endingBalanceField = page.locator("input[name='endingBalance']") ||
    ...
    test("should show transaction notes and memos in detail view", async ({ page }) => {
      ...
      if (await transactionRow.isVisible()) {
        ...
        if (await notesSection.isVisible()) {
          ...
          const editNotesButton = page.locator("button:has-text('Edit Notes')") ||
          ...
          if (await editNotesButton.isVisible()) {
            ...
            const notesEditor = page.locator("textarea[name='notes']") ||
    ...
    test("should handle account transfer transactions", async ({ page }) => {
      ...
      const transferButton = page.locator("[data-testid='transfer-funds']") ||
      ...
      if (await transferButton.isVisible()) {
        ...
        await expect(page.locator("[data-testid='transfer-dialog']") ||
                    page.locator("[role='dialog']")).toBeVisible();
        
        // Should have account selection
        const toAccountSelect = page.locator("select[name='toAccount']") ||
        ...
        if (await toAccountSelect.isVisible()) {
          ...
          // Submit transfer
          const transferSubmitButton = page.locator("button[type='submit']") ||
                                      page.locator("button:has-text('Transfer')");
    ...
    test("should display transaction geolocation and merchant info", async ({ page }) => {
      ...
      const merchantInfo = page.locator("[data-testid='transaction-merchant']") ||
      ...
      if (await merchantInfo.isVisible()) {
        ...
        const locationInfo = page.locator("[data-testid='transaction-location']") ||
                           page.locator(".location-info");
      ...
      const categoryButton = page.locator("[data-testid='auto-categorize']") ||
  ...
  test.describe("Account Management Navigation", () => {
    test("should open add account dialog", async ({ page }) => {
      ...
      const addAccountButton = page.locator("[data-testid='add-account-button']") ||
    ...
    test("should open edit account dialog from account page", async ({ page }) => {
      ...
      // Click edit button
      const editAccountButton = page.locator("[data-testid='edit-account-button']") ||
    ...
    test("should close dialogs when cancelled", async ({ page }) => {
      ...
      const addDialogButton = page.locator("[data-testid='add-account-button']") ||
                            page.locator("button:has-text('Add Account')");
