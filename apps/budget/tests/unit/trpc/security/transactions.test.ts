...
describe("Transaction Security - Unit Tests", () => {
  ...
  describe("Amount Validation", () => {
    ...
    it("should accept valid currency amounts", async () => {
      const validAmounts = [10.5, -25.99, 0.01, 999999.99, -999999.99];
  ...
  describe("Error Handling", () => {
    ...
    it("should handle invalid transaction queries", async () => {
      await expect(caller.transactionRoutes.forAccount({id: 99999})).resolves.toBeDefined(); // Should return empty array, not error
    });
  });
});
