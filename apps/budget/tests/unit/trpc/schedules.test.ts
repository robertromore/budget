...
describe("Schedule tRPC Routes - Unit Tests", () => {
  ...
  describe("Schedule CRUD Operations", () => {
    ...
    it("should delete a schedule", async () => {
      ...
      expect(caller.scheduleRoutes.load({id: created.id})).rejects.toThrow(TRPCError);
    });
  });
  ...
  describe("Amount Types", () => {
    ...
    it("should handle range amount type with both amounts", async () => {
      ...
      expect(result.amount_type).toBe("range");
      expect(result.amount).toBe(50.0);
      expect(result.amount_2).toBe(150.0);
    });
  });

  describe("Error Handling", () => {
    it("should return NOT_FOUND for non-existent schedule ID", async () => {
      expect(caller.scheduleRoutes.load({id: 99999})).rejects.toThrow(TRPCError);
    ...
    it("should return NOT_FOUND when deleting non-existent schedule", async () => {
      expect(caller.scheduleRoutes.remove({id: 99999})).rejects.toThrow(TRPCError);
    ...
    it("should handle invalid ID types", async () => {
      expect(caller.scheduleRoutes.load({id: "invalid" as any})).rejects.toThrow();

      expect(caller.scheduleRoutes.remove({id: "invalid" as any})).rejects.toThrow();
    });

    it("should handle negative IDs", async () => {
      expect(caller.scheduleRoutes.remove({id: -1})).rejects.toThrow();
    });
  });
  ...
  describe("Rate Limiting and Performance", () => {
    ...
    it("should not rate limit queries", async () => {
      const queries = Array.from({length: 20}, () => caller.scheduleRoutes.all());
