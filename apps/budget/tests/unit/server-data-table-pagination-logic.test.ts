...
describe("ServerDataTablePagination Logic", () => {
  ...
  describe("Display Information", () => {
    ...
    it("should format pagination text correctly", () => {
      ...
      expect(formatPaginationText(0, 50, 150))
        .toBe("Showing 1 to 50 of 150 transactions");

      expect(formatPaginationText(2, 50, 125))
        .toBe("Showing 101 to 125 of 125 transactions");

      expect(formatPaginationText(0, 50, 0))
        .toBe("No items to display");

      expect(formatPaginationText(0, 50, 25))
        .toBe("Showing 1 to 25 of 25 transactions");
    });
  });
  ...
  describe("Accessibility", () => {
    ...
    it("should announce page changes for screen readers", () => {
      ...
      expect(generatePageChangeAnnouncement(2, 10, 500))
        .toBe("Navigated to page 3 of 10. Showing 500 total items.");

      expect(generatePageChangeAnnouncement(0, 1, 25))
        .toBe("Navigated to page 1 of 1. Showing 25 total items.");
    });
  });
});