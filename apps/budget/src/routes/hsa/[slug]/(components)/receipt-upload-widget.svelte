...
const receiptsQuery = $derived(
  rpc.medicalExpenses.getReceipts(medicalExpenseId).options()
);
...
async function handleUpload() {
  ...
  try {
    ...
    await rpc.medicalExpenses.uploadReceipt.execute(uploadData);
...
async function handleDelete(receiptId: number) {
  ...
  try {
    await rpc.medicalExpenses.deleteReceipt.execute({
      id: receiptId,
      medicalExpenseId,
    });
  } catch (err: any) {
  ...
  <div class="space-y-6">
    ...
    <div>
      <div
        role="button"
        tabindex="0"
        class={`border-2 border-dashed rounded-lg p-6 text-center transition-colors ${isDragging ? 'border-primary bg-primary/5' : ''}`}
        ondrop={handleDrop}
      ...
      >
        {#if selectedFile}
          ...
          <div class="flex items-center justify-between bg-muted p-4 rounded-md">
    ...
    {#if receipts.length > 0}
      <div class="space-y-2">
        <h4 class="font-medium text-sm">Uploaded Receipts ({receipts.length})</h4>
        <div class="space-y-2">
            ...
            <div class="flex items-center justify-between p-3 bg-muted rounded-md">
              <div class="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div class="flex-1 min-w-0">
                  <p class="font-medium text-sm truncate">{receipt.fileName}</p>
