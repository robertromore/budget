...
async function handleCreateClaim() {
  ...
  try {
    if (editingClaim) {
      await rpc.medicalExpenses.updateClaim.execute({
        id: editingClaim.id,
...
async function submitStatusUpdate() {
  ...
  try {
    ...
    switch (newStatus) {
      ...
      case 'in_review':
        await rpc.medicalExpenses.markClaimInReview.execute({
          id: selectedClaim.id,
          medicalExpenseId: expense.id,
          reviewDate: dateStr,
        });
      ...
      case 'denied':
        await rpc.medicalExpenses.denyClaim.execute({
          id: selectedClaim.id,
          medicalExpenseId: expense.id,
          denialReason,
      ...
      case 'resubmission_required':
        await rpc.medicalExpenses.requestResubmission.execute({
          id: selectedClaim.id,
          medicalExpenseId: expense.id,
          reason: denialReason,
        });
      ...
      case 'withdrawn':
        await rpc.medicalExpenses.withdrawClaim.execute({
          id: selectedClaim.id,
          medicalExpenseId: expense.id,
        });
        break;
...
async function confirmDeleteClaim() {
  ...
  try {
    await rpc.medicalExpenses.deleteClaim.execute({
      id: claimToDelete.id,
      medicalExpenseId: expense.id,
    });
...
<ResponsiveSheet bind:open {onOpenChange} side="right" resizable={true} defaultWidth={800} minWidth={600} maxWidth={1200}>
  {#snippet header()}
    <div>
      <h2 class="text-lg font-semibold">Claim Management</h2>
