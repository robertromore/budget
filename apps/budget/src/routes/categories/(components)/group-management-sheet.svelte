...
const groupedCategories = $derived.by(() => {
  ...
  for (const category of categories) {
    const groupKey = category.groupId ? `group-${category.groupId}` : 'ungrouped';
...
<ResponsiveSheet bind:open>
  ...
  {#snippet content()}
    <Tabs.Root bind:value={activeTab} class="w-full">
      ...
      <!-- Manage Tab -->
      <Tabs.Content value="manage" class="mt-4 space-y-4">
        <!-- Loading State -->
        {#if isLoading}
        ...
        {:else if groups.length > 0}
          <div class="space-y-3">
            {#each groups as group (group.id)}
              ...
              <div
                class="bg-card rounded-lg border transition-all {isDropTarget
                    ...
                    <div class="flex min-w-0 flex-1 items-center gap-3">
                      ...
                      <div class="min-w-0 flex-1">
                        ...
                        <span class="text-muted-foreground text-xs">
                          {group.memberCount}
                          {group.memberCount === 1 ? 'category' : 'categories'}
                        </span>
                      </div>
                    </div>
                ...
                {#if groupCategories.length > 0}
                  <div class="divide-y">
                    {#each groupCategories as category (category.id)}
                      {@const catIconData = category.categoryIcon
                        ? getIconByName(category.categoryIcon)
                        : null}
                      {@const CategoryIcon = catIconData?.icon ?? Tag}
                      {@const isDraggedItem = isDragging && draggedCategory?.id === category.id}
                      <div
                        class="category-row hover:bg-muted/50 p-3 transition-colors {isDraggedItem
                          ? 'opacity-50'
                          : ''}">
                        <div class="flex items-center justify-between gap-3">
                          <div class="flex min-w-0 flex-1 items-center gap-3">
                            ...
                            <CategoryIcon
                              class="h-5 w-5 shrink-0 {category.categoryColor
                                ? ''
                          ...
                          <Button
                            size="icon"
                            variant="ghost"
                            class="text-destructive hover:text-destructive hover:bg-destructive/10 h-7 w-7 shrink-0"
        ...
        {:else}
          <div class="flex flex-col items-center justify-center py-12 text-center">
            ...
            <h3 class="mb-1 font-semibold">No groups yet</h3>
      ...
      <!-- Create/Edit Tab -->
      <Tabs.Content value="create" class="mt-4 space-y-4">
  ...
  <AlertDialog.Content>
    <AlertDialog.Header>
      ...
      <AlertDialog.Description>
        {#if selectedGroup}
          Are you sure you want to delete "{selectedGroup.name}"? This will unassign all categories
