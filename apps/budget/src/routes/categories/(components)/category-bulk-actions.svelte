...
const selectedCategories = $derived(selectedRows.map((row) => row.original));
const totalCount = $derived(allCategories.length);
...
const selectedOrAllCategories = $derived.by(() => {
  ...
  <div class="bg-muted flex items-center gap-2 rounded-md border px-4 py-2">
    <div class="flex items-center gap-2">
      <span class="text-sm font-medium">
        {displayCount}
        {displayCount === 1 ? 'category' : 'categories'} selected
      </span>
    ...
    <div class="ml-auto flex items-center gap-2">
      <Button onclick={() => onBulkDelete(selectedOrAllCategories)} variant="destructive" size="sm">
