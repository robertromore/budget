          ...
          <Tabs.Content value="analytics" class="mt-0 space-y-8">
                            ...
                            <foreignObject
                              ...
                              <div class="rounded-md bg-slate-900 p-2 text-white shadow-lg">
                                <p class="text-sm font-medium">
                                  {indexToLabelMap.get(point.index) ?? point.month}
                                </p>
            ...
            <section>
              ...
              {:else if topPayeesQuery.data && topPayeesQuery.data.length > 0}
                {@const totalAmount = topPayeesQuery.data.reduce(
                  (sum, p) => sum + Math.abs(p.totalAmount),
                  0
                )}
                <div class="space-y-4 rounded-lg border p-6">
                  {#each topPayeesQuery.data as payee}
                    {@const percentage =
                      totalAmount > 0 ? (Math.abs(payee.totalAmount) / totalAmount) * 100 : 0}
                    <div>
                      <div class="mb-2 flex items-center justify-between text-sm">
                        <div class="flex items-center gap-2">
                          <User class="text-muted-foreground h-4 w-4" />
                          <a href="/payees/{payee.payeeSlug}" class="font-medium hover:underline">
                            {payee.payeeName}
                          </a>
                          <span class="text-muted-foreground">({payee.transactionCount} txns)</span>
                        </div>
                        <span class="font-semibold"
                          >{formatCurrency(Math.abs(payee.totalAmount))}</span>
                      </div>
            ...
            <section>
              ...
              <div class="space-y-6">
                ...
                <div class="rounded-lg border p-6">
                  ...
                  {#if statsQuery?.data && statsQuery.data.monthlyAverage > 0}
                    ...
                    <div class="grid gap-4 md:grid-cols-3">
                      ...
                      <div class="rounded-lg bg-slate-50 p-4 dark:bg-slate-900">
                        ...
                        <p class="text-2xl font-bold">
                          {formatCurrency(Math.abs(monthlyAvg * 12))}
                        </p>
                  ...
                  {:else}
                    <div
                      class="text-muted-foreground rounded-lg border border-dashed p-8 text-center">
                    ...
                    <div class="space-y-4">
                      <div
                        ...
                        <div>
                          ...
                          <p class="text-muted-foreground text-sm">
                            Based on your average of {formatCurrency(
                              Math.abs(stats.monthlyAverage)
                            )} with 10% buffer
                          </p>
                        </div>
                      </div>
