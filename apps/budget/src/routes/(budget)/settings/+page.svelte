          ...
          <button
            ...
            <div class="flex gap-0.5">
              {#each previewColors as color}
                <div class="h-6 w-3 first:rounded-l last:rounded-r" style:background-color={color}>
                </div>
              {/each}
            </div>
    ...
    <Card.Content>
      <div class="grid grid-cols-3 gap-3 sm:grid-cols-5">
        {#each borderRadiusOptions as option}
          <button
            type="button"
            class="hover:bg-accent/50 flex flex-col items-center gap-2 rounded-lg border p-3 transition-colors {currentBorderRadius ===
            option.value
              ? 'border-primary bg-primary/10'
              : ''}"
            onclick={() => handleBorderRadiusChange(option.value)}>
            <div class="bg-primary h-10 w-10" style="border-radius: {option.cssValue};"></div>
