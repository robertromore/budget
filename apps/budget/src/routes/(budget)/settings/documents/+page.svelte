...
const hasLlmConfigured = $derived(llmPreferencesQuery.data?.enabled && enabledProviders.length > 0);
  ...
  {:else}
    ...
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description>Choose how text should be extracted from documents</Card.Description>
      </Card.Header>
      <Card.Content class="grid gap-4 sm:grid-cols-2">
        {#each Object.entries(methodMeta) as [key, meta]}
          ...
          <button
            type="button"
            class="hover:bg-accent/50 relative flex cursor-pointer flex-col gap-3 rounded-lg border p-4 text-left transition-colors {isSelected
      ...
      <Card.Content class="space-y-6">
        ...
        <div class="flex items-center justify-between">
          <div class="flex-1">
            ...
            {#if !hasLlmConfigured}
              <p class="text-warning mt-1 text-xs">Requires an LLM provider with vision support</p>
            {/if}
          </div>
          <Switch
    ...
    {#if method === 'tesseract' || method === 'auto' || fallbackToOcr}
      <Card.Root>
        <Card.Header>
          ...
          <Card.Description>Configure Tesseract OCR language for text recognition</Card.Description>
        </Card.Header>
        <Card.Content>
          <div class="flex items-center gap-4">
