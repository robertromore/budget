...
import type { LLMFeatureConfig, LLMProvider } from '$core/schema/workspaces';
...
let providerConfigs = $state<
  Record<LLMProvider, { enabled: boolean; model: string; endpoint?: string; apiKey?: string }>
>({
...
const ollamaModelsQuery = $derived(
  LLMSettings.getOllamaModels(ollamaEndpoint).options(() => ({
  ...
  {:else}
    ...
    <Card.Root data-help-id="llm-providers" data-help-title="LLM Providers">
      <Card.Header>
        ...
        <Card.Description
          >Configure your AI providers. API keys are stored encrypted.</Card.Description>
      </Card.Header>
      <Card.Content class="grid gap-4 sm:grid-cols-2">
        {#each ['openai', 'anthropic', 'google', 'ollama'] as provider}
          ...
          {@const models =
            provider === 'ollama' ? ollamaModels : LLM_MODELS[provider as LLMProvider]}
          <div class="relative">
            <LLMProviderCard
              ...
              <div class="mt-2 flex items-center justify-between text-xs">
                ...
                {:else if ollamaModelsQuery.data?.success}
                  <span class="text-muted-foreground">
                    {ollamaModelsQuery.data.models.length} model{ollamaModelsQuery.data.models
                      .length === 1
                      ? ''
                      : 's'} found
                  </span>
                ...
                <Button
                  ...
                  <RefreshCw
                    class="mr-1 h-3 w-3 {ollamaModelsQuery.isFetching ? 'animate-spin' : ''}" />
                  Refresh
                </Button>
              </div>
            {/if}
