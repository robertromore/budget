...
const hasChanges = $derived(
  settingsQuery.data && selectedLevel !== settingsQuery.data.encryptionLevel
);
</script>
      ...
      {:else}
        <RadioGroup.Root bind:value={selectedLevel} class="space-y-2">
          {#each levelOptions as option}
            ...
            <label
              class="hover:bg-muted/50 flex cursor-pointer items-start gap-3 rounded-md border p-3 transition-colors {selectedLevel ===
              option.value
  ...
  {#if effectiveLevelQuery.data && effectiveLevelQuery.data.level > 0}
    <Card.Root>
      <Card.Header>
        ...
        <Card.Description>Features affected by the current encryption level.</Card.Description>
      </Card.Header>
      <Card.Content>
