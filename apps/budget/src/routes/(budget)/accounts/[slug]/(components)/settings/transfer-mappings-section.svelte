...
const filteredOutgoing = $derived.by(() => {
  ...
  const query = searchQuery.toLowerCase().trim();
  return outgoingMappings.filter(
    (m) =>
...
<div class="space-y-6">
  ...
  {:else if totalMappings === 0}
    <Empty.Root>
      ...
      <Empty.Description>
        No payee strings are configured to transfer to or from this account. Create mappings by
        converting transactions to transfers during import.
      </Empty.Description>
      <Empty.Content>
        ...
        {#if outgoingMappings.length === 0}
          <Card.Root>
            <Card.Content class="py-8 text-center">
              <p class="text-muted-foreground text-sm">
                No outgoing transfer mappings. When you import transactions and mark them as
                transfers, those payee mappings will appear here.
              </p>
            </Card.Content>
          </Card.Root>
        {:else}
          <div class="space-y-4">
            <div class="flex items-center justify-between">
              <p class="text-muted-foreground text-sm">
                {outgoingMappings.length} payee string{outgoingMappings.length !== 1 ? 's' : ''} from
                imports into {account.name}
              </p>
              <Button
                variant="destructive"
                size="sm"
                onclick={() => openDeleteAllDialog('outgoing')}>
      ...
      <Tabs.Content value="incoming">
        {#if incomingMappings.length === 0}
          <Card.Root>
            <Card.Content class="py-8 text-center">
              <p class="text-muted-foreground text-sm">
                No incoming transfer mappings. When someone imports transactions from another
                account and marks them as transfers to this account, those mappings will appear
                here.
              </p>
            </Card.Content>
          </Card.Root>
        {:else}
          <div class="space-y-4">
            <div class="flex items-center justify-between">
              <p class="text-muted-foreground text-sm">
                {incomingMappings.length} payee string{incomingMappings.length !== 1 ? 's' : ''} will
                transfer to {account.name}
              </p>
              <Button
                variant="destructive"
                size="sm"
                onclick={() => openDeleteAllDialog('incoming')}>
                <Trash2 class="mr-2 h-4 w-4" />
                Clear All
              </Button>
            </div>
...
<AlertDialog.Root bind:open={editDialog.open}>
  <AlertDialog.Content>
    <AlertDialog.Header>
      ...
      <AlertDialog.Description>Update the payee string for this mapping.</AlertDialog.Description>
    </AlertDialog.Header>
  ...
  <AlertDialog.Content>
    <AlertDialog.Header>
      ...
      <AlertDialog.Description>
        Are you sure you want to delete this mapping? This payee string will no longer automatically
        create transfers during import.
      </AlertDialog.Description>
    </AlertDialog.Header>
...
<AlertDialog.Root bind:open={deleteAllDialog.open}>
  <AlertDialog.Content>
    <AlertDialog.Header>
      <AlertDialog.Title
        >Delete All {deleteAllDialog.type === 'incoming' ? 'Incoming' : 'Outgoing'} Mappings</AlertDialog.Title>
      <AlertDialog.Description>
        Are you sure you want to delete all {deleteAllDialog.type === 'incoming'
          ? incomingMappings.length
          : outgoingMappings.length}
        {deleteAllDialog.type} transfer mapping{(deleteAllDialog.type === 'incoming'
          ? incomingMappings.length
          : outgoingMappings.length) !== 1
          ? 's'
