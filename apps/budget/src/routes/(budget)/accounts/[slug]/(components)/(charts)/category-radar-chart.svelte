...
<AnalyticsChartShell
  ...
  emptyMessage={categoryData.length < 3
            ...
            <Html pointerEvents={false}>
              {#if hoveredPoint}
                <div
                  ...
                  style="left: {hoveredPoint.x}px; top: {hoveredPoint.y -
                    50}px; transform: translateX(-50%);">
                  <div class="bg-popover rounded-md border px-3 py-2 text-sm shadow-md">
                    <p class="font-medium">{hoveredPoint.key}</p>
                    <p class="text-muted-foreground">
                      {currencyFormatter.format(hoveredPoint.value)}
                    </p>
                  </div>
                </div>
              {/if}
