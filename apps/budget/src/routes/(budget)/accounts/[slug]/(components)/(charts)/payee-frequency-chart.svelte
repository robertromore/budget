...
const comprehensiveStats = $derived.by((): ComprehensiveStats | null => {
  ...
  const frequencyDist = {
    ...
    occasional: payeeData.filter((p) => ['Occasional', 'One-time'].includes(p.frequencyLabel))
      .length,
  };
  ...
  return {
    ...
    distribution: {
      highest: {
        ...
        month: mostFrequent.payee,
        monthLabel: `${mostFrequent.count} visits`,
      },
      lowest: {
        value: leastFrequent.count,
        month: leastFrequent.payee,
        monthLabel: `${leastFrequent.count} visits`,
      },
  ...
  {#snippet headerActions()}
    <div class="flex gap-1">
      <Button
        ...
        size="sm"
        onclick={() => (sortMode = 'frequency')}>
        Visits
      </Button>
      <Button
        variant={sortMode === 'total' ? 'default' : 'ghost'}
        size="sm"
        onclick={() => (sortMode = 'total')}>
        Total
      </Button>
      <Button
        variant={sortMode === 'average' ? 'default' : 'ghost'}
        size="sm"
        onclick={() => (sortMode = 'average')}>
        Average
      </Button>
    </div>
  {/snippet}
            ...
            <div class="pointer-events-none absolute top-4 left-1/2 z-10 -translate-x-1/2">
              <div class="bg-popover rounded-md border px-3 py-2 text-sm shadow-md">
                ...
                <p class="text-muted-foreground">
                  {hoveredItem.count} visit{hoveredItem.count !== 1 ? 's' : ''}
                </p>
                <p class="text-muted-foreground">
                  Total: {currencyFormatter.format(hoveredItem.total)}
                </p>
                <p class="text-muted-foreground">
                  Avg: {currencyFormatter.format(hoveredItem.average)}
                </p>
                <p class="text-muted-foreground text-xs">{hoveredItem.frequencyLabel}</p>
              </div>
            </div>
          {/if}
        </Html>
