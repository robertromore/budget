...
const nameState = createEncryptedFieldState(() => account.name, { operation: 'View account name' });

const notesState = createEncryptedFieldState(() => account.notes, {
      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div class="space-y-2">
          ...
          {#if nameState.isEncrypted && nameState.needsUnlock}
            <div
              class="bg-muted text-muted-foreground flex h-10 items-center gap-2 rounded-md border px-3">
              <Lock class="h-4 w-4" />
          ...
          {:else}
            <Input id="account-name" bind:value={name} placeholder="e.g., Chase Checking" />
          {/if}
        </div>
        ...
        <div class="space-y-2">
          ...
          <Input id="institution" bind:value={institution} placeholder="e.g., Chase Bank" />
        </div>
  ...
  <Card.Root>
    ...
    <Card.Content>
      {#if notesState.isEncrypted && notesState.needsUnlock}
        <div
          class="bg-muted text-muted-foreground flex min-h-[100px] items-center gap-2 rounded-md border px-3 py-2">
          <Lock class="h-4 w-4" />
      ...
      {:else}
        <Textarea bind:value={notes} placeholder="Optional notes about this account..." rows={4} />
      {/if}
    </Card.Content>
  </Card.Root>
