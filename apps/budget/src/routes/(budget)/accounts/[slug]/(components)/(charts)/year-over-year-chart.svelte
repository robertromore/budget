...
let hoveredItem = $state<{ item: (typeof chartData)[0]; keyIndex: number; key: string } | null>(
  null
);
let hoveredStackedItem = $state<{
  item: (typeof chartData)[0];
  key: string;
  segmentIndex: number;
  seriesIndex: number;
} | null>(null);
...
const chartData = $derived.by(() => {
  ...
  for (let i = 0; i < months.length; i++) {
    ...
    const monthData: { month: string; index: number; [key: string]: string | number } = {
      month,
      index: i,
    };
...
const chartColors = $derived.by(() => {
  if (viewMode === 'both') {
    ...
    return availableYears.flatMap((_, i) => [
...
const summaryStats = $derived.by(() => {
  ...
  const bestYearLabel =
    viewMode === 'expenses' ? 'Best Year' : viewMode === 'income' ? 'Top Year' : 'Most Active';
  ...
  return [
    { label: 'Years Compared', value: availableYears.length.toString() },
    {
      label: 'Highest Month',
      value: highestMonth,
      description: currencyFormatter.format(highestAmount),
    },
    {
      label: 'YoY Change',
...
const comprehensiveStats = $derived.by((): ComprehensiveStats | null => {
  ...
  const yearTotals: { year: number; total: number; monthlyValues: number[] }[] = availableYears.map(
    (year) => {
  ...
  return {
    ...
    distribution: {
      ...
      lowest: {
        value: lowestValue === Infinity ? 0 : lowestValue,
        month: lowestMonth,
        monthLabel: lowestMonth,
      },
      ...
      coefficientOfVariation:
        basicStats.mean !== 0 ? (basicStats.stdDev / basicStats.mean) * 100 : 0,
    },
  ...
  {#snippet headerActions()}
    <div class="flex items-center gap-2">
      ...
      <div class="flex gap-1 border-l pl-2">
        <Button
          variant={chartStyle === 'grouped' ? 'default' : 'ghost'}
          size="sm"
          onclick={() => (chartStyle = 'grouped')}>
          Grouped
        </Button>
        <Button
          variant={chartStyle === 'stacked' ? 'default' : 'ghost'}
          size="sm"
          onclick={() => (chartStyle = 'stacked')}>
          Stacked
        </Button>
      </div>
    </div>
        ...
        {#key `${chartStyle}-${viewMode}`}
          {#if chartStyle === 'stacked'}
            ...
            <LayerCake
              ...
              y={(d: Record<string, unknown>) =>
                chartKeys.reduce((sum, key) => sum + ((d[key] as number) || 0), 0)}
              yDomain={[0, yMax]}
              ...
              <Html pointerEvents={false}>
                {#if hoveredStackedItem}
                  ...
                  {@const total = chartKeys.reduce(
                    (sum, key) => sum + ((point[key] as number) || 0),
                    0
                  )}
                      ...
                      {:else}
                        {#each availableYears as year, yi}
                          <p style="color: {yearColors[yi % yearColors.length]}">
                            {year}: {currencyFormatter.format(
                              (point[year.toString()] as number) || 0
                            )}
                          </p>
                        {/each}
                      {/if}
          ...
          {:else}
            ...
            <LayerCake
              ...
              x="index"
              y={(d: Record<string, unknown>) =>
                Math.max(...chartKeys.map((key) => (d[key] as number) || 0))}
              xScale={scaleLinear()}
              xDomain={[-0.5, 11.5]}
              yScale={scaleLinear()}
              yDomain={[0, yMax]}
