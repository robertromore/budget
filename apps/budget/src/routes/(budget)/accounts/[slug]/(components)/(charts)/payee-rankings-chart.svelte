...
const periodFilteredTransactions = $derived.by(() => {
  ...
  return transactions.filter((tx) => {
    ...
    return (
      !isNaN(date.getTime()) &&
      date >= periodRanges.current.start &&
      date <= periodRanges.current.end
    );
  });
});
...
const previousPeriodTransactions = $derived.by(() => {
  ...
  return transactions.filter((tx) => {
    ...
    return (
      !isNaN(date.getTime()) &&
      date >= periodRanges.previous.start &&
      date <= periodRanges.previous.end
    );
  });
});
...
const payeeData = $derived.by(() => {
  const payeeTotals = new Map<string, { id: number; name: string; total: number; count: number }>();
...
const comprehensiveStats = $derived.by((): ComprehensiveStats | null => {
  ...
  return {
    ...
    distribution: {
      highest: {
        value: payeeData[0].total,
        month: payeeData[0].name,
        monthLabel: payeeData[0].name,
      },
      lowest: {
        value: payeeData[n - 1].total,
        month: payeeData[n - 1].name,
        monthLabel: payeeData[n - 1].name,
      },
          ...
          <Html pointerEvents={false}>
            {#if hoveredItem}
              <div class="pointer-events-none absolute top-4 left-1/2 z-10 -translate-x-1/2">
                <div class="bg-popover min-w-55 rounded-md border px-3 py-2 text-sm shadow-md">
                  ...
                  <div class="mt-1 space-y-0.5">
                    ...
                    <p class="text-muted-foreground">
                      {hoveredItem.count} txn{hoveredItem.count !== 1 ? 's' : ''} @ {currencyFormatter.format(
                        hoveredItem.avgTransaction
                      )} avg
                    </p>
            ...
            <g>
              ...
              {#if payee.change !== null}
                <text
                  ...
                  y={yPos}
                  dominant-baseline="middle"
                  class="text-[9px] font-medium {payee.change >= 0
                    ? 'fill-destructive'
                    : 'fill-green-600'}">
