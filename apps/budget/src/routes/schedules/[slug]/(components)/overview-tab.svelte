    ...
    <div class="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
      ...
      <Card.Root>
        <Card.Header class="flex flex-row items-center justify-between space-y-0 pb-1">
          ...
          <DollarSign class="text-muted-foreground h-3 w-3" />
        </Card.Header>
      ...
      <Card.Root>
        <Card.Header class="flex flex-row items-center justify-between space-y-0 pb-1">
          ...
          <CalendarDays class="text-muted-foreground h-3 w-3" />
        </Card.Header>
        <Card.Content class="pt-1">
          ...
          <p class="text-muted-foreground text-xs">Next due date</p>
        </Card.Content>
      </Card.Root>
      ...
      <Card.Root>
        <Card.Header class="flex flex-row items-center justify-between space-y-0 pb-1">
          ...
          <TrendingUp class="text-muted-foreground h-3 w-3" />
        </Card.Header>
        <Card.Content class="pt-1">
      ...
      <Card.Root>
        ...
        <Card.Content class="space-y-2 pt-2">
          <div class="flex items-center justify-between">
            <span class="text-muted-foreground text-xs">Account</span>
            <span class="text-sm font-medium">{schedule.account?.name || 'None'}</span>
          </div>
          <Separator />
          <div class="flex items-center justify-between">
            <span class="text-muted-foreground text-xs">Payee</span>
            <span class="text-sm font-medium">{schedule.payee?.name || 'None'}</span>
          </div>
          <Separator />
          <div class="flex items-center justify-between">
            <span class="text-muted-foreground text-xs">Amount Type</span>
        ...
        <Card.Content class="space-y-2 pt-2">
          ...
          {:else}
            <div class="py-2 text-center">
              <Calendar class="text-muted-foreground mx-auto mb-1 h-6 w-6" />
              <p class="text-muted-foreground text-xs">One-time schedule</p>
            </div>
          {/if}
        </Card.Content>
      </Card.Root>
    ...
    {#if futureProjections.length > 0}
      <Card.Root>
        <Card.Header class="pb-2">
          ...
          <Card.Description class="text-xs">Next few scheduled occurrences</Card.Description>
        </Card.Header>
        <Card.Content class="pt-2">
          <div class="space-y-2">
            {#each futureProjections.slice(0, 4) as projection, index}
              <div class="bg-muted/30 flex items-center justify-between rounded p-2">
                <div class="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div class="text-muted-foreground text-xs">
                      {Math.ceil((projection.date.getTime() - Date.now()) / (1000 * 60 * 60 * 24))} days
                      from now
                    </div>
                  </div>
                </div>
