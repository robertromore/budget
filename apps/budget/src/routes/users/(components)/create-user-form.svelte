...
import {formInsertUserSchema, type FormInsertUserSchema} from '$lib/schema/users';
...
interface Props {
	data: SuperValidated<FormInsertUserSchema>;
	onSuccess?: () => void;
}
...
const form = superForm(data, {
	validators: zod4Client(formInsertUserSchema),
	dataType: 'json',
	resetForm: true,
	invalidateAll: 'force',
	onUpdate({form}) {
	...
	<Card.Content>
		<form method="POST" action="?/create" use:enhance class="space-y-4">
			...
			<Form.Field {form} name="preferences">
				<input type="hidden" name="preferences" value={JSON.stringify({
					locale: 'en-US',
					currency: 'USD',
