...
{:else if useClientSideTable}
  ...
  {:else}
    ...
    <div class="mb-4 bg-red-100 p-4 text-red-800">
      DEBUG: browser={browser}, categoriesState={!!categoriesState}, payeesState={!!payeesState},
      isLoading={isLoading}, transactions.length={transactions.length}
    </div>
    <div class="space-y-4">
      <!-- Advanced Filter Toolbar -->
      {#if serverAccountState}
        <ServerDataTableToolbar accountState={serverAccountState} {accountId} />
      {:else}
          ...
          <Table.Root>
            ...
            <Table.Body>
              {#each simpleFormatted as transaction}
                <Table.Row>
                  <Table.Cell
                    >{new Date(transaction.date.toString()).toLocaleDateString()}</Table.Cell>
...
{:else}
  ...
  <div class="space-y-4">
          ...
          <Table.Body>
            ...
            {:else}
              <Table.Row>
                <Table.Cell colspan={6} class="text-center text-muted-foreground py-8">
                  {filters.searchQuery
                    ? `No transactions found matching "${filters.searchQuery}"`
                    : 'No transactions found'}
                </Table.Cell>
              </Table.Row>
            {/each}
    ...
    {#if serverAccountState && serverAccountState.pagination.totalPages > 1}
      <ServerDataTablePagination accountState={serverAccountState} {accountId} />
    {:else if pagination.totalPages > 1}
      <!-- Fallback basic pagination -->
      <div class="flex items-center justify-between">
        <div class="text-muted-foreground text-sm">
          Showing {pagination.page * pagination.pageSize + 1} to {Math.min(
