  ...
  return currentFile.validatedRows.map((row) => {
    ...
    return {
      ...
      normalizedData: {
        ...row.normalizedData,
        amount:
          importOptions.reverseAmountSigns && row.normalizedData['amount']
...
function handleCategoryUpdate(
  ...
  if (isClearingCategory) {
    ...
    if (row) {
      ...
      const originalPayee =
        (row.normalizedData['originalPayee'] as string) ||
      ...
      if (dismissCategoryId && dismissCategoryName) {
        ...
        const payee = payeeState?.payees
          ? Array.from(payeeState.payees.values()).find(
...
function applyBulkCategoryUpdate(
  rowIndex: number,
  categoryId: number | null,
  categoryName: string | null
) {
...
async function applySmartCategorizationToFile(
  fileId: string,
  rows: ImportRow[]
): Promise<ImportRow[]> {
  const workspaceId = workspaceState?.workspaceId;
 ...
 */
function handleMultiFilePayeeUpdate(
  rowIndex: number,
  payeeId: number | null,
  payeeName: string | null
) {
...
function handleMultiFileCategoryUpdate(
  rowIndex: number,
  categoryId: number | null,
  categoryName: string | null
) {
  ...
  try {
    ...
    const payeeInputs = currentFile.validatedRows
      ...
      .map((row) => {
        ...
        const originalPayee =
          (row as any).originalPayee || (data['originalPayee'] as string | undefined);
        return {
    ...
    for (const suggestion of result.categorySuggestions) {
      if (suggestion.suggestions.length > 0) {
        ...
        if (topSuggestion.confidence >= 0.7) {
          ...
          const isTransfer =
            row?.normalizedData['suggestedTransferAccountId'] ||
...
async function persistCleanupChoices(result: ImportResult) {
  ...
  const categoryAliasRecords: Array<{
    rawString: string;
    categoryId: number;
    sourceAccountId?: number;
  }> = [];
  ...
  for (let i = 0; i < allRows.length; i++) {
    ...
    const categoryId =
      override?.categoryId || (row.normalizedData['inferredCategoryId'] as number | undefined);
  ...
  try {
    ...
    if (categoryAliasRecords.length > 0) {
      promises.push(
        trpc().categoryAliasRoutes.bulkCreate.mutate({ aliases: categoryAliasRecords })
      );
    }
      ...
      {:else if multiFileState.globalStep === 'processing'}
              ...
              onNext={(state) => {
                ...
                for (const group of state.payeeGroups) {
                  ...
                  if (group.transferAccountId && group.transferAccountName) {
                    ...
                    const payeeId =
                      group.userDecision === 'accept' && group.existingMatch?.id
                      ...
                      if (override.transferAccountId) {
                        ...
                        return {
                          ...
                          normalizedData: {
                            ...
                            originalPayee:
                              row.normalizedData['originalPayee'] ?? row.normalizedData['payee'],
                          },
                        };
                      } else {
                        // Regular payee row
            ...
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-muted-foreground text-sm">
                  {multiFilePreviewData.length} transactions - edit payees, categories, and descriptions
                  as needed
                </p>
              </div>
            </div>
        ...
        {:else if isProcessing}
          <div class="mt-4 py-8 text-center">
            ...
            <p
              class="text-muted-foreground mx-auto mt-4 max-w-xs truncate text-sm"
              title={multiFileState.currentFile?.fileName}>
            ...
            <Card.Root class="w-80 max-w-[90vw]">
              <Card.Content class="pt-6">
                <div class="text-center">
                  ...
                  <p
                    class="text-muted-foreground mt-2 truncate px-2 text-sm"
                    title={multiFileState.currentFile?.fileName}>
            ...
            {#if importResult.entitiesCreated}
              <div>
                <div class="text-2xl font-bold">
                  {(importResult.entitiesCreated.payees ?? 0) +
                    (importResult.entitiesCreated.categories ?? 0)}
                </div>
          ...
          {#if importResult.reconciledTransactions && importResult.reconciledTransactions.length > 0}
            <div class="border-t pt-4">
              ...
              <p class="text-muted-foreground mb-2 text-xs">
                These imported transactions matched existing transfers and were merged instead of
                creating duplicates.
              </p>
