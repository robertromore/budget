...
<div class={cn('space-y-6', className)}>
        ...
        <Collapsible.Root open={isExpanded} onOpenChange={() => toggleFile(file.id)}>
          <Collapsible.Trigger>
            {#snippet child({ props })}
              <button
                {...props}
                class="hover:bg-muted/30 flex w-full items-center justify-between p-4 text-left">
            ...
            <div class="border-t px-4 py-3">
              ...
              <div class="bg-muted/30 max-h-64 overflow-auto rounded-lg border">
                ...
                {#if (file.validatedRows?.length ?? 0) > 10}
                  <div
                    class="text-muted-foreground bg-muted/30 border-t px-3 py-2 text-center text-xs">
  ...
  <div class="flex items-center justify-between pt-4">
    <Button variant="outline" onclick={onBack} disabled={isImporting}>Back to Files</Button>
