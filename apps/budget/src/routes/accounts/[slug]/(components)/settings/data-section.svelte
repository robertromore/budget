...
async function confirmDeleteByDateRange() {
  ...
  try {
    ...
    const startDateStr = deleteDateRangeDialog.startDate
      .toDate(timezone)
      .toISOString()
      .split('T')[0];
...
<div class="space-y-6">
  ...
  <Card.Root>
    <Card.Header>
      ...
      <Card.Description>Remove all scheduled transactions linked to this account.</Card.Description>
    </Card.Header>
    <Card.Content>
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p class="text-muted-foreground text-sm">
  ...
  <Card.Root>
    <Card.Header>
      ...
      <Card.Description>Remove import profiles that are linked to this account.</Card.Description>
    </Card.Header>
    <Card.Content>
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p class="text-muted-foreground text-sm">
          {dataCounts?.importProfiles || 0} import profile{(dataCounts?.importProfiles || 0) !== 1
            ? 's'
  ...
  <Card.Root>
    ...
    <Card.Content>
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p class="text-muted-foreground text-sm">
          {dataCounts?.detectedPatterns || 0} detected pattern{(dataCounts?.detectedPatterns ||
            0) !== 1
            ? 's'
            : ''} will be cleared. New patterns will be detected automatically.
        </p>
        <Button
          variant="destructive"
    ...
    {#if hasTransfers}
      <div class="bg-muted/50 flex items-start gap-3 rounded-lg border p-4">
        ...
        <div class="flex-1 space-y-3">
          <p class="text-sm">
            <strong>{dataCounts?.transfers || 0}</strong> of these transactions are transfers linked to
