  ...
  {:else}
                ...
                {#if balanceInfo?.reconciledDate}
                  <li>
                    Reconciled on {formatDateString(balanceInfo.reconciledDate)} at {formatBalance(
                      balanceInfo.reconciledBalance
                    )}
                  </li>
                {/if}
                {#if balanceInfo?.balanceResetDate}
                  <li>
                    Balance reset from {formatDateString(balanceInfo.balanceResetDate)} ({balanceInfo.transactionsBeforeResetDate}
                    transactions excluded)
                  </li>
                {/if}
                {#if (balanceInfo?.archivedTransactionCount ?? 0) > 0}
                  <li>
                    {balanceInfo?.archivedTransactionCount} archived transactions excluded from balance
                  </li>
                {/if}
              </ul>
            </div>
    ...
    <Card.Root>
      ...
      <Card.Content class="space-y-4">
        <p class="text-muted-foreground text-sm">
          Set a date and balance to ignore all transactions before that date. Useful when you have a
          known statement balance but incomplete transaction history before that date.
        </p>
        ...
        {#if balanceInfo?.balanceResetDate}
          <div class="bg-muted rounded-md p-3 text-sm">
            <p>
              Currently set: Transactions before <strong
                >{formatDateString(balanceInfo.balanceResetDate)}</strong>
              are excluded. Starting balance:
              <strong>{formatBalance(balanceInfo.balanceAtResetDate)}</strong>
            </p>
        ...
        <div class="flex justify-end gap-2">
          {#if balanceInfo?.balanceResetDate}
            <Button variant="outline" onclick={handleClearResetDate} disabled={isSavingResetDate}>
    ...
    <Card.Root>
      ...
      <Card.Content class="space-y-4">
        <p class="text-muted-foreground text-sm">
          Archive old transactions to keep them for reference while excluding them from balance
          calculations. Archived transactions appear grayed out in the transaction list.
        </p>
    ...
    <Card.Root>
      <Card.Header>
        <div class="flex items-center gap-3">
          <FileCheck class="text-muted-foreground h-5 w-5" />
          <div>
      ...
      <Card.Content class="space-y-4">
        <p class="text-muted-foreground text-sm">
          Set a reconciliation checkpoint using your bank or credit card statement. The balance
          builds from this checkpoint, ensuring accuracy going forward.
        </p>
        ...
        {#if balanceInfo?.reconciledDate}
          <div class="bg-muted rounded-md p-3 text-sm">
            <p>
              Last reconciled: <strong>{formatDateString(balanceInfo.reconciledDate)}</strong>
              at{' '}
        ...
        <div class="flex justify-end gap-2">
          {#if balanceInfo?.reconciledDate}
            <Button variant="outline" onclick={handleClearReconcile} disabled={isSavingReconcile}>
              <Trash2 class="mr-2 h-4 w-4" />
    ...
    <Card.Root>
      <Card.Header>
        <div class="flex items-center gap-3">
          <Scale class="text-muted-foreground h-5 w-5" />
          <div>
      ...
      <Card.Content class="space-y-4">
        ...
        {#if adjustmentAmount !== null}
          <div
            class="rounded-md border border-amber-200 bg-amber-50 p-3 text-sm dark:border-amber-900 dark:bg-amber-950/30">
  ...
  <AlertDialog.Content>
    <AlertDialog.Header>
      ...
      <AlertDialog.Description>
        ...
        Archived transactions will be excluded from balance calculations but remain visible in the
        transaction list.
      </AlertDialog.Description>
    </AlertDialog.Header>
    <AlertDialog.Footer>
