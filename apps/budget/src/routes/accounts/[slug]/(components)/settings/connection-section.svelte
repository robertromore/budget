...
async function handleDisconnect() {
  if (
    !confirm(
      'Are you sure you want to disconnect this bank account? You will need to reconnect to sync transactions.'
    )
  ) {
    return;
  }
      ...
      <CardContent class="space-y-4">
        ...
        {#if connection.syncStatus === 'error' && connection.syncError}
          <div class="border-destructive/50 bg-destructive/10 rounded-lg border p-3">
            <p class="text-destructive text-sm">{connection.syncError}</p>
          </div>
        {/if}
      ...
      <CardContent class="py-8">
        <div class="flex flex-col items-center justify-center gap-4 text-center">
          <div class="bg-muted flex h-12 w-12 items-center justify-center rounded-full">
            <Link class="text-muted-foreground h-6 w-6" />
          </div>
          <div>
...
<!-- Teller Setup Wizard -->
<TellerSetupWizard bind:open={showTellerWizard} {account} onSuccess={handleConnectionSuccess} />
