...
// Sync pinning changes with the active view
function handlePin(direction: 'left' | 'right' | false) {
  column.pin(direction);

  // Update the view - sync the pinning state from the table
  if (_currentViews?.activeView) {
    // Use setTimeout to ensure the table state is updated before we read it
    setTimeout(() => {
...
function moveLeft() {
  ...
  // Update the view synchronously BEFORE the $effect runs
  if (_currentViews?.activeView) {
    _currentViews.activeView?.syncColumnOrderFromTable();
  }
}
