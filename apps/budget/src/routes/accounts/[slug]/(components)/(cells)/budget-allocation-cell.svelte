<script lang="ts">
  ...
  const remainingAmount = $derived.by(() =>
    Number(transaction.amount) - totalAllocated
  );

  const isFullyAllocated = $derived.by(() =>
    Math.abs(remainingAmount) < 0.01
  );

  const hasUnallocationAmount = $derived.by(() =>
    Math.abs(remainingAmount) > 0.01
  );
  ...
  {#if addingAllocation}
    <div class="flex flex-col gap-2 p-2 border rounded-md bg-background">
      ...
      <input
        ...
        step="0.01"
        placeholder="Amount"
        class="h-8 px-2 text-xs border rounded"
      />

      <div class="flex gap-1">
        <Button size="sm" onclick={handleAddAllocation} disabled={!selectedBudgetId || !allocationAmount}>
          Add
        </Button>
        <Button size="sm" variant="outline" onclick={cancelAddingAllocation}>
          Cancel
        </Button>
      </div>
    </div>
  {:else}
    ...
    {:else}
      <Button
        ...
        class="h-6 px-2 text-xs justify-start"
      >
