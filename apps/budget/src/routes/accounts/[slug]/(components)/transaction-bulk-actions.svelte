...
const pageRowCount = $derived(table.getRowModel().rows.filter(row => typeof row.original.id === 'number').length);
  ...
  <div class="flex items-center gap-2 rounded-md border bg-muted px-4 py-2">
    <div class="flex items-center gap-2">
      <span class="text-sm font-medium">
        {displayCount} {displayCount === 1 ? 'transaction' : 'transactions'} selected
      </span>
