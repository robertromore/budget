...
const distributionStats = $derived.by(() => {
  ...
  // Skewness indicator
  const skewness =
    avgValue > medValue ? 'Right-skewed' : avgValue < medValue ? 'Left-skewed' : 'Symmetric';
  ...
  const percentile90 = quantile(sortedAmounts, 0.9);
...
const summaryStats = $derived.by(() => {
  ...
  return [
    ...
    },
    {
      ...
      description:
        distributionStats.skewness === 'Right-skewed'
          ? 'Many small, few large'
          : distributionStats.skewness === 'Left-skewed'
  ...
  {#snippet headerActions()}
    <div class="flex gap-2">
      ...
      <div class="flex gap-1">
        <Button
          variant={chartStyle === 'histogram' ? 'default' : 'ghost'}
          size="sm"
      ...
      {#if chartStyle === 'histogram'}
        <div class="flex gap-1 border-l pl-2">
          ...
          <Button
            variant={viewMode === 'income' ? 'secondary' : 'ghost'}
            size="sm"
        ...
        {#if chartStyle === 'histogram'}
          ...
          <LayerCake
            ...
            <Svg>
              ...
              <VerticalLine
                value={distributionStats.mean}
                stroke="var(--chart-4)"
                strokeWidth={2}
                strokeDasharray="4,4" />
              <VerticalLine
                value={distributionStats.median}
                stroke="var(--chart-3)"
                strokeWidth={2}
                strokeDasharray="4,4" />
            </Svg>
            <Html pointerEvents={false}>
              {#if hoveredBin}
                <div
                  class="pointer-events-none absolute top-4 left-1/2 z-10 -translate-x-1/2 transform">
