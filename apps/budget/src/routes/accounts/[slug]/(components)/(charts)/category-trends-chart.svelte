...
const comparisonPeriodLabels: Record<
  ComparisonPeriod,
    ...
    case 'this-vs-last-quarter': {
      ...
      currentEnd = new Date(
        Date.UTC(
          todayYear,
          quarterEndMonth,
          getLastDayOfMonthUTC(todayYear, quarterEndMonth),
          23,
          59,
          59,
          999
      ...
      previousEnd = new Date(
        Date.UTC(
          prevYear,
          prevQuarterEndMonth,
          getLastDayOfMonthUTC(prevYear, prevQuarterEndMonth),
          23,
          59,
          59,
          999
    ...
    case 'this-vs-same-month-last-year': {
      ...
      currentEnd = new Date(
        Date.UTC(
          todayYear,
          todayMonth,
          getLastDayOfMonthUTC(todayYear, todayMonth),
          23,
          59,
          59,
          999
      ...
      previousEnd = new Date(
        Date.UTC(
          todayYear - 1,
          todayMonth,
          getLastDayOfMonthUTC(todayYear - 1, todayMonth),
          23,
          59,
          59,
          999
    ...
    default: {
      ...
      previousEnd = new Date(
        Date.UTC(
          prevMonthYear,
          adjustedPrevMonth,
          getLastDayOfMonthUTC(prevMonthYear, adjustedPrevMonth),
          23,
          59,
          59,
          999
...
const summaryStats = $derived.by(() => {
  ...
  const biggestIncrease =
    increases.length > 0 ? increases.reduce((a, b) => (a.change > b.change ? a : b)) : null;
  const biggestDecrease =
    decreases.length > 0 ? decreases.reduce((a, b) => (a.change < b.change ? a : b)) : null;
  ...
  return [
    ...
    },
    {
      ...
      description:
        netChange > 0 ? 'Spending increased' : netChange < 0 ? 'Spending decreased' : 'No change',
    },
  ];
});
...
const comprehensiveStats = $derived.by((): ComprehensiveStats | null => {
  ...
  const lowestCurrent = rawCategoryComparison
    .filter((c) => c.current > 0)
    .reduce((a, b) => (a.current < b.current ? a : b), rawCategoryComparison[0]);
  ...
  const trendDirection: 'up' | 'down' | 'flat' =
    overallChange > 5 ? 'up' : overallChange < -5 ? 'down' : 'flat';
  ...
  return {
    ...
    distribution: {
      highest: {
        value: highestCurrent.current,
        month: highestCurrent.category,
        monthLabel: highestCurrent.category,
      },
      lowest: {
        value: lowestCurrent?.current || 0,
        month: lowestCurrent?.category || '',
        monthLabel: lowestCurrent?.category || '',
      },
    ...
    comparison: {
      ...
      vsHistoricalAvgPercent:
        avgPrevious > 0 ? ((avgCurrent - avgPrevious) / avgPrevious) * 100 : null,
      vsBudgetTarget: null,
      ...
      <DropdownMenu.Root>
        ...
        <DropdownMenu.Content align="end">
          <DropdownMenu.RadioGroup bind:value={comparisonPeriod}>
            <DropdownMenu.RadioItem value="this-vs-last-month"
              >Month vs Month</DropdownMenu.RadioItem>
            <DropdownMenu.RadioItem value="this-vs-last-week">Week vs Week</DropdownMenu.RadioItem>
            <DropdownMenu.RadioItem value="this-vs-last-quarter"
              >Quarter vs Quarter</DropdownMenu.RadioItem>
            <DropdownMenu.RadioItem value="this-vs-same-month-last-year"
              >Year over Year</DropdownMenu.RadioItem>
      ...
      <DropdownMenu.Root>
        ...
        <DropdownMenu.Content align="end">
          <DropdownMenu.RadioGroup bind:value={sortOption}>
            <DropdownMenu.RadioItem value="absolute-change"
              >Biggest Change ($)</DropdownMenu.RadioItem>
            <DropdownMenu.RadioItem value="percent-change"
              >Biggest Change (%)</DropdownMenu.RadioItem>
        ...
        <DropdownMenu.Content align="end">
          <DropdownMenu.RadioGroup bind:value={changeFilter}>
            ...
            <DropdownMenu.RadioItem value="increased">
              <TrendingUp class="text-destructive mr-2 h-4 w-4" />
            ...
            <div class="pointer-events-none absolute top-4 left-1/2 z-10 -translate-x-1/2">
              <div class="bg-popover rounded-md border px-3 py-2 text-sm shadow-md">
                ...
                <p class="text-muted-foreground">
                  {comparisonPeriodLabels[comparisonPeriod].previous}: {currencyFormatter.format(
                ...
                <div class="mt-1 flex items-center gap-2 border-t pt-1">
                  ...
                  <p
                    ...
                    0
                      ? '+'
                      : ''}{currencyFormatter.format(hoveredItem.changeAbs)})
                  </p>
                </div>
