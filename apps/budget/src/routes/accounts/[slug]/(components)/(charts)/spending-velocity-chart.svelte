...
const velocityAsSpending = $derived(
  velocityData.map((d) => ({
    month: d.dateStr,
...
const summaryStats = $derived.by(() => {
  ...
  const previousAvg =
    previous.length > 0
      ? previous.reduce((sum, d) => sum + d.rolling, 0) / previous.length
      : recentAvg;
...
const forecastData = $derived.by(() => {
  ...
  for (let i = 1; i <= forecastDays; i++) {
    ...
    const predictedValue =
      trendForForecast.intercept + trendForForecast.slope * (lastPoint.index + i);
...
<AnalyticsChartShell
  ...
  {#snippet headerActions()}
    <div class="flex gap-1">
      <Button
        variant={windowSize === 7 ? 'default' : 'ghost'}
        size="sm"
        onclick={() => (windowSize = 7)}>
        7 Day
      </Button>
      <Button
        variant={windowSize === 14 ? 'default' : 'ghost'}
        size="sm"
        onclick={() => (windowSize = 14)}>
        14 Day
      </Button>
      <Button
        variant={windowSize === 30 ? 'default' : 'ghost'}
        size="sm"
      ...
      <LayerCake
        ...
        <Svg>
          ...
          {#if chartType === 'line-area'}
            ...
            <Scatter
              fill={(d) =>
                chartSelection.isSelected(d.dateStr) ? 'var(--primary)' : 'var(--chart-1)'}
              radius={(d) => (chartSelection.isSelected(d.dateStr) ? 6 : 4)}
              hoverRadius={7}
              stroke={(d) =>
                chartSelection.isSelected(d.dateStr)
          ...
          <Tooltip externalHoverX={brushHoverX}>
            {#snippet children({ point, x })}
              ...
              {@const vsAvg = historicalAvgForTooltip
                ? ((point.rolling - historicalAvgForTooltip) / historicalAvgForTooltip) * 100
                : null}
