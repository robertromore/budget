...
let hoveredBarInfo = $state<{ item: PaymentAnalysisPoint; keyIndex: number; key: string } | null>(
  null
);
...
<AnalyticsChartShell
  ...
  {#snippet subtitle()}
    {viewMode === 'payments'
      ? 'Monthly payments vs charges'
      : viewMode === 'stacked'
        ? 'Stacked payments and charges'
    ...
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Content align="end" class="w-48">
        ...
        <DropdownMenu.CheckboxItem
          checked={viewMode === 'payments'}
          onCheckedChange={() => (viewMode = 'payments')}>
          Side by Side
        </DropdownMenu.CheckboxItem>
        <DropdownMenu.CheckboxItem
          checked={viewMode === 'stacked'}
          onCheckedChange={() => (viewMode = 'stacked')}>
          Stacked
        </DropdownMenu.CheckboxItem>
        <DropdownMenu.CheckboxItem
          checked={viewMode === 'breakdown'}
          onCheckedChange={() => (viewMode = 'breakdown')}>
          Payment Breakdown
        </DropdownMenu.CheckboxItem>
          ...
          <LayerCake
            ...
            <Svg>
              ...
              <Brush
                onbrush={handleBrushSelect}
                onclick={handleBrushClick}
                ondblclick={handleBrushDblClick}
                onhover={handleBrushHover}
                fill="var(--primary)"
                opacity={0.15}
                cursor="pointer" />
