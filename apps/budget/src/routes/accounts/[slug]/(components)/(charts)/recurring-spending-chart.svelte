...
const comprehensiveStats = $derived.by((): ComprehensiveStats | null => {
  ...
  return {
    ...
    distribution: {
      highest: {
        value: totals[highestIdx],
        month: monthlyData[highestIdx]?.month || '',
        monthLabel: monthlyData[highestIdx]?.month || '',
      },
      lowest: {
        value: totals[lowestIdx],
        month: monthlyData[lowestIdx]?.month || '',
        monthLabel: monthlyData[lowestIdx]?.month || '',
      },
      ...
      {#if containerReady}
        <LayerCake
          ...
          <Svg>
            ...
            <Scatter
              fill={(d) =>
                chartSelection.isSelected(d.month) ? 'var(--primary)' : 'var(--chart-1)'}
            ...
            <Tooltip externalHoverX={brushHoverX}>
              {#snippet children({ point })}
                ...
                {@const recurringPercent =
                  point.total > 0 ? (point.recurring / point.total) * 100 : 0}
                <div class="bg-popover min-w-56 rounded-md border px-3 py-2 text-sm shadow-md">
                  <p class="font-medium">{point.monthLabel}</p>

                  <!-- Spending breakdown -->
                  <div class="mt-1 space-y-0.5">
                    <p class="text-primary">
                      <span class="bg-primary mr-1.5 inline-block h-2 w-2 rounded-sm"></span>
                      Recurring: {currencyFormatter.format(point.recurring)}
                    </p>
                    <p style="color: var(--chart-2)">
                      <span
                        class="mr-1.5 inline-block h-2 w-2 rounded-sm"
                        style="background-color: var(--chart-2)"></span>
                      One-time: {currencyFormatter.format(point.oneTime)}
                    </p>
                  </div>

                  <p class="mt-1 border-t pt-1 font-semibold">
                  ...
                  {#if prevMonth}
                    ...
                    <p class="mt-1 text-xs {change > 0 ? 'text-destructive' : 'text-amount-positive'}">
                      ...
                      {currencyFormatter.format(Math.abs(change))} ({changePercent > 0
                        ? '+'
                        : ''}{formatPercentRaw(changePercent, 0)}) vs prev month
                    </p>
                  {/if}
                  ...
                  {#if topPayees.length > 0}
                    <div class="mt-2 border-t pt-2">
                      <p class="text-muted-foreground mb-1 text-xs">Top Recurring:</p>
                      {#each topPayees as payee}
                        <p class="max-w-48 truncate text-xs">
                          {payee.name}: {currencyFormatter.format(payee.amount)}
                        </p>
                      {/each}
                    </div>
                  {/if}
