...
const outliersByCategory = $derived.by(() => {
  const grouped = new Map<
    string,
    { count: number; total: number; highCount: number; lowCount: number }
  >();
...
const comprehensiveStats = $derived.by((): ComprehensiveStats | null => {
  ...
  return {
    ...
    distribution: {
      highest: {
        value: largestOutlier,
        month: largestTx?.payee || 'Largest Outlier',
        monthLabel: largestTx?.payee || '',
      },
      lowest: {
        value: sortedAmounts[0] || 0,
        month: 'Smallest',
        monthLabel: 'Minimum transaction',
      },
  ...
  {#snippet headerActions()}
    <div class="flex flex-wrap items-center gap-2">
      ...
      <div class="flex items-center gap-2 px-2">
        ...
        <Label for="lower-outliers" class="cursor-pointer text-xs">Include Low</Label>
      </div>
    </div>
  {/snippet}
        ...
        <LayerCake {data} x="absAmount" xScale={scaleLinear()} {xDomain} padding={chartPadding}>
          ...
          <Html pointerEvents={false}>
                  ...
                  <div class="bg-popover min-w-50 rounded-md border px-3 py-2 text-sm shadow-md">
                    ...
                    <p class="font-semibold" style="color: {getPointColor(hoveredItem)}">
                      ...
                      {#if hoveredItem.isOutlier}
                        <span class="ml-1 text-xs">(Outlier)</span>
                      {/if}
                    </p>
                    ...
                    {#if hoveredItem.isOutlier}
                      <div class="text-muted-foreground mt-2 space-y-0.5 border-t pt-2 text-xs">
                        <p>{hoveredItem.deviationFromThreshold.toFixed(2)}× above threshold</p>
                        <p>
                          Top {formatPercentRaw(100 - hoveredItem.percentileRank, 1)} of transactions
                        </p>
                      </div>
                    {:else}
            ...
            {:else if chartStyle === 'histogram'}
              {#if hoveredBin}
                <div class="pointer-events-none absolute top-4 left-1/2 z-10 -translate-x-1/2">
                  <div class="bg-popover min-w-50 rounded-md border px-3 py-2 text-sm shadow-md">
                    <p class="font-medium">
                      {currencyFormatter.format(hoveredBin.x0)} - {currencyFormatter.format(
                        hoveredBin.x1
                      )}
                    </p>
                    <p class="text-primary font-semibold">{hoveredBin.count} transactions</p>
                    {#if iqrStats && hoveredBin.x0 >= iqrStats.upperBound}
                      <p class="text-destructive mt-1 text-xs">Above outlier threshold</p>
                    {/if}
                    <p class="text-muted-foreground mt-2 border-t pt-1 text-xs">
            ...
            {:else}
              ...
              {#if iqrStats}
                <div
                  class="text-muted-foreground absolute bottom-12 left-1/2 flex -translate-x-1/2 gap-4 text-xs">
    ...
    <div class="mt-4 space-y-4">
        <div class="flex flex-wrap justify-center gap-4">
          {#if chartStyle === 'beeswarm'}
            ...
            <div class="flex items-center gap-3 border-r pr-4">
              <span class="text-muted-foreground text-xs font-medium">Expenses:</span>
              <div class="flex items-center gap-1.5">
                <div
                  class="h-3 w-3 rounded-full opacity-60"
                  style="background-color: var(--destructive)">
                </div>
                <span class="text-xs">Normal</span>
              </div>
              <div class="flex items-center gap-1.5">
                <div class="h-3 w-3 rounded-full" style="background-color: var(--destructive)">
                </div>
            ...
            <div class="flex items-center gap-3">
              <span class="text-muted-foreground text-xs font-medium">Income:</span>
              <div class="flex items-center gap-1.5">
                <div
                  class="h-3 w-3 rounded-full opacity-60"
          ...
          {:else}
            ...
            <div class="flex items-center gap-4">
              <div class="flex items-center gap-1.5">
                <div
                  class="h-3 w-5 rounded border"
                  style="background-color: var(--chart-1); border-color: var(--chart-1)">
                </div>
              ...
              <div class="flex items-center gap-1.5">
                <div
                  class="h-0.5 w-4 border-t-2 border-dashed"
                  style="border-color: var(--chart-1)">
                </div>
        ...
        {#if iqrStats}
          <div class="text-muted-foreground text-center text-xs">
            Threshold: >{currencyFormatter.format(iqrStats.upperBound)} ({sensitivityConfig[
              sensitivity
            ].multiplier}×IQR)
            {#if detectLowerOutliers}
            ...
            <div class="rounded-lg border p-4">
              <h4 class="mb-2 text-sm font-medium">
                ...
                <span class="text-muted-foreground font-normal">
                  ({highOutliers.length} high{detectLowerOutliers && lowOutliers.length > 0
                  ...
                  <button
                    ...
                    <div class="ml-2 flex shrink-0 flex-col items-end">
                      ...
                      <span class="text-muted-foreground text-xs">
                        {outlier.deviationFromThreshold.toFixed(2)}× {outlier.outlierType === 'low'
                          ? 'below'
                          : 'above'}
                      </span>
                    </div>
                ...
                {#each outliersByCategory as cat}
                  ...
                  <div class="space-y-1">
                    <div class="flex items-center justify-between text-sm">
                      <span class="text-muted-foreground max-w-32 truncate">{cat.category}</span>
                      <div class="flex items-center gap-2">
                        <span class="text-muted-foreground text-xs"
                          >{currencyFormatter.format(cat.total)}</span>
                        <span class="font-medium">{cat.count}</span>
                      </div>
                    </div>
                    <div class="bg-muted h-1.5 w-full rounded-full">
                      <div
                        class="bg-destructive/70 h-full rounded-full"
