...
<AnalyticsChartShell
  ...
  emptyMessage={currentBalance <= 0
    ? 'No balance to pay off - you are debt free!'
    : 'Unable to calculate payoff scenarios'}
        ...
        <div class="flex flex-1 items-center justify-center">
          <div class="text-center">
            ...
            <p class="text-muted-foreground mt-2">
              {currentBalance <= 0
                ? 'Your balance is $0 - great job!'
                : 'Set a minimum payment in account settings to see projections'}
            </p>
          </div>
        </div>
      {:else}
                        ...
                        {#if dataPoint}
                          <div class="flex items-center justify-between gap-4">
                            ...
                            <span class="font-mono text-xs"
                              >{currencyFormatter.format(dataPoint.balance)}</span>
                          </div>
                        {/if}
                      {/each}
                    </div>
