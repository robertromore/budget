...
const stackKeys = $derived(
  [...topCategories, 'Other'].filter((k) => monthlyData.some((d) => (d[k] as number) > 0))
);
...
const comprehensiveStats = $derived.by((): ComprehensiveStats | null => {
  ...
  const categoryTotals = stackKeys
    .map((key) => ({
      category: key,
  ...
  return {
    ...
    distribution: {
      highest: {
        value: monthlyTotals[highestIdx],
        month: monthlyData[highestIdx]?.monthLabel || '',
        monthLabel: monthlyData[highestIdx]?.monthLabel || '',
      },
      lowest: {
        value: monthlyTotals[lowestIdx],
        month: monthlyData[lowestIdx]?.monthLabel || '',
        monthLabel: monthlyData[lowestIdx]?.monthLabel || '',
      },
                ...
                <div class="bg-popover rounded-md border px-3 py-2 text-sm shadow-md">
                  ...
                  <p class="text-muted-foreground mt-1 border-t pt-1 text-xs">
                    Total: {currencyFormatter.format(
                      stackKeys.reduce((sum, key) => sum + ((point[key] as number) || 0), 0)
                    )}
                  </p>
                </div>
              </div>
