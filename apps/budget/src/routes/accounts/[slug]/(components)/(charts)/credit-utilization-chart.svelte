...
<AnalyticsChartShell
  ...
  emptyMessage={creditLimit <= 0
    ? 'Credit limit not set for this account'
        ...
        <LayerCake
          ...
          <Svg>
            ...
            <Tooltip
              ...
              ondblclick={(point) => handlePointDblClick(point)}>
              {#snippet children({ point })}
                {@const status = getUtilizationStatus(point.utilization)}
                {@const histAvgDiff =
                  historicalAverage !== null ? point.utilization - historicalAverage : null}
                ...
                <div class="bg-popover min-w-50 rounded-md border px-3 py-2 text-sm shadow-md">
                  <p class="font-medium">
                  ...
                  <p class="text-muted-foreground mt-2 border-t pt-1 text-xs">
                    Drag to select range, double-click for details
                  </p>
                </div>
              {/snippet}
            </Tooltip>
