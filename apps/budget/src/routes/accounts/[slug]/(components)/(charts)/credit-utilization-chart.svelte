<script lang="ts">
	...
	import { calculateMonthlyUtilization, type MonthlyUtilizationPoint } from '$lib/utils/credit-card-analytics';
	...
	const effectivePeriod = $derived(
		timePeriodFilter.chartOverrides.get('credit-utilization') ?? timePeriodFilter.globalPeriod
	);

	// Get credit limit from account
	const creditLimit = $derived(account?.debtLimit || 0);

	// Calculate monthly utilization data from ALL transactions
	const allMonthlyData = $derived.by(() => {
		if (!creditLimit || creditLimit <= 0) return [];
		return calculateMonthlyUtilization(transactions, creditLimit);
	});
	...
	const utilizationAsSpending = $derived(
		monthlyData.map((d) => ({
			month: d.month,
			monthLabel: d.monthLabel,
			spending: d.utilization,
			date: d.date,
			index: d.index
		}))
	);

	// Linear regression line for utilization
	const linearTrendData = $derived.by((): TrendLineData | null => {
		if (!showLinearTrend || monthlyData.length < 2) return null;
		return calculateLinearTrend(utilizationAsSpending);
	});
	...
	const historicalAverage = $derived.by((): number | null => {
		...
		const data = allMonthlyData.map((d) => ({ spending: d.utilization }));
	...
	const percentileBands = $derived.by((): PercentileBandsData | null => {
		...
		const data = allMonthlyData.map((d) => ({ ...d, spending: d.utilization }));
		return calculatePercentileBands(data);
	});

	// Forecast: project utilization forward using linear regression
	const FORECAST_HORIZON = 3;
	const forecastData = $derived.by(() => {
		if (!showForecast || monthlyData.length < 6) return [];

		const trend = calculateLinearTrend(utilizationAsSpending);
		if (!trend) return [];
	...
	const hasData = $derived(monthlyData.length > 0 && creditLimit > 0);

	// Y domain for percentage (utilization)
	const yDomain = $derived.by((): [number, number] => {
		if (!hasData) return [0, 100];

		const utilizations = monthlyData.map((d) => d.utilization);
		let max = Math.max(...utilizations, 0);
		...
		// Ensure we show at least up to 100% if any data is above warning
		if (max > WARNING_THRESHOLD) {
			max = Math.max(max, LIMIT_THRESHOLD + 10);
		}
	...
	// value is charges (dollar amount) since that's what the selection panel sums for display
	...
	// Get point fill color based on utilization level and selection state
	function getPointFill(d: MonthlyUtilizationPoint): string {
		if (chartSelection.isSelected(d.month)) {
			return 'var(--primary)';
		}
		if (d.utilization <= EXCELLENT_THRESHOLD) return 'var(--chart-2)'; // Excellent - green
		if (d.utilization <= WARNING_THRESHOLD) return 'var(--chart-5)'; // Good - amber
		if (d.utilization <= LIMIT_THRESHOLD) return 'var(--chart-1)'; // Warning - orange
...
<AnalyticsChartShell
	...
	emptyMessage={creditLimit <= 0 ? 'Credit limit not set for this account' : 'No transaction data available'}
	chartId="credit-utilization"
...
>
	...
	{#snippet subtitle()}
		Percentage of credit limit used each month (lower is better for credit score)
	{/snippet}
			...
			<div class="min-h-0 flex-1">
				...
				>
					<Svg>
						...
						<!-- Threshold lines - only show if within visible y-domain -->
						{#if EXCELLENT_THRESHOLD <= yDomain[1]}
							<HorizontalLine value={EXCELLENT_THRESHOLD} stroke="var(--chart-2)" strokeWidth={1} strokeDasharray="4 2" label="30% Excellent" />
						{/if}
						{#if WARNING_THRESHOLD <= yDomain[1]}
							<HorizontalLine value={WARNING_THRESHOLD} stroke="var(--chart-5)" strokeWidth={1} strokeDasharray="4 2" label="70% Warning" />
						{/if}
						{#if LIMIT_THRESHOLD <= yDomain[1]}
							<HorizontalLine value={LIMIT_THRESHOLD} stroke="var(--destructive)" strokeWidth={1.5} strokeDasharray="4 2" label="100% Limit" />
						{/if}

						{#if chartType === 'line-area'}
							<Area fill="var(--chart-3)" opacity={0.2} />
							<Line stroke="var(--chart-3)" strokeWidth={2} />
							<Scatter
								fill={getPointFill}
						...
						{:else if chartType === 'area'}
							<Area fill="var(--chart-3)" opacity={0.3} />
							<Scatter
								fill={getPointFill}
								radius={getPointRadius}
								hoverRadius={7}
						...
						<Tooltip externalHoverX={brushHoverX} onclick={(point) => handlePointClick(point)} ondblclick={(point) => handlePointDblClick(point)}>
							{#snippet children({ point })}
								{@const status = getUtilizationStatus(point.utilization)}
								{@const histAvgDiff = historicalAverage !== null ? point.utilization - historicalAverage : null}
								{@const trendValue = showLinearTrend ? getTrendValueAtIndex(point.index) : null}
								{@const trendDiff = trendValue !== null ? point.utilization - trendValue : null}
								...
								<div class="min-w-50 rounded-md border bg-popover px-3 py-2 text-sm shadow-md">
									...
									<div class="text-muted-foreground mt-1 text-xs">
										...
										<p>Credit Limit: {currencyFormatter.format(point.creditLimit)}</p>
