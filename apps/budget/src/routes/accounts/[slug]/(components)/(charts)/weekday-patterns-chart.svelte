...
const summaryStats = $derived.by(() => {
  return [
    ...
    },
    {
      ...
      value:
        patterns.weekendVsWeekday > 0
          ? `+${formatPercentRaw(patterns.weekendVsWeekday, 0)}`
          : formatPercentRaw(patterns.weekendVsWeekday, 0),
...
const comprehensiveStats = $derived.by((): ComprehensiveStats | null => {
  ...
  return {
    ...
    distribution: {
      highest: {
        value: patterns.highest.average,
        month: patterns.highest.dayFull,
        monthLabel: patterns.highest.dayFull,
      },
      lowest: {
        value: patterns.lowest.average,
        month: patterns.lowest.dayFull,
        monthLabel: patterns.lowest.dayFull,
      },
            ...
            <div class="pointer-events-none absolute top-4 left-1/2 z-10 -translate-x-1/2">
              <div class="bg-popover rounded-md border px-3 py-2 text-sm shadow-md">
                ...
                <p class="text-muted-foreground text-xs">
                  {hoveredItem.transactions} transactions over {hoveredItem.count}
                  {hoveredItem.dayFull}s
                </p>
              </div>
            </div>
          {/if}
