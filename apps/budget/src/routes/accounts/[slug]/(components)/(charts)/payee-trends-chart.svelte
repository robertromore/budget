...
const summaryStats = $derived.by(() => {
  ...
  const trends = topPayees.map((payee) => {
    ...
    const previousAvg =
      previous.length > 0 ? previous.reduce((s, v) => s + v, 0) / previous.length : recentAvg;
...
const comprehensiveStats = $derived.by((): ComprehensiveStats | null => {
  ...
  const trendDirection: 'up' | 'down' | 'flat' =
    avgTrend > 5 ? 'up' : avgTrend < -5 ? 'down' : 'flat';
    ...
    comparison: {
      ...
      vsHistoricalAvgPercent:
        historicalAverage !== null && historicalAverage > 0
          ? ((basicStats.mean - historicalAverage) / historicalAverage) * 100
          : null,
      vsBudgetTarget: null,
        ...
        <Svg>
          ...
          {#each topPayees as payee, pi}
            ...
            <MultiScatter
              ...
              fill={color}
              radius={4}
              onclick={(point) => handleDrillDown(point, payee)} />
          {/each}
          <Tooltip>
