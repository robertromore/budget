...
const allDailyCashFlow = $derived.by(() => {
  ...
  const sorted = Array.from(dataByDate.entries()).sort((a, b) => a[0].localeCompare(b[0]));
...
const cumulativeAsSpending = $derived(
  dailyCashFlow.map((d) => ({
    month: d.dateStr,
    monthLabel: formatShortDate(d.date),
    ...
    <div class="h-full w-full pb-20">
      {#key `${viewMode}-${chartType}`}
        <LayerCake
          ...
          x="index"
          y={viewMode === 'stacked'
            ? (d: (typeof dailyCashFlow)[0]) => d.income + d.expenses
            : viewMode === 'cumulative'
              ? 'cumulative'
              : 'net'}
          xScale={scaleLinear()}
