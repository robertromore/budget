    ...
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Content align="end" class="w-44">
        ...
        <DropdownMenu.CheckboxItem
          checked={viewMode === 'balance'}
          onCheckedChange={() => (viewMode = 'balance')}>
          Balance
          <span class="text-muted-foreground ml-1 text-xs">(owed)</span>
        </DropdownMenu.CheckboxItem>
        <DropdownMenu.CheckboxItem
          checked={viewMode === 'available'}
          onCheckedChange={() => (viewMode = 'available')}>
          Available Credit
        ...
        <LayerCake
          ...
          <Svg>
            ...
            {#if chartType === 'line-area'}
              <Area
                fill={viewMode === 'balance' ? 'var(--chart-1)' : 'var(--chart-2)'}
                opacity={0.2} />
              <Line
                stroke={viewMode === 'balance' ? 'var(--chart-1)' : 'var(--chart-2)'}
                strokeWidth={2} />
              <Scatter
                fill={getPointFill}
            ...
            <Tooltip
              ...
              {#snippet children({ point })}
                {@const histAvgDiff =
                  historicalAverage !== null ? getDisplayValue(point) - historicalAverage : null}
                {@const trendValue = showLinearTrend ? getTrendValueAtIndex(point.index) : null}
                {@const trendDiff =
                  trendValue !== null ? getDisplayValue(point) - trendValue : null}
