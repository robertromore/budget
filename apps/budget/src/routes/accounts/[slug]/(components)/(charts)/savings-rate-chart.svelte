...
const comprehensiveStats = $derived.by(() => {
  ...
  return {
    ...
    distribution: {
      highest: {
        value: rateStats.summary.highest.value,
        month: rateStats.summary.highest.month,
        monthLabel: rateStats.summary.highest.monthLabel,
      },
      lowest: {
        value: rateStats.summary.lowest.value,
        month: rateStats.summary.lowest.month,
        monthLabel: rateStats.summary.lowest.monthLabel,
      },
      ...
      coefficientOfVariation:
        rateStats.summary.average !== 0
          ? (rateStats.distribution.stdDev / Math.abs(rateStats.summary.average)) * 100
          : 0,
    },
          ...
          <Svg>
            ...
            <Tooltip
              ...
              {#snippet children({ point })}
                ...
                {@const histAvgDiff =
                  historicalAverage !== null ? point.savingsRate - historicalAverage : null}
                    ...
                    {#if showYoYComparison && yoyInfo}
                      <p class={yoyInfo.yoyChange >= 0 ? 'text-green-600' : 'text-amber-600'}>
                        ...
                        <span class="text-muted-foreground"
                          >({formatPercentRaw(yoyInfo.prevYearRate, 1)})</span>
                      </p>
                    {/if}
                  </div>
    ...
    {#if activeAnalysisCount > 0}
      <div
        ...
        {#if showYoYComparison}
          <div class="flex items-center gap-1.5">
            <div class="border-muted-foreground h-0.5 w-4 border-t border-dashed"></div>
            <span>Last Year</span>
          </div>
        {/if}
        {#if showLinearTrend}
