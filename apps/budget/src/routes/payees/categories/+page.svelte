	...
	<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
		<div>
			...
			<p class="text-muted-foreground">
				Organize your payees by creating custom categories
			</p>
		</div>
								...
								<Table.Cell>
									<div class="flex items-center justify-center">
										{#if category.icon}
											...
											>
												{#if iconData?.icon}
													<iconData.icon class="h-5 w-5" style={category.color ? `color: ${category.color}` : ''} />
												{:else}
			...
			<AlertDialog.Description>
				{#if categoryToDelete}
					...
					{#if categoryToDelete.payeeCount && categoryToDelete.payeeCount > 0}
						This will unassign {categoryToDelete.payeeCount} payee{categoryToDelete.payeeCount === 1 ? '' : 's'} from this category.
					{/if}
