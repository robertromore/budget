...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const { file: fileData, columnMapping, accountId: rawAccountId } = await request.json();
    ...
    if (accountId !== null && authorizedAccount) {
      ...
      const existingPayees = await db
        ...
        .where(
          and(
            eq(payeeTable.workspaceId, authorizedAccount.workspaceId),
            isNull(payeeTable.deletedAt)
          )
        );
    ...
    if (accountId !== null && authorizedAccount) {
      ...
      const allAccounts = await db
        ...
        .where(
          and(
            eq(accountsTable.workspaceId, authorizedAccount.workspaceId),
