...
const effectiveLevel = $derived(selectedLevel === 'inherit' ? userLevel : selectedLevel);
...
$effect(() => {
  if (workspaceSettingsQuery?.data) {
    ...
    selectedLevelStr =
      data.encryption.level === 'inherit' ? 'inherit' : String(data.encryption.level);
  }
});
...
const hasChanges = $derived(
  workspaceSettingsQuery?.data &&
    selectedLevelStr !==
      (workspaceSettingsQuery.data.encryption.level === 'inherit'
        ? 'inherit'
...
<div class="space-y-6">
  ...
  <Card.Root>
    <Card.Header>
      ...
      <Card.Description>
        Choose the encryption level for this workspace. This setting can override your personal
    ...
    <Card.Content>
      ...
      {:else if levelOptionsQuery.data}
        <RadioGroup.Root bind:value={selectedLevelStr} class="space-y-3">
          <!-- Inherit Option -->
          <label
            class="hover:bg-muted/50 flex cursor-pointer items-start gap-4 rounded-lg border p-4 transition-colors {selectedLevelStr ===
            'inherit'
            ...
            <div class="flex-1 space-y-2">
              ...
              <p class="text-muted-foreground text-sm">
                Use your personal encryption level (currently Level {userLevel}). Changes to your
                personal settings will apply to this workspace.
              </p>
            </div>
          </label>
          ...
          <p class="text-muted-foreground mb-2 text-sm">
            Or choose a specific level for this workspace:
          </p>
          ...
          {#each levelOptionsQuery.data as option (option.value)}
            ...
            <label
              class="hover:bg-muted/50 flex cursor-pointer items-start gap-4 rounded-lg border p-4 transition-colors {selectedLevel ===
              option.value
                ? 'border-primary bg-primary/5'
                : ''}"
  ...
  <Card.Root class="bg-muted/30">
    <Card.Content class="flex items-start gap-3 pt-6">
      ...
      <div class="text-muted-foreground space-y-2 text-sm">
        <p class="text-foreground font-medium">About Workspace Encryption</p>
        <ul class="list-inside list-disc space-y-1">
          <li>Workspace encryption uses your personal encryption key</li>
          <li>
            All members with access to the workspace will need their own keys to view encrypted data
          </li>
          <li>
            If you select "Inherit", changes to your personal security settings will automatically
            apply
          </li>
