...
function addTailwindClasses(html: string): string {
  return (
    html
      ...
      .replace(
        /<code class="font-mono text-sm">(?![^<]*<\/pre>)/g,
        '<code class="bg-muted rounded px-1.5 py-0.5 font-mono text-sm">'
      )
      // Links
      .replace(
        /<a href="/g,
        '<a class="text-primary underline underline-offset-4 hover:text-primary/80" target="_blank" rel="noopener noreferrer" href="'
      )
      // Horizontal rules
      ...
      .replace(
        /<blockquote>/g,
        '<blockquote class="border-l-4 border-primary/30 pl-4 italic text-muted-foreground mb-4">'
      )
  );
}
