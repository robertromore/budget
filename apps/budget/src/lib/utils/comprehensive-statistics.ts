...
function calculateComparisonStats(
  ...
  const historicalAvg = allTimeData.length > 0 ? mean(allTimeData.map((d) => d.spending)) : null;
...
export function calculateComprehensiveStatsForRate(
  ...
  return {
    ...
    targets: {
      ...
      averageAboveTarget: aboveTarget.length > 0 ? mean(aboveTarget.map((d) => d.rate)) : null,
      averageBelowTarget: belowTarget.length > 0 ? mean(belowTarget.map((d) => d.rate)) : null,
    },
  };
}
...
export function calculateComprehensiveStatsForRanking(
  ...
  const herfindahlIndex = total > 0 ? sum(values.map((v) => Math.pow(v / total, 2))) : 0;
...
export function calculateComprehensiveStatsForDualSeries(
  ...
  const highestIncome = data.reduce((max, d) => (d.income > max.income ? d : max));

  // Expenses stats
  const expenseAvg = mean(expenses);
  const expenseTotal = sum(expenses);
  const highestExpense = data.reduce((max, d) => (d.expenses > max.expenses ? d : max));
