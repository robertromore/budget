...
function computeBudgetSuggestion(
  ...
  if (seasonalTrends && seasonalTrends.length >= 6) {
    ...
    const variance =
      amounts.reduce((sum, a) => sum + Math.pow(a - monthlyAvg, 2), 0) / amounts.length;
...
export function usePayeeInsights(payeeId: number | undefined) {
  ...
  const error = $derived(intelligenceWithProfileQuery?.error || suggestionsQuery?.error || null);
  ...
  const aiExplanation = $derived(
    (intelligenceWithProfileQuery?.data?.payee as { aiExplanation?: string | null })
      ?.aiExplanation ?? null
  );
  const aiExplanationUpdatedAt = $derived(
    (intelligenceWithProfileQuery?.data?.payee as { aiExplanationUpdatedAt?: string | null })
      ...
      factors: [
        {
          ...
          description:
            dataQuality === "excellent"
              ...
              : `${Math.round(metrics.dataQuality.score * 100)}% quality`,
        },
        {
          name: "Pattern Reliability",
          score: metrics.patternReliability?.score ?? 0.5,
          description:
            (metrics.patternReliability?.score ?? 0) > 0.7
              ? "Reliable patterns"
              : "Variable patterns",
        },
      ],
