...
async function runAnalysis() {
  ...
  try {
    ...
    const uncategorized = txnList.filter((t) => !t.categoryId).slice(0, 20);
  ...
  <Card.Content>
    {#if isAnalyzing}
      <div class="flex flex-col items-center gap-3 p-8">
        ...
        <p class="text-muted-foreground text-sm">Analyzing uncategorized transactions...</p>
      </div>
    {:else if !hasRun}
      <div class="p-6 text-center">
        <Tags class="text-muted-foreground mx-auto mb-3 h-10 w-10" />
    ...
    {:else if suggestionsWithMatch.length === 0}
      <div class="p-6 text-center">
        <AlertTriangle class="text-muted-foreground mx-auto mb-3 h-10 w-10" />
        <p class="text-muted-foreground text-sm">
          Found {uncategorizedCount} uncategorized transaction{uncategorizedCount !== 1 ? 's' : ''} but
    ...
    {:else}
      <div class="space-y-1.5">
        <p class="text-muted-foreground mb-3 text-sm">
          {suggestionsWithMatch.length} of {uncategorizedCount} uncategorized transaction{uncategorizedCount !==
          1
            ? 's'
            : ''} have suggestions
        </p>
