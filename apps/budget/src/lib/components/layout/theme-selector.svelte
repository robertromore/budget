...
<ResponsiveSheet bind:open {onOpenChange}>
  {#snippet header()}
    <div>
      ...
      <p class="text-muted-foreground text-sm">Choose a theme for your dashboard</p>
    </div>
  {/snippet}
  ...
  {#snippet content()}
    <div class="flex flex-col gap-4">
      {#if !showCustomPicker}
        <!-- Preset Themes Grid -->
        <div>
          <h3 class="mb-3 text-sm font-medium">Preset Themes</h3>
          <ScrollArea class="-mx-6 h-[400px] px-6">
            <div class="grid grid-cols-1 gap-3 pb-4 @sm:grid-cols-2 @lg:grid-cols-3">
              {#each THEME_PRESETS as preset}
                ...
                <button
                  ...
                  class="group hover:border-primary relative flex flex-col items-start gap-2 rounded-lg border-2 p-3 transition-colors"
                  ...
                  <div class="flex items-center gap-2">
                    <div class="h-6 w-6 rounded-md border" style:background-color={previewColor}>
                    </div>
        ...
        <!-- Custom Color Option -->
        <div>
          <h3 class="mb-3 text-sm font-medium">Custom</h3>
          <button
            type="button"
            onclick={() => (showCustomPicker = true)}
            class="group hover:border-primary relative flex w-full flex-col items-start gap-2 rounded-lg border-2 p-3 transition-colors"
            ...
            <div class="flex items-center gap-2">
              <div
                ...
                style:background-color={isCustom && themePreferences.custom
                  ? themePreferences.custom
                  : customColorValue}>
              </div>
      ...
      {:else}
        ...
        <div class="space-y-4">
          <div class="flex items-center justify-between">
            ...
            <Button variant="ghost" size="sm" onclick={() => (showCustomPicker = false)}>
              Back
            </Button>
          </div>
          ...
          <div class="flex gap-2">
            <Button variant="outline" class="flex-1" onclick={() => (showCustomPicker = false)}>
              Cancel
            </Button>
            <Button class="flex-1" onclick={handleCustomColorApply}>Apply</Button>
          </div>
        </div>
      {/if}
    </div>
