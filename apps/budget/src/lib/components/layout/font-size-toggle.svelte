  ...
  <Tooltip.Trigger>
    {#snippet child({ props })}
      <Button
        ...
        data-help-id="font-size-toggle"
        data-help-title="Font Size">
        <ALargeSmall class="h-4 w-4" />
        <span
          class="bg-muted text-muted-foreground absolute -right-1 -bottom-1 flex h-3.5 w-3.5 items-center justify-center rounded-full text-[8px] font-semibold"
          >{sizeLabels[current]}</span>
