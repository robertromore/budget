...
const visibleTabs = $derived(pageTabs?.config?.tabs.filter((tab) => tab.condition !== false) ?? []);
  ...
  <Tabs.Root
    ...
    <Tabs.List class="h-8 gap-1 bg-transparent p-0">
      {#each visibleTabs as tab (tab.id)}
            ...
            <Tooltip.Trigger>
              {#snippet child({ props })}
                <Tabs.Trigger
                  {...props}
                  value={tab.id}
                  class="data-[state=active]:bg-muted h-8 w-8 px-0 text-sm">
        ...
        {:else}
          <Tabs.Trigger value={tab.id} class="data-[state=active]:bg-muted h-8 px-3 text-sm">
            {#if tab.icon}
