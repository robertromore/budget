...
const monthLabels = $derived.by(() => {
  ...
  for (let m = 0; m < 12; m++) {
    ...
    const dayOfYear = Math.ceil(
      (firstDayOfMonth.getTime() - new Date(firstDayOfMonth.getFullYear(), 0, 0).getTime()) /
        86400000
    );
...
<div class={cn('relative', className)}>
  ...
  <div
    ...
    class="bg-muted/60 relative h-12 touch-none rounded-lg select-none"
    ...
    <!-- Range highlight -->
    <div
      class="bg-primary/20 absolute top-4 bottom-0 rounded transition-all"
      style="left: {rangeStartPercent}%; width: {rangeEndPercent - rangeStartPercent}%">
    </div>

    <!-- Start marker -->
    <div
      class="bg-primary absolute top-4 bottom-0 w-1 rounded-l transition-all"
      style="left: {rangeStartPercent}%">
    </div>

    <!-- End marker -->
    <div
      class="bg-primary absolute top-4 bottom-0 w-1 rounded-r transition-all"
  ...
  {#if isDragging}
    <div
      class="bg-primary text-primary-foreground pointer-events-none absolute top-full left-1/2 z-20 mt-2 flex h-10 min-w-32 -translate-x-1/2 items-center justify-center rounded-lg px-3 text-sm font-bold shadow-lg">
      {previewText}
  ...
  <!-- Instructions -->
  <p class="text-muted-foreground mt-2 text-center text-xs">Click and drag to select date range</p>
</div>
