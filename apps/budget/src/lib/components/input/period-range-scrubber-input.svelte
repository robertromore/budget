...
let selectionPhase = $state<'idle' | 'selecting-start' | 'start-selected' | 'selecting-duration'>(
  'idle'
);
...
function getMonthStyle(monthIndex: number): string {
  const inStartSelectedPhase =
    selectionPhase === 'start-selected' || selectionPhase === 'selecting-start';
  ...
  const rangeOverlaps =
    (inDurationPhase || inIdlePhase) && monthStart <= rangeEnd && monthEnd >= rangeStart;
...
<div bind:this={containerRef} class={cn('relative', className)}>
  ...
  {#if isFocused || isDragging || selectionPhase !== 'idle'}
    <div
      ...
      class="bg-muted/80 absolute top-full right-0 left-0 z-10 mt-1 touch-none rounded-md px-1 py-1.5 backdrop-blur-sm select-none"
      ...
      <!-- Drag indicator -->
      <div
        class="text-muted-foreground mt-0.5 flex cursor-ew-resize justify-center opacity-40 hover:opacity-60">
    ...
    <div class="mt-1 text-center text-[10px]">
      <p class="text-muted-foreground">
        {instructionText}
        {#if selectionPhase === 'start-selected'}
          <button type="button" class="text-primary ml-1 underline" onclick={handleReset}>
            Reset
          </button>
        {/if}
    ...
    {#if isDragging || selectionPhase !== 'idle'}
      <div
        class="bg-primary text-primary-foreground pointer-events-none absolute top-full left-1/2 z-20 mt-8 flex h-10 min-w-24 -translate-x-1/2 items-center justify-center rounded-lg px-3 text-sm font-bold shadow-lg">
        {previewText}
        {#if selectionPhase === 'selecting-duration'}
