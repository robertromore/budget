...
<ResponsiveSheet
  bind:open={() => aiChat.isOpen, (v) => (v ? aiChat.open() : aiChat.close())}
  side="right"
  defaultWidth={480}
  minWidth={360}
  maxWidth={640}
          ...
          {#each aiChat.messages as message (message.id)}
            <Message from={message.role}>
              {#if message.role === 'user'}
                ...
                <div
                  class="bg-primary text-primary-foreground flex size-8 shrink-0 items-center justify-center rounded-full">
                  <User class="size-4" />
                </div>
              {:else}
                <MessageContent variant="flat">
                  {#if message.reasoning}
                    <Reasoning isStreaming={message.reasoningStreaming}>
                      <ReasoningTrigger />
                      <ReasoningContent>
                        <p class="text-sm whitespace-pre-wrap">{message.reasoning}</p>
                      </ReasoningContent>
                    </Reasoning>
                  {/if}
                  {#if message.content}
