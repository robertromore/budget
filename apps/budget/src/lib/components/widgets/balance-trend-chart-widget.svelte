...
<WidgetCard {config} {data} {editMode} {...onUpdate && {onUpdate}} {...onRemove && {onRemove}}>
  <div class="space-y-3">
    <div class="flex items-center justify-between">
      ...
      <div class="text-muted-foreground text-xs">{periodFormatter.toAdverb(period)}</div>
    </div>
    ...
    {#if chartData.length > 0}
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="text-right">
          <div
            class="flex items-center gap-1 {totalChange >= 0 ? 'text-green-600' : 'text-red-600'}">
            {#if totalChange > 0}
      ...
      {#if config.size === 'medium' || config.size === 'large'}
        <div class="grid grid-cols-2 gap-3 text-xs">
          <div class="rounded-lg bg-green-50 p-2 text-center dark:bg-green-950/20">
            <div class="font-medium text-green-600">Highest</div>
            <div class="font-semibold text-green-700 dark:text-green-400">
              {currencyFormatter.format(maxBalance)}
            </div>
          </div>

          <div class="rounded-lg bg-red-50 p-2 text-center dark:bg-red-950/20">
            <div class="font-medium text-red-600">Lowest</div>
      ...
      {#if config.size === 'large' && balanceHistory.length > 1}
        <div class="text-muted-foreground flex justify-between border-t pt-1 text-xs">
          <span>{new Date(balanceHistory[0].date).toLocaleDateString()}</span>
          <span
            >{new Date(balanceHistory[balanceHistory.length - 1].date).toLocaleDateString()}</span>
        </div>
      {/if}
    {:else}
