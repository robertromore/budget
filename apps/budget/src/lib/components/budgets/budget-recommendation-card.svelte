<script lang="ts">
  ...
  import { applyRecommendation, dismissRecommendation, restoreRecommendation } from "$lib/query/budgets";
  ...
  const isProcessing = $derived(applyMutation.isPending || dismissMutation.isPending || restoreMutation.isPending);
...
<Card.Root class="overflow-hidden {isDismissed ? 'opacity-50' : ''}">
    ...
    {#if recommendation.metadata}
      <div class="grid gap-3 text-sm">
        ...
        {#if recommendation.metadata['goalMetadata']}
          {@const goalMeta = recommendation.metadata['goalMetadata'] as { targetAmount?: number; targetDate?: string; monthlyContribution?: number }}
          <div class="space-y-2 rounded-lg bg-blue-50 dark:bg-blue-950 px-3 py-2">
            <div class="flex items-center justify-between">
        ...
        {#if recommendation.metadata['categoryBreakdown'] && Array.isArray(recommendation.metadata['categoryBreakdown'])}
          {@const categoryBreakdown = recommendation.metadata['categoryBreakdown'] as Array<{ categoryName: string; amount: number }>}
          {#if categoryBreakdown.length > 0}
            <div class="space-y-2">
              <span class="text-sm font-medium text-muted-foreground">Top Categories:</span>
  ...
  <Card.Footer class="flex gap-2 bg-muted/50 pt-3">
    ...
    {:else if recommendation.status === "applied"}
      <div class="flex items-center gap-2 text-sm text-green-600 w-full justify-center py-1">
    ...
    {:else if recommendation.status === "dismissed"}
      <div class="flex items-center gap-2 w-full">
        <div class="flex items-center gap-2 text-sm text-muted-foreground flex-1">
    ...
    {:else if recommendation.status === "expired"}
      <div class="flex items-center gap-2 text-sm text-muted-foreground w-full justify-center py-1">
