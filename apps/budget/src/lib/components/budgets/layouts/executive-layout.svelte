<script lang="ts">
  ...
  const healthScore = $derived.by(() => {
    if (progressPercentage > 100) {
      return Math.max(0, 100 - (progressPercentage - 100) * 2);
    } else if (progressPercentage > 90) {
      return 100 - (progressPercentage - 75) * 2;
    } else if (progressPercentage > 75) {
      return 100 - (progressPercentage - 50) * 0.5;
    }
    return 100;
  });
...
<div class="space-y-8 {className}">
  ...
  <div class="text-center space-y-4 pb-8 border-b">
    <div class="flex items-center justify-center gap-3">
      ...
      <div>
        <h2 class="text-2xl font-bold">Budget Health: {healthScore.toFixed(0)}/100</h2>
        <p class="text-muted-foreground capitalize">{status} Performance</p>
      </div>
    </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
    ...
    <BudgetMetricCard
      ...
      icon={DollarSign}
      status={remaining >= 0 ? "good" : "danger"}
    />
      ...
      <Card.Content class="space-y-6">
        ...
        <div class="space-y-3">
          <div class="flex justify-between items-center">
            ...
            <Badge variant={status === 'danger' ? 'destructive' : status === 'warning' ? 'secondary' : 'default'}>
        ...
        <div class="grid grid-cols-2 gap-4 pt-4 border-t">
          <div class="text-center">
            <p class="text-2xl font-bold text-green-600">${allocated.toFixed(0)}</p>
            <p class="text-sm text-muted-foreground">Allocated</p>
          </div>
          <div class="text-center">
            <p class="text-2xl font-bold {spent > allocated ? 'text-red-600' : 'text-blue-600'}">
              ${spent.toFixed(0)}
            </p>
    ...
    <Card.Content>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="text-center space-y-2">
          ...
          <p class="text-sm text-muted-foreground">
            {progressPercentage < 75 ? "Excellent" :
             progressPercentage < 90 ? "Good" :
             progressPercentage < 100 ? "Fair" : "Poor"}
          </p>
        </div>
        ...
        <div class="text-center space-y-2">
          ...
          <p class="text-sm text-muted-foreground">
            {burnRate < (allocated / 30) ? "Improving" : "Stable"}
          </p>
        </div>
      </div>
    </Card.Content>
