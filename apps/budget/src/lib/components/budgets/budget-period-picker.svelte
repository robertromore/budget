...
const selectedIndex = $derived.by(() =>
  sortedPeriods.findIndex(
    (period: BudgetPeriodInstance) => selectedId !== '' && String(period.id) === selectedId
  )
);
...
$effect(() => {
  ...
  const hasSelected =
    selectedId !== '' &&
    periods.some((period: BudgetPeriodInstance) => String(period.id) === selectedId);
  if (!hasSelected) {
...
<div class={cn('flex flex-col gap-2', className)}>
  ...
  <div class="flex items-center gap-2">
    ...
    <Select.Root type="single" bind:value={selectedId} disabled={loading || !sortedPeriods.length}>
      ...
      <Select.Content class="max-h-64">
        {#if !sortedPeriods.length}
          <div class="text-muted-foreground px-3 py-2 text-sm">No periods available</div>
        {:else}
          {#each sortedPeriods as period (period.id)}
            <Select.Item value={String(period.id)} class="flex flex-col gap-0.5">
              <span class="text-foreground text-sm font-medium">{formatRange(period)}</span>
              <span class="text-muted-foreground text-xs">
                Allocated {formatValue(period.allocatedAmount)} · Spent {formatValue(
    ...
    <div class="text-muted-foreground grid gap-1 text-xs">
      <div class="flex items-center justify-between">
        ...
        <span class="text-card-foreground font-medium"
          >{formatValue(selectedPeriod.allocatedAmount)}</span>
      </div>
      <div class="flex items-center justify-between">
        <span>Spent</span>
        <span class="text-card-foreground font-medium"
          >{formatValue(selectedPeriod.actualAmount ?? 0)}</span>
      </div>
      <div class="flex items-center justify-between">
        <span>Rollover</span>
        <span class="text-card-foreground font-medium"
          >{formatValue(selectedPeriod.rolloverAmount ?? 0)}</span>
      </div>
      {#if timezone}
