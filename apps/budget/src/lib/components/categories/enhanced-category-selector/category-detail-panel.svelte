...
<div class="flex h-full flex-col">
  ...
  {#if mode === 'view'}
    <div class="flex items-center justify-between border-b px-4 py-4">
      <div class="flex items-center gap-3">
        {#if showBackButton}
          <button type="button" onclick={onBack} class="hover:bg-accent rounded p-1">
  ...
  {:else}
    <div class="flex items-center gap-3 border-b px-4 py-4">
      <button type="button" onclick={handleCancel} class="hover:bg-accent rounded p-1">
        <ArrowLeft class="h-5 w-5" />
      </button>
      <h3 class="text-lg font-semibold">
  ...
  {#if mode === 'view' && category}
    ...
    <div class="flex-1 space-y-6 overflow-auto p-4">
      ...
      <div class="flex items-start gap-4">
        <div
          ...
          <IconComponent
            class="h-7 w-7"
            style={category.categoryColor ? `color: ${category.categoryColor};` : undefined} />
        </div>
        <div>
    ...
    <div class="border-t p-4">
      <Button variant="outline" class="w-full" onclick={() => onModeChange('edit')}>
