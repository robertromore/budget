  ...
  <Card.Content>
    {#if selectedNode}
      <div class="flex items-start gap-4">
        ...
        <div class="shrink-0">
          <div
            class={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium text-white ${headerColor()}`}>
            {nodeTypeLabel()}
          </div>
          <p class="text-muted-foreground mt-1 text-xs">ID: {selectedNode.id}</p>
        </div>
        ...
        <div class="flex-1">
          ...
          {:else if selectedNode.type === 'condition'}
            ...
            {@const fieldLabel =
              conditionFields[entityType]?.find((f) => f.field === field)?.label || field}
            {@const opLabel = operator ? operatorInfo[operator]?.label || operator : undefined}
            <div class="text-muted-foreground flex gap-3 text-xs">
              <span
                >Field: <strong class="text-foreground">{fieldLabel || '(not set)'}</strong></span>
              <span>Op: <strong class="text-foreground">{opLabel || '(not set)'}</strong></span>
              <span
                >Value: <strong class="text-foreground"
                  >{String(selectedNode.data.value) || '(not set)'}</strong
                ></span>
            </div>
          {:else if selectedNode.type === 'action'}
            {@const actionType = selectedNode.data.type as string | undefined}
            <div class="text-muted-foreground text-xs">
              Type: <strong class="text-foreground"
                >{actionType ? getActionLabel(actionType) : '(not set)'}</strong>
            </div>
          {:else if selectedNode.type === 'group'}
            <div class="text-muted-foreground text-xs">
              Logic: <strong class="text-foreground"
                >{selectedNode.data.operator === 'AND'
                  ? 'All must match'
                  : 'Any can match'}</strong>
            </div>
          {/if}
        </div>
        ...
        <div class="flex shrink-0 gap-2">
          {#if selectedNode.type !== 'trigger'}
            <Button variant="outline" size="sm" onclick={() => onDuplicateNode(selectedNode.id)}>
    ...
    {:else}
      <div class="flex items-center gap-3 py-2">
        <Settings class="text-muted-foreground/50 h-5 w-5" />
        <p class="text-muted-foreground text-sm">Select a node to view its properties</p>
      </div>
    {/if}
  </Card.Content>
</Card.Root>
