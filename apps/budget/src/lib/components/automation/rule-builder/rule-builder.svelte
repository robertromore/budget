...
import { createDefaultRuleConfig, flowToRule, ruleToFlow, type RuleConfig } from './utils';
...
let ruleConfig = $state<RuleConfig>(initialRuleConfig ?? createDefaultRuleConfig(entityType));
...
$effect(() => {
  if (initialFlowState && !initialRuleConfig) {
    try {
      ...
      const imported = flowToRule(initialFlowState.nodes as any[], initialFlowState.edges as any[]);
      ruleConfig = imported;
    } catch {
...
<div class={cn('flex flex-col', className)}>
  <Tabs.Root bind:value={activeView}>
    ...
    <Tabs.Content value="natural-language" class="mt-4">
      <div class="space-y-4">
        <RuleBuilderNl {ruleConfig} {entityType} {readonly} onRuleChange={handleNLChange} />
