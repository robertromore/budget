...
$effect(() => {
  ...
  if (initialRuleConfig) {
    ...
    if (initialFlowState?.nodes?.length) {
      const positionMap = new Map(initialFlowState.nodes.map((n) => [n.id, n.position]));
...
<div class={cn('flex flex-col gap-4', className)}>
  ...
  <!-- Flow Canvas -->
  <div
    class="bg-muted/20 h-80 overflow-hidden rounded-lg border"
  ...
  {#if !readonly && validationErrors.length > 0}
    <div class="border-destructive/50 bg-destructive/10 rounded-lg border p-3">
      <h4 class="text-destructive mb-2 text-sm font-medium">Validation Errors</h4>
