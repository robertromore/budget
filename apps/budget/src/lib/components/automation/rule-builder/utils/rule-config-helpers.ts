 ...
 */
export function updateTrigger(config: RuleConfig, updates: Partial<TriggerConfig>): RuleConfig {
  return {
    ...config,
    trigger: {
      ...config.trigger,
 ...
 */
export function findGroupById(group: ConditionGroup, id: string): ConditionGroup | null {
  if (group.id === id) {
    return group;
  }
 ...
 */
export function removeConditionById(config: RuleConfig, conditionId: string): RuleConfig {
  const newConditions = deepCloneConditionGroup(config.conditions);
 ...
 */
export function moveAction(config: RuleConfig, actionId: string, toIndex: number): RuleConfig {
