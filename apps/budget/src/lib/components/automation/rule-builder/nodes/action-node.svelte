...
const targetPosition = $derived(
  isHorizontalLayout($layoutDirection) ? Position.Left : Position.Top
);
...
$effect(() => {
  localParams = data.params || {};
});
$effect(() => {
  localContinueOnError = data.continueOnError || false;
});
...
<div
  class="action-node bg-background w-72 rounded-lg border-2 shadow-md transition-all {selected
  ...
  <Handle
    ...
    class="!h-3 !w-3 !border-2 !border-white !bg-orange-500" />
  ...
  <div class="space-y-3 p-3">
    ...
    <div class="space-y-1.5">
      <Label class="text-muted-foreground text-xs">Action Type</Label>
          ...
          <div class="space-y-1.5">
            ...
            {:else if param.type === 'text'}
              <Textarea
                value={String(localParams?.[param.name] || '')}
                oninput={(e) =>
                  handleParamChange(param.name, (e.target as HTMLTextAreaElement).value)}
            ...
            {:else if param.type === 'reference'}
                ...
                <Select.Root
                  ...
                  <Select.Trigger class="h-8 text-sm">
                    {#if localParams?.[param.name]}
                      {@const account = accountsState?.accounts.get(
                        localParams[param.name] as number
                      )}
              ...
              {:else if param.referenceType === 'categoryGroup'}
                <Select.Root
                  ...
                  <Select.Trigger class="h-8 text-sm">
                    {#if localParams?.[param.name]}
                      {@const category = categoriesState?.getById(
                        localParams[param.name] as number
                      )}
    ...
    <div class="flex items-center justify-between border-t pt-3">
      <Label class="text-muted-foreground text-xs">Continue on error</Label>
      <Switch checked={localContinueOnError} onCheckedChange={handleContinueOnErrorChange} />
    </div>
  </div>
</div>
