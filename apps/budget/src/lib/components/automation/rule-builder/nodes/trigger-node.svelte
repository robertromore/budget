...
const sourcePosition = $derived(
  isHorizontalLayout($layoutDirection) ? Position.Right : Position.Bottom
);
...
const availableEvents = $derived(localEntityType ? triggerEvents[localEntityType] || [] : []);
...
<div
  class="trigger-node bg-background w-64 rounded-lg border-2 shadow-md transition-all {selected
  ...
  <div class="space-y-3 p-3">
    <div class="space-y-1.5">
      <Label class="text-muted-foreground text-xs">When</Label>
      <Select.Root type="single" value={localEntityType} onValueChange={handleEntityTypeChange}>
    ...
    <div class="space-y-1.5">
      <Label class="text-muted-foreground text-xs">Event</Label>
      <Select.Root
        type="single"
        value={localEvent}
