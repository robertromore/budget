...
const currentOperator = $derived(
  localOperator ? operatorInfo[localOperator as ConditionOperator] : null
);
...
<div
  class="condition-node bg-background w-72 rounded-lg border-2 shadow-md transition-all {selected
  ...
  <div class="flex items-center justify-between rounded-t-md bg-blue-500 px-3 py-2 text-white">
    ...
    <div class="flex items-center gap-1.5">
      ...
      <Switch checked={localNegate} onCheckedChange={handleNegateChange} class="scale-75" />
    </div>
  </div>
  ...
  <div class="space-y-3 p-3">
    ...
    <div class="space-y-1.5">
      <Label class="text-muted-foreground text-xs">Field</Label>
          ...
          {#each availableFields as field (field.field)}
            <Select.Item value={field.field}>
              <div class="flex flex-col">
                ...
                <span class="text-muted-foreground text-xs">{field.type}</span>
              </div>
            </Select.Item>
          {/each}
        </Select.Content>
    ...
    <!-- Operator Selection -->
    <div class="space-y-1.5">
      <Label class="text-muted-foreground text-xs">Operator</Label>
      <Select.Root
        type="single"
        value={localOperator}
    ...
    {#if currentOperator && currentOperator.valueCount > 0}
      <div class="space-y-1.5">
        <Label class="text-muted-foreground text-xs">Value</Label>
        ...
        {:else if selectedField?.type === 'reference' && selectedField.referenceType}
          ...
          {:else}
            <Input
              ...
              value={String(localValue || '')}
              oninput={(e) =>
                handleReferenceValueChange(parseInt((e.target as HTMLInputElement).value) || null)}
              placeholder="ID..."
              class="h-8 text-sm" />
          {/if}
        {:else}
    ...
    {#if localOperator === 'between'}
      <div class="space-y-1.5">
        <Label class="text-muted-foreground text-xs">And</Label>
        {#if selectedField?.type === 'number'}
          <NumericInput
