...
<div
  class="group flex flex-wrap items-center gap-2 rounded-md border border-orange-200 bg-orange-50/50 px-3 py-2 dark:border-orange-800 dark:bg-orange-950/30">
  <!-- "Then" prefix -->
      ...
      {#each availableActions as actionDef (actionDef.type)}
        <Select.Item value={actionDef.type}>
          <div class="flex flex-col">
            ...
            <span class="text-muted-foreground text-xs">{actionDef.description}</span>
          </div>
        </Select.Item>
      {/each}
    </Select.Content>
    ...
    {#each selectedAction.params as param (param.name)}
      ...
      {#if param.type === 'boolean'}
        <div class="flex items-center gap-1.5">
          <span class="text-muted-foreground text-sm">{param.label.toLowerCase()}:</span>
          <Switch
      ...
      {:else if param.type === 'number'}
        <div class="flex items-center gap-1.5">
          ...
          <Input
            ...
            value={String(paramValue || '')}
            oninput={(e) =>
              handleParamChange(param.name, parseFloat((e.target as HTMLInputElement).value) || 0)}
