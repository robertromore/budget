...
let { variant = 'condition', label, showDropdown = false, onAdd, children }: Props = $props();
...
{#if showDropdown && children}
  <DropdownMenu.Root>
    <DropdownMenu.Trigger
      class="hover:bg-accent focus-visible:ring-ring inline-flex h-7 items-center justify-center gap-1 rounded-md px-2 text-xs font-medium transition-colors focus-visible:ring-1 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 {buttonClass}">
...
{:else}
  <Button variant="ghost" size="sm" class="h-7 gap-1 px-2 text-xs {buttonClass}" onclick={onAdd}>
    <Plus class="h-3.5 w-3.5" />
    {displayLabel}
  </Button>
{/if}
