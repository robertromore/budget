...
const filteredRules = $derived(() => {
  ...
  // Status filter
  if (statusFilter !== 'all') {
    result = result.filter((r) => (statusFilter === 'enabled' ? r.isEnabled : !r.isEnabled));
  }
...
<div class="space-y-4">
  ...
  <div class="flex flex-wrap items-center gap-3">
    <div class="relative min-w-[200px] flex-1">
      ...
      <Input type="search" placeholder="Search rules..." class="pl-9" bind:value={searchQuery} />
    </div>
  ...
  {:else if filteredRules().length === 0}
    <div class="rounded-lg border border-dashed py-12 text-center">
      <Zap class="text-muted-foreground/50 mx-auto mb-3 h-10 w-10" />
      {#if rules.length === 0}
  ...
  {:else}
    <div class="grid gap-4 md:grid-cols-2">
      {#each filteredRules() as rule (rule.id)}
        <RuleCard {rule} {onToggleEnabled} {onEdit} {onDuplicate} {onDelete} {onViewLogs} />
      {/each}
    </div>
  {/if}
</div>
