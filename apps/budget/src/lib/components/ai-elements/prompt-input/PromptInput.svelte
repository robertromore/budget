<script lang="ts">
	...
	// Sync props to context reactively (fixes state_referenced_locally warning)
	$effect(() => {
		attachmentsContext.accept = accept;
		attachmentsContext.multiple = multiple;
		attachmentsContext.maxFiles = maxFiles;
		attachmentsContext.maxFileSize = maxFileSize;
		attachmentsContext.onError = onError;
	});
