  ...
  <Card.Content class="space-y-4">
    ...
    {:else if recommendations.length === 0}
      <div class="flex flex-col items-center justify-center py-12 text-center">
        <CircleAlert class="text-muted-foreground/50 mb-4 h-12 w-12" />
    ...
    {:else}
      {#each groupedRecommendations as group (group.groupName)}
        {@const isProcessing =
          approveMutation.isPending || dismissMutation.isPending || rejectMutation.isPending}
        ...
        <div class="bg-card rounded-lg border">
          ...
          <div class="bg-muted/30 border-b p-4">
            <div class="flex items-center justify-between gap-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <div class="flex items-center gap-2">
                    <h3 class="text-base font-semibold">{group.groupName}</h3>
                    {#if group.isNewGroup}
                      <Badge variant="outline" class="text-xs">New Group</Badge>
                    {/if}
                  </div>
                  <div class="mt-1 flex items-center gap-2">
                    <span class="text-muted-foreground text-xs">
                      {group.recommendations.length}
                      {group.recommendations.length === 1 ? 'category' : 'categories'}
                    </span>
          ...
          <div class="divide-y">
            {#each group.recommendations as recommendation (recommendation.id)}
              <div class="hover:bg-muted/50 p-3 transition-colors">
                <div class="flex items-center justify-between gap-4">
