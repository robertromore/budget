...
<Card.Root class="hover:shadow-md transition-shadow cursor-pointer" onclick={viewGroup}>
		...
		<div class="flex items-center gap-4 text-sm text-muted-foreground">
			...
			{#if group.pendingRecommendationCount > 0}
				<Badge variant="secondary">
					{group.pendingRecommendationCount} {group.pendingRecommendationCount === 1 ? 'suggestion' : 'suggestions'}
				</Badge>
			{/if}
		</div>
	</Card.Content>
