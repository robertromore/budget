  ...
  {#if isLoading}
    ...
    <div class="flex flex-col items-center justify-center gap-2 py-4">
      <div class="relative">
        ...
        <Brain class="text-primary/60 absolute inset-0 m-auto h-3 w-3" />
      </div>
      <p class="text-muted-foreground animate-pulse text-xs">Calculating budget...</p>
    </div>
  {:else if suggestion}
    ...
    <div class="mb-1.5 flex items-center justify-between">
      <div class="flex items-center gap-1.5">
        ...
        {#if suggestion.tier === 'ai'}
          <Tooltip.Root>
            <Tooltip.Trigger>
              <Badge
                variant="outline"
                class="h-4 border-violet-300 px-1 text-[10px] text-violet-600">
        ...
        {:else if suggestion.tier === 'ml'}
          <Tooltip.Root>
            <Tooltip.Trigger>
              <Badge variant="outline" class="text-primary border-primary/40 h-4 px-1 text-[10px]">
      ...
      <div class="flex items-center gap-1.5">
        {#if onRefresh}
          <Tooltip.Root>
            <Tooltip.Trigger>
              <Button variant="ghost" size="sm" class="h-5 w-5 p-0" onclick={onRefresh}>
    ...
    <div class="flex items-center justify-between gap-2">
      ...
      {#if onApply}
        <Button
          variant="outline"
          size="sm"
          onclick={() => onApply(displayAmount ?? suggestion.totalSuggested)}>
          Set Budget
        </Button>
      {/if}
    </div>
    ...
    {#if onFeedback}
      <div class="mt-2 flex items-center gap-2 border-t pt-2">
        <span class="text-muted-foreground text-xs"
          >{submittedRating ? 'Thanks!' : 'Helpful?'}</span>
    ...
    <Collapsible.Root bind:open={reasoningExpanded} class="mt-2">
      ...
      <Collapsible.Content>
        <div
          class="text-muted-foreground bg-muted/50 mt-1.5 space-y-1 rounded p-2 text-xs leading-relaxed">
