...
let { type, title, description, priority, actionLabel, onAction, onDismiss }: Props = $props();
...
<div class="relative rounded-lg border p-4 {config().borderClass} {config().bgClass}">
  {#if onDismiss}
    <button
      onclick={onDismiss}
