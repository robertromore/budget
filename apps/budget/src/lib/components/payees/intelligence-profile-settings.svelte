...
let { open = $bindable(false), onOpenChange, payeeId, profile, onProfileUpdated }: Props = $props();
...
const predictionMethodOptions = [
  {
    ...
    description: 'Inherit from workspace settings',
    icon: null,
  },
  {
    value: 'statistical',
    label: 'Statistical Only',
    description: 'Fast pattern matching (no AI/ML)',
    icon: Zap,
  },
  {
    value: 'ml',
  ...
  },
  {
    value: 'ai',
    label: 'AI Powered',
    description: 'ML predictions + natural language explanations',
    icon: Sparkles,
  },
] as const;
...
<ResponsiveSheet bind:open {onOpenChange}>
    ...
    <div class="space-y-6">
      ...
      <div class="space-y-3">
        ...
        <div class="grid gap-2">
          {#each predictionMethodOptions as option (option.value)}
            ...
            <button
              type="button"
              class="hover:bg-muted/50 flex items-start gap-3 rounded-lg border p-3 text-left transition-colors {isSelected
      ...
      <div class="space-y-6" class:opacity-50={!enabled} class:pointer-events-none={!enabled}>
        ...
        <div class="space-y-3">
          ...
          <p class="text-muted-foreground text-sm">
            Include only transactions from these category types. Leave all unchecked to include all
            types.
          </p>
          <div class="grid grid-cols-2 gap-3">
            {#each categoryTypeOptions as option (option.value)}
              ...
              <button
                ...
                disabled={isLoading}>
                <Checkbox checked={isSelected} class="mt-0.5" />
                <div>
        ...
        <div class="space-y-3">
          ...
          <p class="text-muted-foreground text-sm">Filter by transaction direction.</p>
        ...
        <div class="space-y-3">
          ...
          <p class="text-muted-foreground text-sm">Limit analysis to recent transactions.</p>
        ...
        <div class="space-y-3">
          ...
          <p class="text-muted-foreground text-sm">Filter transactions by absolute amount value.</p>
  ...
  {#snippet footer()}
    <div class="flex items-center justify-between gap-3">
      <Button variant="outline" onclick={handleReset} disabled={isLoading || !profile?.enabled}>
