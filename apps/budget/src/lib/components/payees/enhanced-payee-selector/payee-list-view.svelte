...
const alphabetGroups = $derived.by((): AlphabetGroup[] => {
  ...
  const sorted = [...filteredPayees].sort((a, b) => (a.name || '').localeCompare(b.name || ''));
...
const availableLetters = $derived(new Set(alphabetGroups.map((g) => g.letter)));

// Check if search has no results for create button
const noResults = $derived(searchQuery.trim() && filteredPayees.length === 0);
...
$effect(() => {
  if (scrollContainer && selectedId !== null && alphabetGroups.length > 0) {
    ...
    requestAnimationFrame(() => {
      const selectedElement = scrollContainer?.querySelector(`[data-payee-id="${selectedId}"]`);
      if (selectedElement) {
  ...
  <div class="relative flex flex-1 overflow-hidden">
    ...
    {#if alphabetGroups.length > 0 && !searchQuery.trim()}
      <div
        ...
        class="flex w-6 shrink-0 touch-none flex-col items-center justify-center py-2 select-none"
      ...
      {#if activeLetter}
        <div
          class="bg-primary text-primary-foreground pointer-events-none absolute top-1/2 left-10 z-20 flex h-14 w-14 -translate-y-1/2 items-center justify-center rounded-lg text-2xl font-bold shadow-lg">
          {activeLetter}
        </div>
      {/if}
    {/if}
    ...
    <!-- Scrollable list -->
    <div bind:this={scrollContainer} class="flex-1 space-y-6 overflow-auto p-4">
      ...
      {#if noResults}
        ...
        <div class="py-8 text-center">
          ...
          {#if allowCreate}
            <Button variant="outline" class="mt-4" onclick={handleCreateNew}>
