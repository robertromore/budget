...
const categories = $derived(
  categoriesState ? (categoriesState.getActiveCategories() as EditableEntityItem[]) : []
);
...
async function handleSubmit(e: Event) {
  ...
  try {
    ...
    error = err instanceof Error ? err.message : 'Failed to save payee';
  } finally {
    isSaving = false;
  }
}
...
<form onsubmit={handleSubmit} class="flex h-full flex-col">
  <div class="flex-1 space-y-4 overflow-auto p-4">
    ...
    <div class="space-y-2">
      ...
      <Input id="payee-name" bind:value={name} placeholder="Enter payee name" required autofocus />
    </div>
    ...
    <div class="space-y-2">
      ...
      <p class="text-muted-foreground text-xs">Auto-applied to new transactions with this payee</p>
    </div>
  ...
  <div class="flex gap-2 border-t p-4">
    <Button type="button" variant="outline" onclick={onCancel} disabled={isSaving} class="flex-1">
      Cancel
    </Button>
