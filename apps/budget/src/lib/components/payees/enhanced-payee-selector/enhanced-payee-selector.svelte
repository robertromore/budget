...
import type { DisplayMode, EditMode, LayoutMode, QuickEditPayeeData } from './types';
...
const focusedPayee = $derived(
  focusedPayeeId ? (allPayees.find((p) => p.id === focusedPayeeId) ?? null) : null
);
...
const transferAccounts = $derived(accounts.filter((a) => a.id !== currentAccountId));
...
const showDetailPanel = $derived(focusedPayeeId !== null || editMode === 'create');
        ...
        <div class="flex h-[calc(100vh-14rem)] min-h-[300px] flex-col">
          ...
          <div class="border-b p-3">
            <Command.Root class="rounded-lg border" shouldFilter={false}>
              <Command.Input placeholder="Search accounts..." bind:value={transferSearchValue} />
            </Command.Root>
          </div>
