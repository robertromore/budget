...
const dayChartData = $derived.by(() => {
  ...
  const rotated = [
    ...baseDist.slice(7 - patterns.mostCommonDay),
    ...baseDist.slice(0, 7 - patterns.mostCommonDay),
  ];
...
<div class="space-y-6">
  {#if stats}
          ...
          <div>
              ...
              {#if visible}
                <LayerCake
                  ...
                  <Svg>
                    ...
                    <AxisX gridlines={false} tickMarks={false} format={(d) => d} />
                  </Svg>
                </LayerCake>
              {/if}
            </div>
            {#if patterns.mostCommonDay !== null}
              <p class="text-muted-foreground mt-1 text-center text-xs">
                Most common: <span class="text-foreground font-medium"
                  >{dayNames[patterns.mostCommonDay]}</span>
              </p>
            {/if}
          </div>
        {/if}
    ...
    {/if}
  {:else}
    <p class="text-muted-foreground py-8 text-center text-sm">No spending data available yet.</p>
  {/if}
</div>
