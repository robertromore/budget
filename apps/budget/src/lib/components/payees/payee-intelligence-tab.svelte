...
let { payeeId, onApplyCategory, onApplyBudget, onDetectSubscription }: Props = $props();
...
const profileQuery = $derived(payeeId ? getIntelligenceProfile(payeeId).options() : null);
...
function handleExplainInsights() {
  ...
  const seasonalStr =
    seasonalTrends.length > 0
      ? '- Seasonal patterns: ' +
        seasonalTrends
          .map(
            (t: { month: number; avgAmount: number }) =>
              monthNames[t.month - 1] + ': ' + formatCurrency(t.avgAmount)
          )
          .join(', ')
      : '';
  ...
  const lines = [
    isIncome
      ? 'Analyze my income patterns for this payee:'
    ...
    '- ' +
      ...
      ': ' +
      formatCurrency(Math.abs(data.stats.totalAmount)),
    '- Average transaction: ' + formatCurrency(data.stats.avgAmount),
    '- Range: ' +
      formatCurrency(data.stats.minAmount) +
      ' - ' +
      formatCurrency(data.stats.maxAmount),
    '- Monthly average: ' + formatCurrency(data.stats.monthlyAverage),
    '',
    '**Predictions:**',
    data.nextTransaction
      ? '- Next transaction: ' +
        data.nextTransaction.date +
        ' for ' +
        formatCurrency(data.nextTransaction.amount) +
        ' (' +
        Math.round(data.nextTransaction.confidence * 100) +
        '% confidence)'
      : '- No transaction prediction available',
    data.budgetSuggestion
      ? '- Suggested budget: ' +
        formatCurrency(data.budgetSuggestion.totalSuggested) +
        '/month (' +
        Math.round(data.budgetSuggestion.confidence * 100) +
        '% confidence)'
      : '',
    '',
    '**Payment Patterns:**',
...
const hasData = $derived(!data.isLoading && (data.intelligence !== null || data.stats !== null));
...
const showSuggestionBanner = $derived(
  !hasActiveProfile && suggestions !== null && suggestions.reasoning.length > 0 && hasData
);
            ...
            {#snippet child({ props })}
              <Button
                ...
                {#if data.alerts.length > 0}
                  <span
                    class="bg-destructive text-destructive-foreground absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full text-[10px] font-medium">
                    {data.alerts.length}
                  </span>
                {/if}
              </Button>
        ...
        <div class="flex items-start justify-between gap-4">
          ...
          <div class="flex items-center gap-2">
            <Popover.Root>
              <Popover.Trigger>
                {#snippet child({ props })}
                  <Button {...props} variant="ghost" size="sm">Why?</Button>
                {/snippet}
              </Popover.Trigger>
        ...
        {:else if data.stats}
          <div class="grid grid-cols-2 gap-4 md:grid-cols-3">
            <div class="text-center">
              <div
                class="text-muted-foreground mb-1 flex items-center justify-center gap-1 text-xs">
    ...
    {#if !data.isLoading}
      <div class="flex flex-wrap items-start gap-3">
        ...
        {#if data.suggestions?.suggestedCategoryId}
          <div class="bg-muted/30 flex items-center gap-3 rounded-md border px-2.5 py-1.5 text-sm">
            ...
            <Button
              ...
              onclick={() => onApplyCategory?.(data.suggestions!.suggestedCategoryId!)}>
              Apply
            </Button>
          </div>
        {/if}
