...
<div
  class="group hover:bg-muted/50 flex w-full cursor-pointer items-start gap-3 rounded-lg p-3 text-left transition-colors {notification.read
    ? 'opacity-60'
    : ''}"
  onclick={handleClick}>
  <Icon
  ...
  <button
    type="button"
    class="hover:bg-muted rounded p-1 opacity-0 transition-opacity group-hover:opacity-100"
    onclick={(e) => {
      e.stopPropagation();
