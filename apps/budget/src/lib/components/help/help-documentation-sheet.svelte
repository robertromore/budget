...
// Search results
const searchResults = $derived(searchQuery.trim() ? searchHelp(searchQuery, helpContent) : []);
  ...
  {#snippet header()}
    {#if isSearchMode}
      ...
      <div class="flex items-center gap-2">
        <Button variant="ghost" size="icon" class="h-8 w-8 shrink-0" onclick={exitSearchMode}>
  ...
  {#snippet footer()}
    {#if isSearchMode}
      ...
      <div class="text-muted-foreground flex items-center justify-center gap-4 text-sm">
        <span>
          <kbd class="bg-muted rounded border px-1.5 py-0.5 font-mono text-xs">↑</kbd>
          <kbd class="bg-muted rounded border px-1.5 py-0.5 font-mono text-xs">↓</kbd>
          to navigate
        </span>
        <span>
          <kbd class="bg-muted rounded border px-1.5 py-0.5 font-mono text-xs">Enter</kbd>
          to select
        </span>
        <span>
          <kbd class="bg-muted rounded border px-1.5 py-0.5 font-mono text-xs">Esc</kbd>
          to cancel
        </span>
      </div>
    {:else}
      ...
      <div class="flex items-center justify-between">
        ...
        <span class="text-muted-foreground text-sm">
          Press <kbd class="bg-muted rounded px-1.5 py-0.5 font-mono text-xs">Esc</kbd> to close
        </span>
