...
async function analyzePatterns() {
  ...
  try {
    const result =
      (await trpc().recurringRoutes?.detect?.query?.({
        accountIds,
        months: 6,
        minTransactions: 3,
        minConfidence: 50,
...
async function handleConfirm(pattern: RecurringPattern) {
  ...
  try {
    if (action === 'subscription' || action === 'schedule') {
      ...
      toast.success(
        `${action === 'subscription' ? 'Subscription' : 'Schedule'} created for ${pattern.payeeName}`
      );
...
function getPatternTypeBadge(type: RecurringPattern['patternType']): {
...
<Sheet.Root {open} {onOpenChange}>
  <Sheet.Content class="sm:max-w-xl">
    <Sheet.Header>
      ...
      <Sheet.Description>
        We found {patterns.length} recurring payment pattern{patterns.length !== 1 ? 's' : ''} in your
        transactions. Choose how to track each one.
      </Sheet.Description>
    </Sheet.Header>
    ...
    <div class="py-4">
      {#if isAnalyzing}
        <div class="flex flex-col items-center justify-center py-12">
          ...
          <p class="text-muted-foreground mt-4 text-sm">Analyzing transactions...</p>
        </div>
                  ...
                  {#if !isProcessed}
                    ...
                    <RadioGroup.Root
                      ...
                      <div class="flex items-center space-x-2">
                        ...
                        <label for={`${key}-sub`} class="cursor-pointer text-sm">
                          Track as Subscription
                        </label>
                      </div>
                      <div class="flex items-center space-x-2">
                        <RadioGroup.Item value="schedule" id={`${key}-sched`} />
                        <label for={`${key}-sched`} class="cursor-pointer text-sm">
                          Create Schedule
                        </label>
                      </div>
                      <div class="flex items-center space-x-2">
                        <RadioGroup.Item value="budget" id={`${key}-budget`} />
                        <label for={`${key}-budget`} class="cursor-pointer text-sm">
                          Add to Budget
                        </label>
                      </div>
                      <div class="flex items-center space-x-2">
                        <RadioGroup.Item value="ignore" id={`${key}-ignore`} />
                        <label for={`${key}-ignore`} class="cursor-pointer text-sm"> Ignore </label>
                      </div>
                    </RadioGroup.Root>
                  {/if}
                </Card.Content>
