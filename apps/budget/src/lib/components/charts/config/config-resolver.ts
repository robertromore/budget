...
function resolveColors(
  ...
  if (Array.isArray(colorConfig)) {
    return colorConfig.map((color, index) => {
      ...
      switch (color) {
        case "primary":
          return colorUtils.getChartColor(0); // Blue
        case "secondary":
          return colorUtils.getChartColor(4); // Orange
        case "success":
          return colorUtils.getChartColor(1); // Green
        case "destructive":
          return colorUtils.getChartColor(2); // Red
        case "warning":
          return colorUtils.getChartColor(3); // Yellow
        case "muted":
          return colorUtils.getChartColor(7); // Gray/Pink
        default:
...
function inferChartType(data: ChartDataPoint[]): ChartType {
  ...
  const hasDateX = data.some(
    (d) => d.x instanceof Date || (typeof d.x === "string" && !isNaN(Date.parse(d.x)))
  );
  if (hasDateX) {
    return "line";
  }
 ...
 */
export function resolveChartConfig(props: UnifiedChartProps): ResolvedChartConfig & {
  type: ChartType;
  data: ChartDataPoint[];
...
export function validateChartData(
  ...
  const xRange: [any, any] =
    xValues.length > 0
      ? [
          Math.min(...xValues.filter((x) => typeof x === "number")),
          Math.max(...xValues.filter((x) => typeof x === "number")),
        ]
      : [null, null];
