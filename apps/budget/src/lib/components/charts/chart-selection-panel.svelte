...
const analyzeActions: ActionItem[] = [
  ...
  },
  {
    ...
    icon: Calculator,
    description: 'Average, median, std dev',
  },
  ...
  },
  {
    ...
    icon: Calendar,
    description: 'Compare same months',
  },
];
...
const actionActions: ActionItem[] = [
  {
    id: 'set-budget',
    label: 'Set as Budget Target',
    icon: Target,
    description: 'Use for budget planning',
  },
...
{#if chartSelection.isActive}
  <div
    class="animate-in fade-in-0 slide-in-from-bottom-4 fixed bottom-4 left-1/2 z-50 -translate-x-1/2 duration-200"
    role="toolbar"
    aria-label="Chart selection actions">
    <div
      class="bg-background/95 supports-backdrop-filter:bg-background/80 flex items-center gap-1.5 rounded-lg border px-3 py-2 shadow-lg backdrop-blur">
