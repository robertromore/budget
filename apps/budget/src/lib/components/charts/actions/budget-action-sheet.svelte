...
const selectedBudget = $derived(budgetsQuery?.data?.find((b) => b.id === selectedBudgetId) ?? null);
...
async function handleSubmit() {
  ...
  try {
    ...
    const newMetadata = {
      ...
      allocatedAmount: roundToCents(suggestedAmount),
    };
...
<ResponsiveSheet bind:open {onOpenChange} defaultWidth={500}>
  {#snippet header()}
    ...
    <Sheet.Description>Use your selected spending data to set a budget target</Sheet.Description>
  {/snippet}
  ...
  {#snippet content()}
    <div class="space-y-6">
      ...
      <div class="bg-muted/50 rounded-lg border p-4">
        <div class="mb-2 flex items-center gap-2">
          ...
          <span class="text-sm font-medium"
            >Based on {stats.count} selected month{stats.count !== 1 ? 's' : ''}</span>
        </div>
        <div class="grid grid-cols-2 gap-3 text-sm">
          ...
          <div>
            ...
            <span class="text-destructive ml-1 font-semibold"
              >{currencyFormatter.format(stats.highest)}</span>
          </div>
          <div>
      ...
      <div class="space-y-3">
        ...
        {#if calculationMethod === 'custom'}
          <div class="ml-6">
            <NumericInput bind:value={customAmount} buttonClass="w-40" />
          </div>
        {/if}
      </div>
      ...
      <div class="border-primary/20 bg-primary/5 rounded-lg border-2 p-4 text-center">
        <p class="text-muted-foreground text-sm">Suggested Budget Target</p>
        <p class="text-primary text-3xl font-bold">{currencyFormatter.format(suggestedAmount)}</p>
      ...
      <div class="space-y-3">
        ...
        {:else if budgetOptions.length === 0}
          <p class="text-muted-foreground text-sm">No active budgets found</p>
        {:else}
          <Select.Root
            type="single"
            onValueChange={(v) => {
            ...
            <Select.Content>
              {#each budgetOptions as budget}
                <Select.Item value={String(budget.value)}>
                  {budget.label}
                  <span class="text-muted-foreground ml-2 text-xs">({budget.type})</span>
                </Select.Item>
              {/each}
            </Select.Content>
          </Select.Root>
  ...
  {#snippet footer()}
    <div class="flex justify-between gap-2">
      ...
      <Button onclick={handleSubmit} disabled={!selectedBudget || isSubmitting}>
