...
<ResponsiveSheet
  ...
  {#snippet header()}
    ...
    <Sheet.Description>
      {filteredTransactions.length} transactions across {chartSelection.count} selected month{chartSelection.count ===
      1
        ? ''
        : 's'}
    </Sheet.Description>
  ...
  {#snippet content()}
    {#if transactionsQuery?.isLoading}
      <div class="flex h-40 items-center justify-center">
        <p class="text-muted-foreground text-sm">Loading transactions...</p>
      </div>
    {:else if filteredTransactions.length === 0}
      <div class="flex h-40 items-center justify-center">
        <p class="text-muted-foreground text-sm">No transactions found in selected months</p>
      </div>
    {:else}
      <div class="space-y-6">
        <!-- Summary Stats -->
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <div class="bg-muted/50 rounded-lg p-3">
            <p class="text-muted-foreground text-xs">Avg Transaction</p>
