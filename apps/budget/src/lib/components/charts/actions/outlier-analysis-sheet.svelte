...
const outlierAnalysis = $derived.by(() => {
  ...
  const iqrOutliers = sortedPoints.filter((p) => p.value < lowerFence || p.value > upperFence);
      ...
      <div class="space-y-6">
        ...
        <Card.Root>
          ...
          <Card.Content>
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
              <div>
                <p class="text-muted-foreground text-xs">Total Outliers</p>
              ...
              <div>
                <p class="text-muted-foreground text-xs">Outlier Rate</p>
                <p class="text-2xl font-bold tabular-nums">
                  {formatPercentRaw(outlierAnalysis.outlierPercentage, 0)}
                </p>
              </div>
              <div>
                <p class="text-muted-foreground text-xs">Mean</p>
                <p class="text-lg font-semibold tabular-nums">
                  {currencyFormatter.format(mean)}
                </p>
              </div>
              <div>
                <p class="text-muted-foreground text-xs">Std Dev (σ)</p>
                <p class="text-lg font-semibold tabular-nums">
          ...
          <Card.Content>
            <div class="space-y-3">
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                  <Progress
                    value={(outlierAnalysis.extreme.length / chartSelection.count) * 100}
                    class="h-2 w-24" />
                  <span class="w-8 text-right text-sm font-medium tabular-nums"
                    >{outlierAnalysis.extreme.length}</span>
                </div>
              </div>
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                ...
                <div class="flex items-center gap-2">
                  <Progress
                    value={(outlierAnalysis.significant.length / chartSelection.count) * 100}
                    class="h-2 w-24" />
                  <span class="w-8 text-right text-sm font-medium tabular-nums"
                    >{outlierAnalysis.significant.length}</span>
                </div>
              </div>
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                ...
                <div class="flex items-center gap-2">
                  <Progress
                    value={(outlierAnalysis.moderate.length / chartSelection.count) * 100}
                    class="h-2 w-24" />
                  <span class="w-8 text-right text-sm font-medium tabular-nums"
                    >{outlierAnalysis.moderate.length}</span>
                </div>
              </div>
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                ...
                <div class="flex items-center gap-2">
                  <Progress
                    value={(outlierAnalysis.normal.length / chartSelection.count) * 100}
                    class="h-2 w-24" />
                  <span class="w-8 text-right text-sm font-medium tabular-nums"
                    >{outlierAnalysis.normal.length}</span>
                </div>
              </div>
            </div>
          </Card.Content>
        ...
        <Card.Root>
          <Card.Header class="pb-2">
            ...
            <Card.Description
              >Alternative outlier detection using interquartile range</Card.Description>
          </Card.Header>
          <Card.Content>
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
              <div>
                <p class="text-muted-foreground text-xs">Q1 (25th)</p>
                <p class="font-semibold tabular-nums">
                  {currencyFormatter.format(outlierAnalysis.q1)}
                </p>
              </div>
              <div>
                <p class="text-muted-foreground text-xs">Q3 (75th)</p>
                <p class="font-semibold tabular-nums">
                  {currencyFormatter.format(outlierAnalysis.q3)}
                </p>
              </div>
              <div>
                <p class="text-muted-foreground text-xs">IQR</p>
                <p class="font-semibold tabular-nums">
                  {currencyFormatter.format(outlierAnalysis.iqr)}
                </p>
              </div>
              <div>
                <p class="text-muted-foreground text-xs">IQR Outliers</p>
                <p class="font-semibold tabular-nums">{outlierAnalysis.iqrOutliers.length}</p>
              </div>
            </div>
            <div class="bg-muted/50 mt-4 rounded-md p-3 text-sm">
              <p class="text-muted-foreground">
                Normal range: {currencyFormatter.format(outlierAnalysis.lowerFence)} to {currencyFormatter.format(
                  outlierAnalysis.upperFence
                )}
              </p>
            </div>
          </Card.Content>
        ...
        {#if outlierAnalysis.consecutivePatterns.length > 0}
          <Card.Root>
            <Card.Header class="pb-2">
              ...
              <Card.Description>
                Runs of consecutive high or low values may indicate trends rather than random
                outliers
              </Card.Description>
            </Card.Header>
            <Card.Content>
          ...
          <Card.Content>
            <div class="max-h-64 space-y-1 overflow-y-auto">
              {#each outlierAnalysis.pointsWithStats.sort((a, b) => Math.abs(b.zScore) - Math.abs(a.zScore)) as point}
                <div
                  class="hover:bg-muted/50 flex items-center justify-between rounded px-2 py-1.5 text-sm">
                  <div class="flex items-center gap-2">
                    <div
