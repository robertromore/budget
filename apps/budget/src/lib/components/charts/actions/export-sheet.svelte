...
<ResponsiveSheet bind:open {onOpenChange} defaultWidth={600}>
  ...
  {#snippet content()}
    <div class="space-y-6">
      ...
      <div class="space-y-4">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-muted-foreground text-xs">Add column headers to the CSV</p>
          </div>
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-muted-foreground text-xs">Add summary statistics at the end</p>
          </div>
      ...
      <div class="space-y-2">
        <Label>Preview</Label>
        <Textarea value={exportContent} readonly class="h-64 font-mono text-xs" />
      </div>

      <!-- Summary -->
      <div class="rounded-lg border p-3">
        <h4 class="text-sm font-medium">Export Summary</h4>
        <div class="text-muted-foreground mt-2 grid grid-cols-2 gap-2 text-sm">
          <div>Data Points: {chartSelection.count}</div>
          <div>Total: {currencyFormatter.format(chartSelection.totalValue)}</div>
          <div>
            Date Range: {chartSelection.sortedByDate[0]?.label} - {chartSelection.sortedByDate[
              chartSelection.count - 1
            ]?.label}
          </div>
        </div>
      </div>
