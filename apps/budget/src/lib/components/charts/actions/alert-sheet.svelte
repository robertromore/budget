...
<ResponsiveSheet bind:open {onOpenChange} defaultWidth={500}>
  {#snippet header()}
    ...
    <Sheet.Description>Get notified when spending crosses your threshold</Sheet.Description>
  {/snippet}
  ...
  {#snippet content()}
    <div class="space-y-6">
      ...
      <div class="space-y-2">
        ...
        <Input id="alert-name" bind:value={alertName} placeholder={defaultName} />
        <p class="text-muted-foreground text-xs">Leave blank to auto-generate</p>
      </div>
      ...
      <div class="border-primary/20 bg-primary/5 rounded-lg border-2 p-4 text-center">
        <p class="text-muted-foreground text-sm">Alert Threshold</p>
        <p class="text-primary text-3xl font-bold">{currencyFormatter.format(threshold)}</p>
