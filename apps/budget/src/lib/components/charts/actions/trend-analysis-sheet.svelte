...
const trendAnalysis = $derived.by(() => {
  ...
  const sortedByResidual = [...residuals].sort(
  ...
  if (n >= 4) {
    ...
    const earlySlope = (yValues[q1End] - yValues[0]) / q1End || 0;
    const lateSlope = (yValues[n - 1] - yValues[q3Start]) / (n - 1 - q3Start) || 0;
      ...
      <div class="space-y-6">
        ...
        <Card.Root>
          ...
          <Card.Content>
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
              <div>
                <p class="text-muted-foreground text-xs">Direction</p>
              ...
              <div>
                <p class="text-muted-foreground text-xs">Monthly Change</p>
                <p
              ...
              <div>
                <p class="text-muted-foreground text-xs">% Change</p>
                <p
                  class="text-lg font-semibold tabular-nums"
        ...
        <Card.Root>
          ...
          <Card.Content>
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-3">
              <div>
                <p class="text-muted-foreground text-xs">Trend Strength (R²)</p>
                <p class="text-lg font-semibold tabular-nums">
                  {formatPercentRaw(trendAnalysis.rSquared * 100, 1)}
                </p>
                <p class="text-xs {getTrendStrength(trendAnalysis.rSquared).color}">
                  {getTrendStrength(trendAnalysis.rSquared).label}
                </p>
              </div>
              <div>
                <p class="text-muted-foreground text-xs">Volatility (CV)</p>
                <p class="text-lg font-semibold tabular-nums">
                  {formatPercentRaw(trendAnalysis.volatility, 1)}
                </p>
                <p class="text-muted-foreground text-xs">
                  {trendAnalysis.volatility < 20
                    ? 'Low'
                    : trendAnalysis.volatility < 40
                      ? 'Moderate'
                      : 'High'}
                </p>
              </div>
              <div>
                <p class="text-muted-foreground text-xs">Momentum</p>
                <p
                  class="text-lg font-semibold tabular-nums"
                ...
                <p class="text-muted-foreground text-xs">Recent vs Earlier</p>
              </div>
            </div>
          </Card.Content>
        </Card.Root>
              ...
              {#if trendAnalysis.largestIncrease && trendAnalysis.largestIncrease.change > 0}
                <div
                  ...
                  <div class="text-right">
                    <p class="text-destructive font-semibold tabular-nums">
        ...
        <Card.Root>
          <Card.Header class="pb-2">
            ...
            <Card.Description
              >Periods that differed most from the expected trend line</Card.Description>
          </Card.Header>
          <Card.Content>
            <div class="space-y-2">
          ...
          <Card.Content>
            <div class="max-h-48 space-y-1 overflow-y-auto">
              {#each trendAnalysis.periodChanges as change}
                <div
                  class="hover:bg-muted/50 flex items-center justify-between rounded px-2 py-1 text-sm">
