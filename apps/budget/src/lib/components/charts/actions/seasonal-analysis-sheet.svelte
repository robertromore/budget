...
const seasonalAnalysis = $derived.by(() => {
  ...
  const yearOverYear = new Map<
    number,
    { years: number[]; values: number[]; change: number | null }
  >();
  sortedPoints.forEach((p) => {
    const month = p.date.getMonth();
  ...
  const seasonalVariance =
    monthAverages.reduce((sum, v) => sum + Math.pow(v - overallAvg, 2), 0) / monthAverages.length ||
    0;
  const seasonalityStrength =
    overallAvg !== 0 ? (Math.sqrt(seasonalVariance) / overallAvg) * 100 : 0;
        ...
        <Card.Root>
          ...
          <Card.Content>
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
              <div>
                <p class="text-muted-foreground text-xs">Seasonality Strength</p>
                <p class="text-lg font-semibold tabular-nums">
                  {formatPercentRaw(seasonalAnalysis.seasonalityStrength, 0)}
                </p>
                <p
                  class="text-xs {getSeasonalityLabel(seasonalAnalysis.seasonalityStrength).color}">
                  {getSeasonalityLabel(seasonalAnalysis.seasonalityStrength).label}
                </p>
              </div>
              <div>
                <p class="text-muted-foreground text-xs">Months with Data</p>
                <p class="text-lg font-semibold tabular-nums">
                  {seasonalAnalysis.monthsWithData} of 12
                </p>
              </div>
              <div>
                <p class="text-muted-foreground text-xs">Highest Month</p>
        ...
        {#if seasonalAnalysis.yoyChanges.length > 0}
          <Card.Root>
            <Card.Header class="pb-2">
              ...
              <Card.Description>Comparing same months across different years</Card.Description>
            </Card.Header>
            <Card.Content>
              <div class="space-y-2">
                {#each seasonalAnalysis.yoyChanges as yoy}
                  <div class="flex items-center justify-between rounded-md border p-3">
                    <div>
                      ...
                      <p class="text-muted-foreground mt-1 text-xs">
                        {yoy.firstYear}: {currencyFormatter.format(yoy.firstValue)} → {yoy.lastYear}:
                        {currencyFormatter.format(yoy.lastValue)}
                      </p>
                    </div>
                  ...
                  {#each seasonalAnalysis.monthStats as month}
                    <tr class="border-b last:border-0">
                      ...
                      <td class="text-muted-foreground py-2 text-right tabular-nums"
                        >{currencyFormatter.format(month.range)}</td>
