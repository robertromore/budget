...
<WizardFormWrapper
  ...
  subtitle={isUpdate
    ...
    : 'Add a new recurring or one-time transaction schedule'}
  ...
  {#snippet formContent()}
    <form
      id={uniqueFormId}
      method="post"
      action="/schedules?/save-schedule"
      use:enhance
      class="space-y-6">
            ...
            <Form.Control>
              {#snippet children({ props })}
                <RepeatingDateInput
                  ...
                  bind:isRepeating={$formData.recurring} />
          ...
          {#if $formData.recurring}
            <div class="bg-muted/50 flex items-center justify-between rounded-lg border p-4">
              <div class="space-y-0.5">
                <Label for="schedule-auto-add" class="text-sm font-medium"
                  >Automatically add transactions</Label>
                <div class="text-muted-foreground text-xs">
                  When enabled, upcoming transactions will appear in account views for better
                  planning
                </div>
              </div>
