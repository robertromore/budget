...
<Card.Root class={cn('', className)}>
  <Card.Header class="pb-3">
    ...
    <Card.Description>Search transactions using everyday language</Card.Description>
  </Card.Header>
      ...
      <div class="space-y-2">
        ...
        <div class="flex flex-wrap gap-2">
          {#each examples.slice(0, 6) as example (example.query)}
            <button
              type="button"
              class="bg-muted/50 hover:bg-muted rounded-full border px-3 py-1 text-xs transition-colors"
    ...
    {:else if parsedQuery && !showExamples}
      ...
      {:else if searchQuery && !isLoading}
        ...
        <div class="py-6 text-center">
          ...
          <p class="text-muted-foreground text-xs">Try adjusting your search query</p>
        </div>
      {/if}
    {/if}
  </Card.Content>
