...
const config = $derived(riskConfig[riskLevel as RiskKey] ?? riskConfig.unknown);
  ...
  <Card.Content class="relative space-y-3">
    ...
    <div class="flex items-center gap-2 pt-1">
      {#if onViewTransaction}
        <Button variant="outline" size="sm" onclick={onViewTransaction}>View Transaction</Button>
      {/if}
      {#if onDismiss}
