...
<button type="button" class="fixed inset-0 z-110" onclick={onClose} aria-label="Close mode picker"
></button>

<!-- Mode picker popup -->
<div
  class="bg-popover fixed z-120 w-48 rounded-lg border p-2 shadow-lg"
  style="left: {adjustedPosition().x}px; top: {adjustedPosition().y}px;"
  role="menu"
  aria-label="Select intelligence mode for {title}">
  <div class="mb-2 flex items-center justify-between border-b pb-2">
    <span class="text-muted-foreground text-xs font-medium">Intelligence Mode</span>
    <button
      type="button"
  ...
  <div class="space-y-1">
    {#if availableModes.includes('ml')}
      <button
        ...
        {:else}
          <kbd class="bg-muted text-muted-foreground rounded px-1 text-xs">M</kbd>
        {/if}
      </button>
    {/if}
    ...
    {#if availableModes.includes('llm')}
      <button
        ...
        {#if currentMode === 'llm'}
          <Check class="h-4 w-4 text-violet-600" />
        {:else}
          <kbd class="bg-muted text-muted-foreground rounded px-1 text-xs">L</kbd>
        {/if}
      </button>
    {/if}
  </div>
  ...
  <div class="mt-2 border-t pt-2">
    <p class="text-muted-foreground text-xs">
      Click or press <kbd class="bg-muted rounded px-1">Enter</kbd> to enhance with selected mode
    </p>
  </div>
</div>
