...
{#if rect}
  <button
    ...
    class={cn(
      ...
      zIndex,
      isProcessing
        ? 'animate-processing-pulse cursor-wait border-violet-600 bg-violet-500/20'
        : 'cursor-pointer',
      !isProcessing && isHighlighted
        ...
        : !isProcessing &&
            'border-violet-400/60 bg-violet-500/5 hover:border-violet-500 hover:bg-violet-500/10'
    )}
    style="
      top: {rect.top - 4}px;
      left: {rect.left - 4}px;
    ...
    {#if isHighlighted || isProcessing}
      <span
        class={cn(
          'absolute flex items-center gap-1.5 rounded px-2 py-1 text-xs font-medium whitespace-nowrap shadow-lg',
          isProcessing ? 'bg-violet-700' : 'bg-violet-600',
          'text-white',
          isModal ? 'z-115' : 'z-110',
          showLabelRight ? 'top-1/2 left-full ml-2 -translate-y-1/2' : 'left-0',
...
@keyframes processing-pulse {
  ...
  100% {
    box-shadow:
      0 0 0 3px hsl(270, 80%, 55%, 0.5),
      0 0 25px hsl(270, 80%, 55%, 0.4);
    border-color: hsl(270, 80%, 55%);
  }
  50% {
    box-shadow:
      0 0 0 5px hsl(270, 90%, 50%, 0.7),
      0 0 35px hsl(270, 90%, 50%, 0.6);
