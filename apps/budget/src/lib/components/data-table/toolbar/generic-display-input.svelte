...
const visibleColumns = $derived(
  table.getAllColumns().filter((column) => {
    ...
    return (
      !Object.keys(columnVisibility).includes(column.id) || columnVisibility[column.id] === true
    );
  })
);

const hidableColumns = $derived(table.getAllColumns().filter((column) => column.getCanHide()));
...
// Computed visibility value for multi-select
const visibilityIds = $derived(visibleColumns.map((col) => col.id));
...
function toggleSort(columnId: string) {
  ...
  if (existingIndex === -1) {
    ...
    if (existing && !existing.desc) {
      ...
      newSorting[existingIndex] = { id: existing.id, desc: true };
    } else {
      // Remove sort
...
// Check display preference for sheet vs popover
...
{#if useSheet}
  <ResponsiveSheet
    triggerClass={cn(buttonVariants({ variant: 'outline' }), 'h-8')}
    defaultWidth={400}
    minWidth={320}
    maxWidth={600}>
    {#snippet trigger()}
    ...
    {/snippet}
    {#snippet header()}
      <h2 class="text-lg font-semibold">Display Options</h2>
    {/snippet}
    {#snippet content()}
