...
import type { LLMProvider } from '$core/schema/workspaces';
    ...
    <Card.Content class="space-y-4">
      ...
      {#if showEndpoint}
        <div class="space-y-2">
          ...
          <p class="text-muted-foreground text-xs">Local Ollama server endpoint</p>
        </div>
      {/if}
            ...
            {#each models as modelOption}
              <label
                ...
                <RadioGroup.Item
                  ...
                  id={`${provider}-model-${modelOption.id}`}
                  class="mt-0.5" />
        ...
        {#if provider === 'ollama'}
          ...
          {#if selectedModel?.installed === false}
            <p class="text-muted-foreground text-xs">
              To install this model, run: <code
