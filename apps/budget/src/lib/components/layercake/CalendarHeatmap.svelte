...
const totalWeeks = $derived.by(() => {
  return (
    Math.ceil((dateRange.end.getTime() - dateRange.start.getTime()) / (7 * 24 * 60 * 60 * 1000)) + 1
  );
});
...
const layout = $derived.by(() => {
  ...
  const maxWeeksAtMinSize = Math.floor((availableWidth + cellGap) / (minCellSize + cellGap));
  ...
  for (let rowIndex = 0; rowIndex < rowCount; rowIndex++) {
    ...
    while (weekInRow < weeksPerRow && currentDate <= dateRange.end) {
      ...
      for (let dayOfWeek = currentDate.getDay(); dayOfWeek < 7; dayOfWeek++) {
        ...
        if (
          month !== globalLastMonth &&
          currentDate >= dateRange.start &&
          labelX - lastLabelX >= 40
        ) {
    ...
    <g transform="translate(0, {row.yOffset})">
      ...
      {#if showMonthLabels}
        {#each row.monthLabels as { x, label }}
          <text {x} y={14} font-size="11" fill="var(--muted-foreground)">
            {label}
          </text>
        {/each}
      {/if}
      ...
      {#each row.cells as cell}
        ...
        <rect
          ...
          class="calendar-cell"
          style="cursor: {hasValue || onclick
            ? 'pointer'
            : 'default'}; transition: fill-opacity 0.15s, stroke 0.15s;"
