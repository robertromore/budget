...
interface Props {
  ...
  onhover?: (
    info: { item: any; key: string; segmentIndex: number; seriesIndex: number } | null
  ) => void;
  /** Click callback */
  onclick?: (
    info: { item: any; key: string; segmentIndex: number; seriesIndex: number },
    event: MouseEvent
  ) => void;
...
{#each stackedData as series, seriesIndex}
  ...
  {#each series as segment, segmentIndex}
    ...
    {@const isStackHovered =
      hoveredBar?.segmentIndex === segmentIndex || externalHoveredIndex === segmentIndex}
    {@const item = sortedData[segmentIndex]}
    {@const stackIsSelected = isSelected?.(item) ?? false}
    {@const effectiveOpacity = isStackHovered
      ? hoverOpacity
      : hasSelection && !stackIsSelected
        ? unselectedOpacity
        : opacity}
