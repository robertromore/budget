...
async function testImportSystem() {
  ...
  try {
    ...
    console.log(`   Matched: "${match.payee?.name}" (confidence: ${match.confidence}, score: ${match.score.toFixed(2)})`);
    ...
    console.log(`   Matched: "${categoryMatch.category?.name}" (confidence: ${categoryMatch.confidence}, score: ${categoryMatch.score.toFixed(2)})`);
