...
async function fetchAccounts(workspaceId: number): Promise<AccountSummary[]> {
  ...
  for (const account of accountList) {
    ...
    const balance = isDebt ? -initialBalance + transactionSum : initialBalance + transactionSum;
...
async function fetchTopCategories(
  ...
  const totalExpenses = categorySpending.reduce((sum, cat) => sum + Number(cat.totalAmount), 0);
  ...
  return categorySpending
    ...
    .map((cat) => ({
      ...
      percentOfTotal:
        totalExpenses > 0 ? Math.round((Number(cat.totalAmount) / totalExpenses) * 100) : 0,
    }));
}
 ...
 */
async function fetchTopPayees(workspaceId: number, limit: number = 10): Promise<PayeeSpending[]> {
  const { startDate, endDate } = getLast30DaysRange();
