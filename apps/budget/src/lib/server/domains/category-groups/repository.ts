...
> {
	 ...
	 */
	override async create(data: NewCategoryGroupRecommendation): Promise<CategoryGroupRecommendation> {
		const [recommendation] = await db
			.insert(categoryGroupRecommendations)
			.values(data)
			.returning();
	 ...
	 */
	async updateStatus(id: number, status: CategoryGroupRecommendationStatus): Promise<CategoryGroupRecommendation> {
		const [recommendation] = await db
	...
	async findPending(): Promise<(CategoryGroupRecommendation & { categoryName: string | null })[]> {
		const results = await db
			...
			.orderBy(desc(categoryGroupRecommendations.confidenceScore), categoryGroupRecommendations.createdAt);
	 ...
	 */
	async findByStatus(status: CategoryGroupRecommendationStatus): Promise<CategoryGroupRecommendation[]> {
		return await db
			.select()
			.from(categoryGroupRecommendations)
...
> {
	...
	async getSettings(): Promise<CategoryGroupSettings> {
		const [settings] = await db.select().from(categoryGroupSettings).where(eq(categoryGroupSettings.id, 1));
