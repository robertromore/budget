...
import type { CategoryGroupRecommendation, NewCategoryGroupRecommendation } from "$lib/schema/category-groups";
...
export class CategoryGroupRecommendationService {
	...
	private async getUngroupedCategories(): Promise<Category[]> {
		...
		const ungrouped = await db
			...
			.where(and(eq(categories.isActive, true), isNull(categoryGroupMemberships.id), isNull(categories.deletedAt)));
	...
	async generateRecommendations(): Promise<CategoryGroupRecommendation[]> {
		...
		const groupingRules = [
			...
			{pattern: /\b(rent|mortgage|hoa|property|landlord)\b/i, groupName: "Housing", confidence: 0.95},
			{
		...
		for (const rec of allRecommendations) {
			...
			if (createdDate < thirtyDaysAgo) {
				await db
					...
					.where(eq(require("$lib/schema/category-groups").categoryGroupRecommendations.id, rec.id));
			}
		}
	}
}
