...
export const insertAIConversationSchema = createInsertSchema(aiConversations, {
  title: (schema) =>
    schema.max(100, "Title must be less than 100 characters").optional().nullable(),
  summary: (schema) =>
    schema.max(500, "Summary must be less than 500 characters").optional().nullable(),
});
...
export const insertAIConversationMessageSchema = createInsertSchema(aiConversationMessages, {
  ...
  reasoning: (schema) =>
    schema.max(10000, "Reasoning must be less than 10000 characters").optional().nullable(),
});
