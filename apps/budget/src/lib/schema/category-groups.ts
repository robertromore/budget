...
export const categoryGroupMembershipsRelations = relations(
...
export const categoryGroupRecommendationStatusEnum = ["pending", "approved", "dismissed", "rejected"] as const;
export type CategoryGroupRecommendationStatus = (typeof categoryGroupRecommendationStatusEnum)[number];
...
export const categoryGroupRecommendations = sqliteTable(
	"category_group_recommendations",
	{
		...
		status: text("status", {enum: categoryGroupRecommendationStatusEnum}).notNull().default("pending"),
		createdAt: text("created_at")
			.notNull()
			.default(sql`(datetime('now'))`),
		updatedAt: text("updated_at")
...
export const formUpdateCategoryGroupSchema = formInsertCategoryGroupSchema
	.partial()
	.extend({
