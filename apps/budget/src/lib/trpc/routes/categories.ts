import {formInsertCategorySchema, removeCategoriesSchema, removeCategorySchema} from "$lib/schema";
...
export const categoriesRoutes = t.router({
  ...
  allWithBudgets: publicProcedure.query(
    withErrorHandler(async ({ctx}) => categoryService.getAllCategoriesWithBudgets(ctx.workspaceId))
  ),
