...
export const workspaceRoutes = t.router({
	...
	getCurrent: publicProcedure.query(async ({ctx}) => {
		const [workspace] = await ctx.db.select().from(workspaces).where(eq(workspaces.id, ctx.workspaceId)).limit(1);
	...
	list: publicProcedure.query(async ({ctx}) => {
		return await ctx.db.select().from(workspaces).where(isNull(workspaces.deletedAt)).orderBy(workspaces.displayName);
	}),
	...
	create: publicProcedure.input(formInsertWorkspaceSchema).mutation(async ({ctx, input}) => {
		...
		const existing = await ctx.db.select().from(workspaces).where(eq(workspaces.slug, input.slug)).limit(1);
	 ...
	 */
	delete: publicProcedure.input(z.object({workspaceId: z.number()})).mutation(async ({ctx, input}) => {
