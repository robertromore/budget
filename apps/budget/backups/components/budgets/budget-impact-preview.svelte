...
const budgetImpacts = $derived.by(() => {
  return allocations.map((allocation) => {
    ...
    const currentSpent = budget.transactions?.reduce((sum, t) =>
      sum + Math.abs(t.allocatedAmount), 0) || 0;
...
{#if budgetImpacts.length > 0}
  <div class="space-y-3 p-4 border border-border rounded-lg bg-muted/20">
    ...
    <div class="space-y-3">
      {#each budgetImpacts as impact}
        ...
        <div class="space-y-2 p-3 border rounded-md {statusClasses}">
          <!-- Budget Name and Status Icon -->
          <div class="flex items-start justify-between">
            <div class="flex items-center gap-2">
              <StatusIcon class="h-4 w-4 shrink-0" />
              <span class="font-medium text-sm">{impact.budget.name}</span>
            </div>
            {#if impact.status === 'over'}
              <span class="text-xs font-medium px-2 py-0.5 rounded bg-red-200 dark:bg-red-900 text-red-800 dark:text-red-200">
                Over Budget
              </span>
            {/if}
          </div>
            ...
            <div class="space-y-1">
              <div class="flex justify-between text-xs">
                ...
                {#if impact.remaining !== null}
                  <span class:text-red-600={impact.remaining < 0} class:dark:text-red-400={impact.remaining < 0}>
                    {impact.remaining >= 0 ? 'Remaining' : 'Over'}: {formatCurrency(Math.abs(impact.remaining))}
                  </span>
                {/if}
              </div>
              <Progress
                value={Math.min(impact.percentUsed, 100)}
                max={100}
                class="h-2"
              />
            </div>
          ...
          <!-- Impact Amount -->
          <div class="flex items-center gap-2 text-xs pt-1 border-t border-current/20">
            <span class="text-muted-foreground">Transaction Impact:</span>
            <span class="font-mono font-medium" class:text-green-600={impact.allocation.amount < 0} class:dark:text-green-400={impact.allocation.amount < 0}>
              {impact.allocation.amount < 0 ? '−' : '+'}{formatCurrency(Math.abs(impact.allocation.amount))}
            </span>
          </div>
        </div>
      {/each}
