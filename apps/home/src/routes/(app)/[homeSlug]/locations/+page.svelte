<script lang="ts">
  ...
  const treeQuery = createQuery(rpc.locations.getHomeLocationTree(data.home.id).options());
  const createMut = createMutation(rpc.locations.createHomeLocation.options());
