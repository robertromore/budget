<script lang="ts">
  ...
  const floorPlanQuery = createQuery(
    rpc.floorPlans.getFloorPlan(data.home.id, currentFloor).options()
  );
  const floorLevelsQuery = createQuery(
    rpc.floorPlans.getFloorLevels(data.home.id).options()
  );
  const saveMutation = createMutation(rpc.floorPlans.saveFloorPlan.options());
