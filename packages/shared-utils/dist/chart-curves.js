import { curveBasis, curveCardinal, curveCatmullRom, curveLinear, curveMonotoneX, curveNatural, curveStep, } from "d3-shape";
/**
 ...
 */
export function getCurveFunction(curveName) {
    if (!curveName)
        return curveLinear;
 ...
 */
export function isCurveString(value) {
 ...
 */
export function transformCurveInConfig(config) {
    if (!config["curve"])
        return config;
    // If curve is already a function, return as-is
    if (typeof config["curve"] === "function")
        return config;
...
//# sourceMappingURL=chart-curves.js.map