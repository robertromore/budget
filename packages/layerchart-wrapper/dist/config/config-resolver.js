 ...
 */
import { DEFAULT_AXES_CONFIG, DEFAULT_STYLING_CONFIG, DEFAULT_INTERACTIONS_CONFIG, DEFAULT_TIME_FILTERING_CONFIG, DEFAULT_CONTROLS_CONFIG, DEFAULT_ANNOTATIONS_CONFIG, DEFAULT_THRESHOLD_CONFIG, CHART_TYPE_DEFAULTS } from './chart-config';
 ...
 */
export function resolveChartConfig(props) {
    // Merge controls first to get available types
    const controls = deepMerge(deepMerge(DEFAULT_CONTROLS_CONFIG, {}), props.controls || {});
    ...
    const finalControls = deepMerge(deepMerge(DEFAULT_CONTROLS_CONFIG, typeDefaults.controls || {}), props.controls || {});
    // Merge configurations with priority: user config > type defaults > system defaults
    const axes = deepMerge(deepMerge(DEFAULT_AXES_CONFIG, typeDefaults.axes || {}), props.axes || {});
    const styling = deepMerge(deepMerge(DEFAULT_STYLING_CONFIG, typeDefaults.styling || {}), props.styling || {});
    const interactions = deepMerge(deepMerge(DEFAULT_INTERACTIONS_CONFIG, typeDefaults.interactions || {}), props.interactions || {});
    const timeFiltering = deepMerge(deepMerge(DEFAULT_TIME_FILTERING_CONFIG, typeDefaults.timeFiltering || {}), props.timeFiltering || {});
    const annotations = deepMerge(deepMerge(DEFAULT_ANNOTATIONS_CONFIG, typeDefaults.annotations || {}), props.annotations || {});
    const threshold = deepMerge(deepMerge(DEFAULT_THRESHOLD_CONFIG, typeDefaults.threshold || {}), props.threshold || {});
    // Resolve colors
 ...
 */
export function standardizeChartData(rawData, mapping) {
