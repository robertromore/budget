<script lang="ts">
  ...
  import SideBySideChartView from './side-by-side-chart-view.svelte';
  ...
  const showLegend = $derived(
    Boolean(config.styling.legend.show &&
      ...
      >
        ...
        <Tooltip.Context mode="bisect-x">
            ...
            <Group center>
              <DynamicChartRenderer
                ...
                config={{
                  ...
                  stroke: ['line', 'spline'].includes(chartType) ? (effectiveColors[0] || 'hsl(var(--chart-1))') : undefined,
                  curve: ['line', 'spline', 'area'].includes(chartType) && config.controls.allowCurveChange ? selectedCurve : undefined,
          ...
          {#if config.interactions?.highlight?.enabled && config.interactions?.tooltip?.enabled}
            ...
            <Highlight
              axis={crosshairAxis === 'none' ? 'none' : (crosshairAxis ?? 'both')}
              ...
              lines={highlight.showLines !== false ? {
                ...
                strokeWidth: 1,
                class: crosshairStyle === 'dashed' ? '[stroke-dasharray:5,3]' :
                       crosshairStyle === 'dotted' ? '[stroke-dasharray:2,2]' :
          ...
          {#if config.styling.points?.show && (['line', 'spline'].includes(chartType))}
            ...
            <Points
              ...
              fill={config.styling.points.fill === 'auto' ? pointColor : (config.styling.points.fill || pointColor)}
              stroke={config.styling.points.stroke === 'auto' ? pointColor : (config.styling.points.stroke || pointColor)}
            ...
            <Group center>
              <DynamicChartRenderer
                ...
                config={{
                  ...
                  strokeWidth: ['line', 'spline', 'threshold'].includes(chartType) ? 2 : undefined,
                  fill: ['line', 'spline'].includes(chartType) ? 'none' : undefined,
                  stroke: ['line', 'spline'].includes(chartType) ? (effectiveColors[1] || 'hsl(var(--chart-2))') : undefined,
                  curve: ['line', 'spline', 'area'].includes(chartType) && config.controls.allowCurveChange ? selectedCurve : undefined,
                  innerRadius: chartType === 'pie' ? 0 : chartType === 'arc' ? 40 : undefined,
                  outerRadius: chartType === 'pie' ? 80 : chartType === 'arc' ? 120 : undefined
                }}
    ...
    <Chart
      ...
      {...(!isChartCircular && !isChartHierarchical ? {
        ...
        ...(isChartCircular && effectiveColors.length > 0 ? {
          ...
          cDomain: chartData.map((d: any) => d.label || d.category || d.name).filter((v, i, a) => a.indexOf(v) === i),
      ...
      >
          ...
          <Group center>
            <DynamicChartRenderer
              ...
              config={{
                ...
                fill: ['line', 'spline'].includes(chartType) ? 'none' : undefined,
                stroke: ['line', 'spline'].includes(chartType) ? (effectiveColors[0] || 'hsl(var(--chart-1))') : undefined,
                curve: ['line', 'spline', 'area', 'threshold'].includes(chartType) && config.controls.allowCurveChange ? selectedCurve : undefined,
        ...
        {#if config.styling.points.show && ['area', 'line', 'spline'].includes(chartType)}
          ...
          <Points
            ...
            fill={config.styling.points.fill === 'auto' ? pointColor : (config.styling.points.fill || pointColor)}
            stroke={config.styling.points.stroke === 'auto' ? pointColor : (config.styling.points.stroke || pointColor)}
            strokeWidth={config.styling.points.strokeWidth ?? DEFAULT_STYLING_CONFIG.points.strokeWidth ?? 1}
            fillOpacity={config.styling.points.fillOpacity || 1.0}
          />
        {/if}
        ...
        {#if config.styling.points?.show && (['line', 'spline'].includes(chartType))}
          {#if isMultiSeries && seriesData.length > 0}
            ...
            {#each seriesData as series, index}
              {@const pointColor = effectiveColors[index % effectiveColors.length] || 'hsl(var(--chart-1))'}
              <Points
                data={series}
                r={config.styling.points.radius ?? DEFAULT_STYLING_CONFIG.points.radius ?? 6}
                fill={config.styling.points.fill === 'auto' ? pointColor : (config.styling.points.fill || pointColor)}
        ...
        {#if !isChartCircular}
          ...
          {#if config.styling.labels?.show}
            <Labels
              ...
              x="x"
              y="y"
              format={config.styling.labels.format || ((d) => {
