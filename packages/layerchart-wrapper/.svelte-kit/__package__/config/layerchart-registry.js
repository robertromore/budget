...
import { Arc, Area, Axis, Bars, Calendar, Circle, Grid, Group, Hull, Labels, Legend, 
// Line, // This is for straight lines between two points, not line charts
Pie, Points, Rule, Spline, Threshold, } from "layerchart";
/**
 * Registry of all available LayerChart components
 */
export const LAYERCHART_COMPONENT_REGISTRY = {
 ...
 */
export function getAvailableChartTypes(dataFormat, features) {
    return Object.entries(LAYERCHART_COMPONENT_REGISTRY)
        ...
        const featureMatch = !features || features.every((feature) => config.supportedFeatures.includes(feature));
 ...
 */
export function getComponentConfig(chartType) {
...
export function getComponentsByCategory(category) {
    return Object.fromEntries(Object.entries(LAYERCHART_COMPONENT_REGISTRY).filter(([_, config]) => config.category === category));
}
/**
 * Validate if data is compatible with chart type
 */
export function validateDataCompatibility(chartType, data) {
