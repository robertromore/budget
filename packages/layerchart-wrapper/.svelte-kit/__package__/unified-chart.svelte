<script lang="ts">
  import { calculateOptimalOpacity, createThemeDetector } from '@budget-shared/utils';
      ...
      config={{
        ...
        styling: {
          ...
          points: {
            ...
            show: config.controls.allowPointsChange ? (showPoints ?? false) : (config.styling.points?.show ?? false),
          ...
          grid: {
            ...config.styling.grid,
            show: config.controls.allowGridChange ? (showGrid ?? false) : (config.styling.grid?.show ?? false),
            horizontal: config.controls.allowGridChange ? (showHorizontal ?? false) : (config.styling.grid?.horizontal ?? false),
            vertical: config.controls.allowGridChange ? (showVertical ?? false) : (config.styling.grid?.vertical ?? false),
            opacity: config.controls.allowGridChange ? gridOpacity : (config.styling.grid?.opacity ?? 0.5)
          },
          labels: {
            ...config.styling.labels,
            show: config.controls.allowLabelChange ? (showLabels ?? false) : (config.styling.labels?.show ?? false),
