 ...
 */
function memoizeTransformation<T>(key: string, computeFn: () => T, maxCacheSize: number = 50): T {
...
export function createChartDataProcessor(props: ProcessorConfig): ChartDataProcessor {
  ...
  const processedData = (() => {
    ...
    const sourceData =
      !viewModeData || viewMode === "combined" ? data : viewModeData.combined || data;
    ...
    const hasSourceData =
      config.timeFiltering.sourceData?.length > 0 && config.timeFiltering.sourceProcessor;
  ...
  const chartData = (() => {
    ...
    return memoizeTransformation(cacheKey, () => {
      ...
      const transformOptionsInternal: any = {
        categoryField: categoryField || "category",
        valueField: "y",
        seriesField: "series",
      };
      ...
      if (config.styling.colors !== "auto") {
        transformOptionsInternal.colors = config.styling.colors as string[];
      }

      const transformed = transformDataForChartType(
        dataToProcess,
        chartType,
