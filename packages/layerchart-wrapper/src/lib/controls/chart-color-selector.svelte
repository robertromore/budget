<script lang="ts">
  ...
  import { CHART_COLOR_SCHEMES, getColorSchemesByCategory } from '@budget-shared/utils/chart-colors';
  ...
  <Select.Root type="single" bind:value={selectedScheme}>
      ...
      <div class="flex items-center gap-2">
        <div class="flex gap-0.5">
          {#each selectedColorScheme!.colors.slice(0, 4) as color}
            <div
              class="size-2.5 rounded-full border border-border/30"
    ...
    <Select.Content>
      {#each Object.entries(colorSchemesByCategory) as [categoryName, schemes]}
        {#if schemes.length > 0}
          <!-- Category Header -->
          <div class="px-2 py-1 text-xs font-medium text-muted-foreground uppercase tracking-wide border-b">
            {categoryName}
          </div>
              ...
              <div class="flex items-center gap-3">
                <div class="flex gap-1">
                  {#each scheme.colors.slice(0, 4) as color}
                    <div
                      class="w-3 h-3 rounded-full border border-border/30"
                      style="background-color: {color}"
                    ></div>
                  {/each}
                </div>
