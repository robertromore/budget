import { claimStatusKeys, medicalExpenseTypeKeys, receiptTypeKeys } from "$core/schema";
...
export const medicalExpensesRouter = t.router({
  ...
  update: publicProcedure.input(updateMedicalExpenseSchema).mutation(async ({ input }) => {
    try {
      ...
      if (inputData.treatmentDescription)
        data.treatmentDescription = inputData.treatmentDescription;
      if (inputData.amount !== undefined) data.amount = inputData.amount;
      if (inputData.insuranceCovered !== undefined)
        data.insuranceCovered = inputData.insuranceCovered;
