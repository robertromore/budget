...
/**
 * Optional budget state callbacks for optimistic UI updates.
...
let stateCallbacks: BudgetStateCallbacks = {};

export function setBudgetStateCallbacks(callbacks: BudgetStateCallbacks): void {
...
export const createBudget = defineMutation<CreateBudgetRequest, BudgetWithRelations>({
  ...
  onSuccess: (budget, variables) => {
    stateCallbacks.upsertBudget?.(budget);
...
>({
  ...
  onSuccess: (_, { id, deleteLinkedSchedule }) => {
    stateCallbacks.removeBudget?.(id);
