 ...
 */
function generateRealAccountExamples(
 ...
 */
function generateRealSpendingExamples(
  data: Awaited<ReturnType<typeof fetchWorkspaceData>>
): TrainingExample[] {
  const examples: TrainingExample[] = [];
 ...
 */
function generateRealPayeeExamples(
  data: Awaited<ReturnType<typeof fetchWorkspaceData>>
): TrainingExample[] {
  const examples: TrainingExample[] = [];
  ...
  const topPayees = data.payees
    ...
    .map(
      (p, i) =>
        `${i + 1}. **${p.name}**: ${formatCurrency(p.totalSpent!)} (${p.transactionCount} transactions)`
    )
    .join("\n");
 ...
 */
function generateRealTransactionExamples(
  data: Awaited<ReturnType<typeof fetchWorkspaceData>>
): TrainingExample[] {
  const examples: TrainingExample[] = [];
  ...
  for (const [payeeName, count] of payeeCounts) {
    if (count >= 2) {
      ...
      const rows = payeeTransactions
        .map(
          (t) =>
            `| ${formatDate(t.date)} | ${formatCurrency(t.amount)} | ${t.categoryName || "Uncategorized"} |`
        )
        .join("\n");
 ...
 */
function generateRealBudgetExamples(
  data: Awaited<ReturnType<typeof fetchWorkspaceData>>
): TrainingExample[] {
  const examples: TrainingExample[] = [];
  ...
  const budgetLines = data.budgets
    .map((b) => {
      ...
      const spent =
        data.categorySpending.find((c) => c.categoryId === b.categoryId)?.totalSpent || 0;
  ...
  for (const budget of data.budgets.slice(0, 3)) {
    ...
    const spent =
      data.categorySpending.find((c) => c.categoryId === budget.categoryId)?.totalSpent || 0;
...
export async function generateRealDataset(workspaceId: number): Promise<TrainingDataset> {
  ...
  console.log(
    `Found: ${data.accounts.length} accounts, ${data.payees.length} payees, ${data.recentTransactions.length} recent transactions`
  );
